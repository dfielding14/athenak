...
`docs/cgl_lf_phase_i_handoff.md` first. Accepted corrected-E03
`R02/s28_rankio_t9p75_t10` job `4761058` reached exact `t = 10` in `2335`
seconds. Formal and independently regenerated inspection pass with complete
terminal ranked products, zero strict LF failure counters, no restart-marker
bypass, terminal `lf_hwproj = 310229365059`, and forcing-work residual
...
threshold. That historical F-100 authorization was only
`R02/s18_rankio_t7_t7p25`. Authenticated `s18` job `4754394` is accepted
through exact `t = 7.25` for `0.650000` node-hours. Historical F-101 canonical
...
only `R02/s19_rankio_t7p25_t7p5`. F-102 hardened the helper before any `s19`
mutation. Authenticated `s19` job `4757300` is accepted through exact `t = 7.5`
for `0.680556` displayed node-hours, bringing corrected-E03 use to `17.463890`
displayed node-hours. Canonical F-103 recost evidence SHA-256
...
accepted through exact `t = 7.75` for `0.658333` displayed node-hours,
bringing corrected-E03 use to `18.122223` displayed node-hours. Canonical
F-104 recost evidence SHA-256
...
documentation checkpoint is committed, pushed, archived, and cataloged,
validate the archive ledger, rerun hardened reconciliation, and repeat the
queue/shared-root audit with explicit stale beta-25 acknowledgement. Require
completion of the durable handoff checklist items 9--11 before preparation.
Then prepare only `R02/s21_rankio_t7p75_t8` on one node from authenticated `s20`
...
timeout margins. It does not ratchet automatically. The reviewed timing
estimator selects `2560` seconds for the proposed `s21` quarter-unit segment.
That authorization is historical. Authenticated `s21` job `4758233` is
accepted through exact `t = 8.0` for `0.658056` displayed node-hours, bringing
...
validate the archive ledger, rerun hardened reconciliation, and repeat the
queue/shared-root audit with explicit stale beta-25 acknowledgement. Require
completion of the durable handoff checklist items 12--14 before preparation.
Then prepare only `R02/s22_rankio_t8_t8p25` on one node from authenticated
`s21` terminal siblings with Slurm walltime `01:05:00`, Athena timeout
`00:55:00`, and a one-segment `2700`-second threshold retaining `600` seconds
on both timeout margins. It does not ratchet automatically. The reviewed
timing estimator selects `2560` seconds for the proposed `s22` quarter-unit
segment.
That authorization is historical. Authenticated `s22` job `4758398` is
accepted through exact `t = 8.25` for `0.651389` displayed node-hours,
bringing corrected-E03 use to `19.431668` displayed node-hours. Canonical
F-106 recost evidence SHA-256
...
validate the archive ledger, rerun hardened reconciliation, and repeat the
queue/shared-root audit with explicit stale beta-25 acknowledgement. Require
completion of the durable handoff checklist items 15--17 before preparation.
Then prepare only `R02/s23_rankio_t8p25_t8p5` on one node from authenticated
`s22` terminal siblings with Slurm walltime `01:05:00`, Athena timeout
`00:55:00`, and a one-segment `2700`-second threshold retaining `600` seconds
on both timeout margins. It does not ratchet automatically. The reviewed
timing estimator selects `2560` seconds for the proposed `s23` quarter-unit
segment.
That authorization is historical. Authenticated `s23` job `4758475` is
accepted through exact `t = 8.5` for `0.634722` displayed node-hours,
bringing corrected-E03 use to `20.066390` displayed node-hours. Canonical
F-107 recost evidence SHA-256
...
validate the archive ledger, rerun hardened reconciliation, and repeat the
queue/shared-root audit with explicit stale beta-25 acknowledgement. Require
completion of the durable handoff checklist items 19--21 before preparation.
Then prepare only `R02/s24_rankio_t8p5_t8p75` on one node from authenticated
`s23` terminal siblings with Slurm walltime `01:05:00`, Athena timeout
`00:55:00`, and a one-segment `2700`-second threshold retaining `600` seconds
on both timeout margins. It does not ratchet automatically. The reviewed
timing estimator selects `2560` seconds for the proposed `s24` quarter-unit
segment.
That authorization is historical. Authenticated `s24` job `4758576` is
accepted through exact `t = 8.75` for `0.651667` displayed node-hours,
bringing corrected-E03 use to `20.718057` displayed node-hours. Canonical
F-108 recost evidence SHA-256
...
validate the archive ledger, rerun hardened reconciliation, and repeat the
queue/shared-root audit with explicit stale beta-25 acknowledgement. Require
completion of the durable handoff checklist items 22--24 before preparation.
Then prepare only `R02/s25_rankio_t8p75_t9` on one node from authenticated
`s24` terminal siblings with Slurm walltime `01:05:00`, Athena timeout
`00:55:00`, and a one-segment `2700`-second threshold retaining `600` seconds
on both timeout margins. It does not ratchet automatically. The reviewed
timing estimator selects `2560` seconds for the proposed `s25` quarter-unit
segment.
That authorization is historical. Authenticated `s25` job `4758713` is
accepted through exact `t = 9` for `0.627500` displayed node-hours,
bringing corrected-E03 use to `21.345557` displayed node-hours. Canonical
F-109 recost evidence SHA-256
...
validate the archive ledger, rerun hardened reconciliation, and repeat the
queue/shared-root audit with explicit stale beta-25 acknowledgement. Require
completion of the durable handoff checklist items 25--27 before preparation.
Then prepare only `R02/s26_rankio_t9_t9p25` on one node from authenticated
`s25` terminal siblings with Slurm walltime `01:05:00`, Athena timeout
`00:55:00`, and a one-segment `2700`-second threshold retaining `600` seconds
on both timeout margins. It does not ratchet automatically. The reviewed
timing estimator selects `2560` seconds for the proposed `s26` quarter-unit
segment.
That authorization is historical. Authenticated `s26` job `4759155` is
accepted through exact `t = 9.25` for `0.643889` displayed node-hours,
bringing corrected-E03 use to `21.989446` displayed node-hours. Canonical
F-110 recost evidence SHA-256
...
continuation is `R02/s27_rankio_t9p25_t9p75` on one node with Slurm walltime
...
reviewed cadence is half-unit segmentation. Derive R17 bounds separately from
...
accepted through exact `t = 9.75` for `1.358056` displayed node-hours,
bringing corrected-E03 use to `23.347502` displayed node-hours. Canonical
F-111 recost evidence SHA-256
...
quarter is `R02/s28_rankio_t9p75_t10` on one node with Slurm walltime
`01:10:00`, Athena timeout `01:00:00`, and a `3000`-second threshold retaining
`600` seconds on both timeout margins. Finish and analyze R02 once through
exact `t = 10`; then use the largest evidence-backed standard-layout segments
that fit the hard two-hour limit and both guards for R03--R16. Derive R17
bounds separately from retained eight-node timing evidence and execute R17
last.
...
| Production accounting path | `scripts/frontier/cgl_lf_stage_i.py` validates mapped cases and aliases, uses `batch` with default production `normal` QOS, requires the E03 approval token, archives executable/input/bundle/utility provenance, submits atomically under lock, enforces sequential reservations, rejects requests above two hours, authenticates restart markers, and requires formal inspection before record. E03 started empty by design and now records accepted R02 jobs through `4759856` and exact `t = 9.75` at `23.347502` displayed cumulative node-hours. Hardened reconciliation reports `28/28/28` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. F-103 through F-111 are normal observed publications under the retained lifecycle companion. | Before preparing a successor, commit, push, archive, and catalog the post-F-111 documentation checkpoint, validate the archive ledger, and require completion of the durable handoff checklist items 31--32 before preparing only bounded final-quarter `R02/s28_rankio_t9p75_t10` on one node from authenticated `s27` terminal restart siblings with Slurm walltime `01:10:00`, Athena timeout `01:00:00`, and its reviewed one-segment `3000`-second threshold retaining `600` seconds on both timeout margins. Inspect, account, reconcile, and recost, then finish and analyze R02 once through exact `t = 10`. Execute R03--R16 with the largest evidence-backed standard-layout segments fitting the hard two-hour limit and both guards. Derive R17 bounds separately from retained eight-node timing evidence and execute R17 last. |
...
Job `4757300` used verified launch bundle
`athenak-feature-cgl-through-28113fd06.bundle` with SHA-256
`e1910db1dcb008a6b39888536e7b6fabf527fac525fb3550b466a7801ffafaf0`.
Job `4757761` used verified launch bundle
`athenak-feature-cgl-through-07e044440.bundle` with SHA-256
`4dc5c3820b385401e0d60bfaecde9bf91fbb16467ae4ff5c6604c35652beec15`.
Job `4758233` used verified launch bundle
`athenak-feature-cgl-through-aa3e2a59d.bundle` with SHA-256
`e8a0f2028d67f908efd3ef0390f2994321ef11a6a1aa6f0d1cbb78732c18182b`.
Job `4758398` used verified launch bundle
`athenak-feature-cgl-through-4bb580e22.bundle` with SHA-256
`8e4f6dbc5a7318b3e17db33348ca8fdd346c5ba8d161faaf20c23a0fdeb49411`.
Job `4758475` used verified launch bundle
`athenak-feature-cgl-through-849b052dd.bundle` with SHA-256
`adf897b8f60149bdcf89bc0a42f69f224ec6486155341411df1ccd8bd2d7b8ed`.
Job `4758576` used verified launch bundle
`athenak-feature-cgl-through-6d1f032a2.bundle` with SHA-256
`62587378e8480cbe94caa174734f6d3cf6e4c56c2755b2b96ffe0e2bba525ef4`.
...
Job `4759856` used verified launch bundle
`athenak-feature-cgl-through-b9fcf7a5e.bundle` with SHA-256
`e85c66511cc399cbe893b9abcbf7c64160907b2167a35745d2135d2a9cec761e`.
The sole candidate bounded final-quarter `s28` launch requires the committed
corrected post-F-111 documentation archive. Its actual bundle name and SHA-256
...
| Accepted corrected E03 R02 quarter-unit continuation through exact `t = 7.25` | Job `4754394`; retained inspection SHA-256 `400cec346d2fe9be2fd15c8de0b44c9f1edf24e7c2903e22dd6949960ca6d636`; retained independent validation SHA-256 `5760966f1ccaf2c88f4c14307de281864e1aa93cdee01a33ff513feac3662905` | `0.650000` |
| Corrected E03 R02 prefix through exact `t = 7.25` | Jobs through `4754394`; truthfully adopted F-101 recost evidence JSON SHA-256 `df208829aec85c88fcc2caa757a21ad3ff3372aeb82628cbfe7bd41851faed9a` | `16.783334` |
| Accepted corrected E03 R02 quarter-unit continuation through exact `t = 7.5` | Job `4757300`; retained inspection SHA-256 `8b9cfb6f3d0ec215234d9481e031299da1286117424d756f654270930881f744`; retained independent validation SHA-256 `6ebd5054e8d8082d75343b9cd176d7faf8c3b773774d8e145302bf3bfd859dcf` | `0.680556` |
| Corrected E03 R02 prefix through exact `t = 7.5` | Jobs through `4757300`; canonical F-103 recost evidence JSON SHA-256 `ab7fa80746fa4d2c65f515b3ace71d5668586e27fde91bbdeb578fc3a4509d98` | `17.463890` |
| Accepted corrected E03 R02 quarter-unit continuation through exact `t = 7.75` | Job `4757761`; retained inspection SHA-256 `4094abc01fb9fa2f4e690f17c549270d519a1fbbeab63c49ff8856bb4cfba791`; retained independent validation SHA-256 `eec64e5b065414912385549c2b42406683ec6fce4787bd525c357a4226ef958f` | `0.658333` |
| Corrected E03 R02 prefix through exact `t = 7.75` | Jobs through `4757761`; canonical F-104 recost evidence JSON SHA-256 `f8d05a4483a902f21f49c9ebb8935dba3e21d4186c40eda050ae57d169f6bb41` | `18.122223` |
| Accepted corrected E03 R02 quarter-unit continuation through exact `t = 8.0` | Job `4758233`; retained inspection SHA-256 `6c1fe59719c109b011b9c2db4d8e6ff5af57b028c818c5f29b2b370203112199`; retained independent validation SHA-256 `11d20c2cf311563ea36b366f472cee90323d4835d59e9d9ab4f9776cefe1ab44` | `0.658056` |
| Corrected E03 R02 prefix through exact `t = 8.0` | Jobs through `4758233`; canonical F-105 recost evidence JSON SHA-256 `242b4ae79b215690d2b552aba9e583cf209d70c822f8dd76da4e7af1b7ba6a5b` | `18.780279` |
| Accepted corrected E03 R02 quarter-unit continuation through exact `t = 8.25` | Job `4758398`; retained inspection SHA-256 `77479a51cda10b1a399a9060c5a7153dec115389cd4f0ba5a6fec167c462be49`; retained independent validation SHA-256 `e8854e4b233d7b1b64fcc53e69975041e052e98477b85e74b8e0dc1d5a332d08` | `0.651389` |
| Corrected E03 R02 prefix through exact `t = 8.25` | Jobs through `4758398`; canonical F-106 recost evidence JSON SHA-256 `fe78232b5396d9d3ee6a5ef0e39f7aa445cf2c17125691a7d3e63d4a67978bc2` | `19.431668` |
| Accepted corrected E03 R02 quarter-unit continuation through exact `t = 8.5` | Job `4758475`; retained inspection SHA-256 `318786f1be49ef894606f579709942bdb38f0de37c4ab23b357569b0deb36b2b`; retained independent validation SHA-256 `08cd3c894106f295eda79571faca85dc23c1aaf621ba1a5c1eb09c253bb152fe` | `0.634722` |
| Corrected E03 R02 prefix through exact `t = 8.5` | Jobs through `4758475`; canonical F-107 recost evidence JSON SHA-256 `1e4db459d7c0b5211108dee07fe1efa643ac7a655adadb907deb57f36822d699` | `20.066390` |
| Accepted corrected E03 R02 quarter-unit continuation through exact `t = 8.75` | Job `4758576`; retained inspection SHA-256 `24a7013abca332a65196552f0ce1a6031314deb56e8ddbebb130b75be4b887de`; retained independent validation SHA-256 `0a271b1f53f3363c2598a76356a7ca3063a38e5d8c0ab63a64e15b4ebcfa2229` | `0.651667` |
| Corrected E03 R02 prefix through exact `t = 8.75` | Jobs through `4758576`; canonical F-108 recost evidence JSON SHA-256 `dbd3f5e3b9f21532da546caca4bdc66263d69c3b60b35938972edd04b2ab3e19` | `20.718057` |
| Accepted corrected E03 R02 quarter-unit continuation through exact `t = 9` | Job `4758713`; retained inspection SHA-256 `313cb0565ad7d3f7bbe63be63cbd6055de951842cb101aeb523102a37f2b2f48`; retained independent validation SHA-256 `d2b8d4d044d4569c25a089bd5e3a23839c785e81ca258da091b9d718d63aa5cb` | `0.627500` |
| Corrected E03 R02 prefix through exact `t = 9` | Jobs through `4758713`; canonical F-109 recost evidence JSON SHA-256 `67f9096900b5ec3ce8368cd39c6f6a8c649dcff2436261fcf72aa328cd3ac0ea` | `21.345557` |
| Accepted corrected E03 R02 quarter-unit continuation through exact `t = 9.25` | Job `4759155`; retained inspection SHA-256 `36ab7085547ddf32eaab0acc89b8376cbe721202bc9449a6a255df7cb6136bb3`; retained independent validation SHA-256 `24ee6e4a0abefdb50e0d2ce974156b88ac22b7b9070d03a4569103a2c44fd3c9` | `0.643889` |
| Corrected E03 R02 prefix through exact `t = 9.25` | Jobs through `4759155`; canonical F-110 recost evidence JSON SHA-256 `26f6eeb1a92960c006126b64e73c51b4e2a06c14efff50a04660aad36f8b8706` | `21.989446` |
| Accepted corrected E03 R02 aggressive bounded half-unit continuation through exact `t = 9.75` | Job `4759856`; retained inspection SHA-256 `16c840e2ccc62d9a9648ca00fc7bc0240157fc99ef675dbf7f290a2ed827f004`; retained generalized independent validation SHA-256 `32cf08cbc44fde55a3c4adfffb95262fb1e10e285204cec9ff752dc7fa10b513` | `1.358056` |
| Corrected E03 R02 prefix through exact `t = 9.75` | Jobs through `4759856`; canonical F-111 recost evidence JSON SHA-256 `354dde3f93a6ebe73532e2b7f851a8068675b7798964c707146c37428cb9b195` | `23.347502` |
...
10. Preserve the completed corrected-policy qualification and retained
    ...
    Authenticated quarter-unit continuation job `4754394` is accepted through
    exact `t = 7.25`; the corrected prefix then used `16.783334` displayed
    node-hours. Historical F-101 recost evidence SHA-256
    `df208829aec85c88fcc2caa757a21ad3ff3372aeb82628cbfe7bd41851faed9a`
    was truthfully adopted under the retained companion. Authenticated
    quarter-unit continuation job `4757300` is accepted through exact
    `t = 7.5`; the corrected prefix now uses `17.463890` displayed node-hours
    (`62870 / 3600 = 17.46388888888889` exact node-hours). Canonical F-103
    recost evidence JSON SHA-256 is
    `ab7fa80746fa4d2c65f515b3ace71d5668586e27fde91bbdeb578fc3a4509d98`.
    Authenticated quarter-unit continuation job `4757761` is accepted through
    exact `t = 7.75`; the corrected prefix now uses `18.122223` displayed
    node-hours (`65240 / 3600 = 18.122222222222224` exact node-hours).
    Canonical F-104 recost evidence JSON SHA-256 is
    `f8d05a4483a902f21f49c9ebb8935dba3e21d4186c40eda050ae57d169f6bb41`.
    Authenticated quarter-unit continuation job `4758233` is accepted through
    exact `t = 8.0`; the corrected prefix now uses `18.780279` displayed
    node-hours (`67609 / 3600 = 18.780277777777776` exact node-hours).
    Canonical F-105 recost evidence JSON SHA-256 is
    `242b4ae79b215690d2b552aba9e583cf209d70c822f8dd76da4e7af1b7ba6a5b`.
    Authenticated quarter-unit continuation job `4758398` is accepted through
    exact `t = 8.25`; the corrected prefix now uses `19.431668` displayed
    node-hours (`69954 / 3600 = 19.43166666666667` exact node-hours).
    Canonical F-106 recost evidence JSON SHA-256 is
    `fe78232b5396d9d3ee6a5ef0e39f7aa445cf2c17125691a7d3e63d4a67978bc2`.
    Authenticated quarter-unit continuation job `4758475` is accepted through
    exact `t = 8.5`; the corrected prefix now uses `20.066390` displayed
    node-hours (`72239 / 3600 = 20.066388888888888` exact node-hours).
    Canonical F-107 recost evidence JSON SHA-256 is
    `1e4db459d7c0b5211108dee07fe1efa643ac7a655adadb907deb57f36822d699`.
    Authenticated quarter-unit continuation job `4758576` is accepted through
    exact `t = 8.75`; the corrected prefix now uses `20.718057` displayed
    node-hours (`74585 / 3600 = 20.718055555555555` exact node-hours).
    Canonical F-108 recost evidence JSON SHA-256 is
    `dbd3f5e3b9f21532da546caca4bdc66263d69c3b60b35938972edd04b2ab3e19`.
    Authenticated quarter-unit continuation job `4758713` is accepted through
    exact `t = 9`; the corrected prefix now uses `21.345557` displayed
    node-hours (`76844 / 3600 = 21.345555555555556` exact node-hours).
    Canonical F-109 recost evidence JSON SHA-256 is
    `67f9096900b5ec3ce8368cd39c6f6a8c649dcff2436261fcf72aa328cd3ac0ea`.
...
`t = 7.0`; promoted F-100 recost evidence historically authorized only
`R02/s18_rankio_t7_t7p25`.
That quarter-unit continuation is now accepted as job `4754394` through exact
`t = 7.25`; historical F-101 recost evidence was truthfully adopted under the
retained companion and historically authorized only
`R02/s19_rankio_t7p25_t7p5`. F-102 hardened the Stage I helper before any `s19`
mutation.
That quarter-unit continuation is now accepted as job `4757300` through exact
`t = 7.5`; canonical F-103 recost evidence normally published under the
retained companion historically authorized only `R02/s20_rankio_t7p5_t7p75`.
That quarter-unit continuation is now accepted as job `4757761` through exact
`t = 7.75`; canonical F-104 recost evidence normally published under the
retained companion conditionally authorizes only `R02/s21_rankio_t7p75_t8`
on one node from authenticated `s20` terminal siblings next after the
post-F-104 documentation checkpoint is committed, pushed, archived, and
cataloged, the archive ledger is validated, hardened reconciliation is rerun,
and the queue/shared-root audit repeats explicit stale beta-25 acknowledgement.
Require completion of the durable handoff checklist items 9--11 before
preparation. Use Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a reviewed
one-segment `2700`-second threshold retaining `600` seconds on both timeout
margins. The threshold is scoped only to `s21` and cannot ratchet
automatically.
That quarter-unit continuation is now accepted as job `4758233` through exact
`t = 8.0`; canonical F-105 recost evidence normally published under the
retained companion conditionally authorizes only `R02/s22_rankio_t8_t8p25`
on one node from authenticated `s21` terminal siblings next after the
post-F-105 documentation checkpoint is committed, pushed, archived, and
cataloged, the archive ledger is validated, hardened reconciliation is rerun,
and the queue/shared-root audit repeats explicit stale beta-25 acknowledgement.
Require completion of the durable handoff checklist items 12--14 before
preparation. Use Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s22` and cannot ratchet
automatically.
That quarter-unit continuation is now accepted as job `4758398` through exact
`t = 8.25`; canonical F-106 recost evidence normally published under the
retained companion conditionally authorizes only `R02/s23_rankio_t8p25_t8p5`
on one node from authenticated `s22` terminal siblings next after the
post-F-106 documentation checkpoint is committed, pushed, archived, and
cataloged, the archive ledger is validated, hardened reconciliation is rerun,
and the queue/shared-root audit repeats explicit stale beta-25 acknowledgement.
Require completion of the durable handoff checklist items 15--17 before
preparation. Use Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s23` and cannot ratchet
automatically.
That quarter-unit continuation is now accepted as job `4758475` through exact
`t = 8.5`; canonical F-107 recost evidence normally published under the
retained companion conditionally authorizes only `R02/s24_rankio_t8p5_t8p75`
on one node from authenticated `s23` terminal siblings next after the
post-F-107 documentation checkpoint is committed, pushed, archived, and
cataloged, the archive ledger is validated, hardened reconciliation is rerun,
and the queue/shared-root audit repeats explicit stale beta-25 acknowledgement.
Require completion of the durable handoff checklist items 19--21 before
preparation. Use Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s24` and cannot ratchet
automatically.
That quarter-unit continuation is now accepted as job `4758576` through exact
`t = 8.75`; canonical F-108 recost evidence normally published under the
retained companion conditionally authorizes only `R02/s25_rankio_t8p75_t9`
on one node from authenticated `s24` terminal siblings next after the
post-F-108 documentation checkpoint is committed, pushed, archived, and
cataloged, the archive ledger is validated, hardened reconciliation is rerun,
and the queue/shared-root audit repeats explicit stale beta-25 acknowledgement.
Require completion of the durable handoff checklist items 22--24 before
preparation. Use Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s25` and cannot ratchet
automatically.
That quarter-unit continuation is now accepted as job `4758713` through exact
`t = 9`; canonical F-109 recost evidence normally published under the
retained companion conditionally authorizes only `R02/s26_rankio_t9_t9p25`
on one node from authenticated `s25` terminal siblings next after the
post-F-109 documentation checkpoint is committed, pushed, archived, and
cataloged, the archive ledger is validated, hardened reconciliation is rerun,
and the queue/shared-root audit repeats explicit stale beta-25 acknowledgement.
Require completion of the durable handoff checklist items 25--27 before
preparation. Use Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
...
That aggressive bounded half-unit continuation is now accepted as job
`4759856` through exact `t = 9.75`; canonical F-111 recost evidence normally
published under the retained companion authorizes bounded final-quarter
`R02/s28_rankio_t9p75_t10` on one node from authenticated `s27` terminal
siblings next after the post-F-111 documentation checkpoint is committed,
pushed, archived, and cataloged, the archive ledger is validated,
authenticated hardened reconciliation is rerun, and the scoped
queue/shared-root audit requires no queued `cgl_*` workflow job, a free strict
root lock, and explicit stale beta-25 acknowledgement. Use Slurm walltime
`01:10:00`, Athena timeout `01:00:00`, and the reviewed `3000`-second threshold
retaining both `600`-second guards. Actual AthenaK submission retains the
Stage I helper's separate all-user queue preflight.
...
| D. MKS24 production | Preserve completed E02 R16 and R02/R17 timing products only as pipeline and cost evidence. The reviewed E03 token and clean reconciliation now bind fresh R02 jobs through `4759856`, accepted from `t = 0` through exact `t = 9.75` for `23.347502` displayed cumulative node-hours. F-079--F-111 controller provenance, hardening, accepted-segment recosting, truthful F-101 legacy adoption, and normal retained-companion publications are archived or promoted. Before preparing a successor, commit, push, archive, and catalog the post-F-111 documentation checkpoint, validate the archive ledger, and require completion of the durable handoff checklist items 31--32 before preparing only bounded final-quarter `R02/s28_rankio_t9p75_t10` on one node from authenticated `s27` terminal siblings with Slurm walltime `01:10:00`, Athena timeout `01:00:00`, and its reviewed one-segment `3000`-second threshold retaining `600` seconds on both timeout margins. Inspect, account, reconcile, and recost; then finish and analyze R02 once through exact `t = 10`. Execute R03--R16 using the largest evidence-backed standard-layout segments fitting the hard two-hour limit and both guards. Derive R17 bounds separately from retained eight-node timing evidence and execute R17 last. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. The beta-1 inventory definition remains excluded unless Phase A assigns it a published role. | Accepted corrected-epoch run bundles and measured ledger/storage. | Forcing fidelity, safety, stationarity, accounting, storage, or budget gate fails. |
...
As of the accepted job `4759856` record, corrected F-111 recost, and normal
observed-publication timestamp `2026-06-03T19:35:41+00:00`:
...
5. The next task is not to launch the weak-guide extension or continue old
   ...
   `18/18/18` ledger rows/manifests/reservations, no active reservation, no
   transaction, and `issues = []`. Authenticated `R02/s18_rankio_t7_t7p25`
   job `4754394` is accepted through exact `t = 7.25` for `2340` seconds
   (`0.650000` node-hours). Historical F-101 canonical recost evidence SHA-256
   `df208829aec85c88fcc2caa757a21ad3ff3372aeb82628cbfe7bd41851faed9a`
   was truthfully adopted under the retained companion. F-102 hardened the
   helper before any `s19` mutation. Authenticated
   `R02/s19_rankio_t7p25_t7p5` job `4757300` is accepted through exact
   `t = 7.5` for `2450` seconds (`0.680556` displayed node-hours), bringing
   corrected-E03 use to `17.463890` displayed node-hours. It retains one
   complete eight-rank snapshot group and terminal eight-rank restart siblings,
   zero strict LF failure counters, no restart-marker bypass, terminal
   `lf_hwproj = 279167641195`, and sampled-history forcing-work relative
   residual `8.520423303056633e-12`. Canonical F-103 recost evidence SHA-256 is
   `ab7fa80746fa4d2c65f515b3ace71d5668586e27fde91bbdeb578fc3a4509d98`;
   observed-publication audit SHA-256 is
   `28e8526eb3c225cde028476dea95657b170264ea7f77156b8a839e2fe03bbf6b`.
   Authenticated `R02/s20_rankio_t7p5_t7p75` job `4757761` is accepted
   through exact `t = 7.75` for `2370` seconds (`0.658333` displayed
   node-hours), bringing corrected-E03 use to `18.122223` displayed
   node-hours. It retains one complete eight-rank snapshot group and terminal
   eight-rank restart siblings, zero strict LF failure counters, no
   restart-marker bypass, terminal `lf_hwproj = 282606449055`, and
   sampled-history forcing-work relative residual `3.7666708324783834e-12`.
   ...
   exact `t = 8.0` for `2369` seconds (`0.658056` displayed node-hours),
   bringing corrected-E03 use to `18.780279` displayed node-hours. It retains
   ...
   terminal `lf_hwproj = 286108988748`, and sampled-history forcing-work
   relative residual `2.8542854449297643e-12`. Canonical F-105 recost evidence
   SHA-256 is
   `242b4ae79b215690d2b552aba9e583cf209d70c822f8dd76da4e7af1b7ba6a5b`;
   observed-publication audit SHA-256 is
   ...
   exact `t = 8.25` for `2345` seconds (`0.651389` displayed node-hours),
   bringing corrected-E03 use to `19.431668` displayed node-hours. It retains
   one complete eight-rank snapshot group and terminal eight-rank restart
   siblings, zero strict LF failure counters, no restart-marker bypass,
   terminal `lf_hwproj = 290890469421`, and sampled-history forcing-work
   relative residual `3.021717626579328e-12`. Canonical F-106 recost evidence
   SHA-256 is
   `fe78232b5396d9d3ee6a5ef0e39f7aa445cf2c17125691a7d3e63d4a67978bc2`;
   observed-publication audit SHA-256 is
   ...
   exact `t = 8.5` for `2285` seconds (`0.634722` displayed node-hours),
   bringing corrected-E03 use to `20.066390` displayed node-hours. It retains
   one complete eight-rank snapshot group and terminal eight-rank restart
   siblings, zero strict LF failure counters, no restart-marker bypass,
   terminal `lf_hwproj = 295614667845`, and sampled-history forcing-work
   relative residual `2.3508932051548193e-12`. Canonical F-107 recost evidence
   SHA-256 is
   `1e4db459d7c0b5211108dee07fe1efa643ac7a655adadb907deb57f36822d699`;
   observed-publication audit SHA-256 is
   ...
   exact `t = 8.75` for `2346` seconds (`0.651667` displayed node-hours),
   bringing corrected-E03 use to `20.718057` displayed node-hours. It retains
   one complete eight-rank snapshot group and terminal eight-rank restart
   siblings, zero strict LF failure counters, no restart-marker bypass,
   terminal `lf_hwproj = 299627446834`, and sampled-history forcing-work
   relative residual `1.4737883591844796e-12`. Canonical F-108 recost evidence
   SHA-256 is
   `dbd3f5e3b9f21532da546caca4bdc66263d69c3b60b35938972edd04b2ab3e19`;
   observed-publication audit SHA-256 is
   ...
   exact `t = 9` for `2259` seconds (`0.627500` displayed node-hours),
   bringing corrected-E03 use to `21.345557` displayed node-hours. It retains
   one complete eight-rank snapshot group and terminal eight-rank restart
   siblings, zero strict LF failure counters, no restart-marker bypass,
   terminal `lf_hwproj = 302201795827`, and sampled-history forcing-work
   relative residual `1.83194669112365e-12`. Canonical F-109 recost evidence
   SHA-256 is
   `67f9096900b5ec3ce8368cd39c6f6a8c649dcff2436261fcf72aa328cd3ac0ea`;
   observed-publication audit SHA-256 is
   ...
   exact `t = 9.75` for `4889` seconds (`1.358056` displayed node-hours),
   bringing corrected-E03 use to `23.347502` displayed node-hours. It retains
   two complete eight-rank snapshot groups and terminal eight-rank restart
   siblings, zero strict LF failure counters, no restart-marker bypass,
   terminal `lf_hwproj = 308629878188`, and sampled-history forcing-work
   relative residual `4.667144504856836e-13`. Canonical F-111 recost evidence
   SHA-256 is
   `354dde3f93a6ebe73532e2b7f851a8068675b7798964c707146c37428cb9b195`;
   observed-publication audit SHA-256 is
   ...
   `issues = []`. Commit, push, archive, and catalog the post-F-111
   ...
   only bounded final-quarter `R02/s28_rankio_t9p75_t10` on one node from
   authenticated `s27` terminal siblings with Slurm walltime `01:10:00`, Athena
   timeout `01:00:00`, and its reviewed one-segment `3000`-second threshold
   retaining both `600`-second guards. Actual AthenaK submission retains the
