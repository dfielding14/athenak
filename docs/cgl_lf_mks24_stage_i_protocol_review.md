...
Authenticated `R02/s20_rankio_t7p5_t7p75` job `4757761` is formally and
independently inspected and recorded `accepted` through exact `t = 7.75` for
`0.658333` displayed node-hours, bringing corrected E03 Stage I use to
`18.122223` displayed node-hours.
...
That F-103 authorization is historical. Authenticated
`R02/s20_rankio_t7p5_t7p75` job `4757761` is accepted through exact `t = 7.75`
for `2370` seconds (`0.658333` displayed node-hours). Its exact scheduler row
is `4757761|cgl_mks24_E03_forcing_policy_R02_s20_rankio_t7p5_t7p75|COMPLETED|0:0|1|2370|2026-06-02T22:01:48|2026-06-02T22:41:35`;
formal inspection SHA-256 is
`4094abc01fb9fa2f4e690f17c549270d519a1fbbeab63c49ff8856bb4cfba791`;
recorded manifest SHA-256 is
`06e70f554196b0da40cb343bf0c5a7bd8e9b7ee2fafdf0e88e4019a9980a6ea0`;
retained independent validation SHA-256 is
`eec64e5b065414912385549c2b42406683ec6fce4787bd525c357a4226ef958f`;
retained scheduler evidence SHA-256 is
`0d726cffe32b00f14ccad5627bf4823eb9092da491c2c5aba878a52b25040573`.
It retains one complete eight-rank snapshot group and terminal eight-rank
restart siblings, zero strict LF failure counters, finite synchronized
histories, no restart-marker bypass, terminal `lf_hwproj = 282606449055`, and
sampled-history forcing-work relative residual `3.7666708324783834e-12`.
The accepted prefix totals `65240` seconds (`18.122222222222224` exact and
`18.122223` displayed node-hours). Hardened reconciliation closes with
`21/21/21` ledger rows/manifests/reservations, no active reservation, no
transaction, and `issues = []`.

The retained F-104 generator SHA-256 is
`ca3eebaaf0d04dd82a3daa9ee40006a888e0ac82ca74872e8bc63afb8a54e971`.
...
The normal retained-companion publication leaves canonical-only bytes, a
mode-`0444` forensic copy, empty transaction directories, an empty user queue,
and a free strict lock. With prior selected `2560`, previous observed `2450`,
observed `2370`, and local acceleration `2370`, the reviewed `s21` estimator
selects `2560 <= 2700` seconds. It projects `829.2566672222222` matrix
node-hours with `70.74333277777782` margin. Commit, push, archive, and catalog
the post-F-104 documentation checkpoint, validate the archive ledger, rerun
hardened reconciliation, and repeat the queue/shared-root audit with explicit
acknowledgement of the stale beta-25 record. Require completion of the durable
handoff checklist items 9--11 before preparation. Then prepare only
`R02/s21_rankio_t7p75_t8` on one node from authenticated `s20` terminal
siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s21` and cannot ratchet
automatically. Any scientific, provenance, scheduler, storage, budget, or
reconciliation failure blocks successor preparation. Inspect, account,
reconcile, and recost before any further extension. Finish R02, execute
R03--R16 sequentially, and complete `R17` last.

That F-104 authorization is also historical. Authenticated
`R02/s21_rankio_t7p75_t8` job `4758233` is accepted through exact `t = 8.0`
for `2369` seconds (`0.658056` displayed node-hours). Its exact retained
scheduler row is
`4758233|cgl_mks24_E03_forcing_policy_R02_s21_rankio_t7p75_t8|COMPLETED|0:0|1|2369|2026-06-02T23:59:21|2026-06-03T00:46:02`;
formal inspection SHA-256 is
`6c1fe59719c109b011b9c2db4d8e6ff5af57b028c818c5f29b2b370203112199`;
recorded manifest SHA-256 is
`69bd7db74380862fb887f218499879774a3a3c6e94ecbc3ba9ccf855a06b4ca2`;
retained independent validation SHA-256 is
`11d20c2cf311563ea36b366f472cee90323d4835d59e9d9ab4f9776cefe1ab44`;
retained scheduler evidence SHA-256 is
`24a54707bcf8de9cf7fd85f7a975d58900c35bdfe0289d354e612299120f4f21`.
It retains one complete eight-rank snapshot group and terminal eight-rank
restart siblings, zero strict LF failure counters, finite synchronized
histories, no restart-marker bypass, terminal `lf_hwproj = 286108988748`, and
sampled-history forcing-work relative residual `2.8542854449297643e-12`.
The accepted prefix totals `67609` seconds (`18.780277777777776` exact and
`18.780279` displayed node-hours). Hardened reconciliation closes with
`22/22/22` ledger rows/manifests/reservations, no active reservation, no
transaction, and `issues = []`.

The retained F-105 generator SHA-256 is
`0d06691c0fcd30dbd8796f3fdd3dcab70486f772638cfcdcfabbb2b5298b3896`.
Canonical F-105 recost evidence SHA-256 is
...
The normal retained-companion publication leaves canonical-only bytes, a
mode-`0444` forensic copy, empty transaction directories, an empty user queue,
and a free strict lock. With prior selected `2560`, observed `2369`, and local
acceleration `2369`, the reviewed `s22` estimator selects `2560 <= 2700`
seconds. It projects `829.2566672222222` matrix node-hours with
`70.74333277777782` margin. Commit, push, archive, and catalog the post-F-105
documentation checkpoint, validate the archive ledger, rerun hardened
reconciliation, and repeat the queue/shared-root audit with explicit
acknowledgement of the stale beta-25 record. Require completion of the durable
handoff checklist items 12--14 before preparation. Then prepare only
...
That F-105 authorization is also historical. Authenticated
`R02/s22_rankio_t8_t8p25` job `4758398` is accepted through exact `t = 8.25`
for `2345` seconds (`0.651389` displayed node-hours). Its exact retained
scheduler row is
...
formal inspection SHA-256 is
`77479a51cda10b1a399a9060c5a7153dec115389cd4f0ba5a6fec167c462be49`;
recorded manifest SHA-256 is
`b02e754cf521a0b0e2205bcd13947c0c12b6a443adb268f9ea91a2eeeff5a803`;
retained independent validation SHA-256 is
`e8854e4b233d7b1b64fcc53e69975041e052e98477b85e74b8e0dc1d5a332d08`;
retained scheduler evidence SHA-256 is
`e6af51d0cbbd1aef3e83df0fa99f64da77d7aa991a59c0375b21b4fe07c44ea1`.
It retains one complete eight-rank snapshot group and terminal eight-rank
restart siblings, zero strict LF failure counters, finite synchronized
histories, no restart-marker bypass, terminal `lf_hwproj = 290890469421`, and
sampled-history forcing-work relative residual `3.021717626579328e-12`.
The accepted prefix totals `69954` seconds (`19.43166666666667` exact and
`19.431668` displayed node-hours). Hardened reconciliation closes with
`23/23/23` ledger rows/manifests/reservations, no active reservation, no
transaction, and `issues = []`.

The retained F-106 generator SHA-256 is
`341fda3812143288806285b6f8ec3969d43d0e0bd22f150190e5e4f44c57b3b6`.
Canonical F-106 recost evidence SHA-256 is
...
The normal retained-companion publication leaves canonical-only bytes, a
mode-`0444` forensic copy, empty transaction directories, an empty user queue,
and a free strict lock. With prior selected `2560`, observed `2345`, and local
acceleration `2345`, the reviewed `s23` estimator selects `2560 <= 2700`
seconds. It projects `829.2566672222222` matrix node-hours with
`70.74333277777782` margin. Commit, push, archive, and catalog the post-F-106
documentation checkpoint, validate the archive ledger, rerun hardened
reconciliation, and repeat the queue/shared-root audit with explicit
acknowledgement of the stale beta-25 record. Require completion of the durable
handoff checklist items 15--17 before preparation. Then prepare only
...
That F-107 authorization is also historical. Authenticated
`R02/s24_rankio_t8p5_t8p75` job `4758576` is accepted through exact `t = 8.75`
for `2346` seconds (`0.651667` displayed node-hours). Its exact retained
scheduler row is
...
formal inspection SHA-256 is
`24a7013abca332a65196552f0ce1a6031314deb56e8ddbebb130b75be4b887de`;
recorded manifest SHA-256 is
`24f407dcc74011479f783ccb16d08536ce87a35314d1637f93b78135adb27ed8`;
retained independent validation SHA-256 is
`0a271b1f53f3363c2598a76356a7ca3063a38e5d8c0ab63a64e15b4ebcfa2229`;
retained scheduler evidence SHA-256 is
`06b86448d2c4eb8c3765d9c3912ee1cb9d946f0d07ad606dfae6456ab1075f7f`.
It retains one complete eight-rank snapshot group and terminal eight-rank
restart siblings, zero strict LF failure counters, finite synchronized
histories, no restart-marker bypass, terminal `lf_hwproj = 299627446834`, and
sampled-history forcing-work relative residual `1.4737883591844796e-12`.
The accepted prefix totals `74585` seconds (`20.718055555555555` exact and
`20.718057` displayed node-hours). Hardened reconciliation closes with
`25/25/25` ledger rows/manifests/reservations, no active reservation, no
transaction, and `issues = []`.

The retained F-108 generator SHA-256 is
`c488c4d6f51a2d73e00e212f0e485d920876eb2ed79e8e18988788969e344d05`.
Canonical F-108 recost evidence SHA-256 is
...
The normal retained-companion publication leaves canonical-only bytes, a
mode-`0444` forensic copy, empty transaction directories, an empty user queue,
and a free strict lock. With prior selected `2560`, previous observed `2285`,
observed `2346`, and local acceleration `2407`, the reviewed `s25` estimator
selects `2560 <= 2700` seconds. It projects `829.2566672222222` matrix
node-hours with `70.74333277777782` margin. Commit, push, archive, and catalog
the post-F-108 documentation checkpoint, validate the archive ledger, rerun
hardened reconciliation, and repeat the queue/shared-root audit with explicit
acknowledgement of the stale beta-25 record. Require completion of the durable
handoff checklist items 22--24 before preparation. Then prepare only
...
That F-108 authorization is also historical. Authenticated
`R02/s25_rankio_t8p75_t9` job `4758713` is accepted through exact `t = 9`
for `2259` seconds (`0.627500` displayed node-hours). Its exact retained
scheduler row is
...
formal inspection SHA-256 is
`313cb0565ad7d3f7bbe63be63cbd6055de951842cb101aeb523102a37f2b2f48`;
recorded manifest SHA-256 is
`18299de7cb6229291bcc9c40fd6e0d12076efb16ab6a64891e80983368fdd401`;
retained independent validation SHA-256 is
`d2b8d4d044d4569c25a089bd5e3a23839c785e81ca258da091b9d718d63aa5cb`;
retained scheduler evidence SHA-256 is
`6e9239d754639e888949b1f9699be479dd95f586ffd6d5ba8de0ac0a664328da`.
It retains one complete eight-rank snapshot group and terminal eight-rank
restart siblings, zero strict LF failure counters, finite synchronized
histories, no restart-marker bypass, terminal `lf_hwproj = 302201795827`, and
sampled-history forcing-work relative residual `1.83194669112365e-12`.
The accepted prefix totals `76844` seconds (`21.345555555555556` exact and
`21.345557` displayed node-hours). Hardened reconciliation closes with
`26/26/26` ledger rows/manifests/reservations, no active reservation, no
transaction, and `issues = []`.

The retained F-109 generator SHA-256 is
`1a480a6ebf2b1f27ddd453d126fb7e58f1919f4e8825da5eb68eb5cd6007541d`.
Canonical F-109 recost evidence SHA-256 is
...
The normal retained-companion publication leaves canonical-only bytes, a
mode-`0444` forensic copy, empty transaction directories, an empty user queue,
and a free strict lock. With prior selected `2560`, previous observed `2346`,
observed `2259`, and local acceleration `2259`, the reviewed `s26` estimator
selects `2560 <= 2700` seconds. It projects `829.2566672222222` matrix
node-hours with `70.74333277777782` margin. Commit, push, archive, and catalog
the post-F-109 documentation checkpoint, validate the archive ledger, rerun
hardened reconciliation, and repeat the queue/shared-root audit with explicit
acknowledgement of the stale beta-25 record. Require completion of the durable
handoff checklist items 25--27 before preparation. Then prepare only
