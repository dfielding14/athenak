...
communication-heavy work until it completes, and surfaces fatal failures. The scheduler
...
- During `MeshBlockPack::AddPhysics`, every constructed physics module receives the shared
  `tl_map` and registers its tasks, caching returned `TaskID`s so other modules can depend
  on them (`src/mesh/meshblock_pack.cpp:94`, `src/hydro/hydro.hpp:39`).
...
|-----|------------------------|
| `before_timeintegrator` | Operator-split setup to run once per timestep (e.g., turbulence forcing seeds). |
...
- `fail`: an unrecoverable error occurred; callers typically emit diagnostics before
  returning `fail`, and at present the driver aborts via the failing task
  (`src/bvals/bvals_tasks.cpp:160`).
...
forcing) can add work immediately before or after specific steps (`src/hydro/hydro.hpp:39`).
