...
files, archives input and executable checksums, uses the \texttt{batch}
...
The first submitted segment is \texttt{R16/s00\_t0\_t2}, Frontier job
\texttt{4674731}; it is an active calculation, not an accepted result.
...
normalization statement.  The first mapped production segment has been
submitted, but quantitative MKS24 simulation-to-reference comparisons require
completed accepted bundles; none is claimed by this manuscript version.
...
began with the required low-resolution scale-separation case
\texttt{R16/s00\_t0\_t2}; at the time of writing that segment is running and
is not counted as a scientific result.  Thus the present manuscript
establishes the method, target contract, and acceptance procedure, but does
not yet claim reproduction of the MKS24 turbulent conclusions.
...
products only after their corresponding gates pass.
...
the report is updated.  A committed production path and its first submitted
segment now exist, but production/statistical and paper-reference comparison
gates remain open until accepted long-time output bundles are analyzed.
