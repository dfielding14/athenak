...
\texttt{4675322} from \(t=0\) and began executing on 2026-05-25.  A complete
initial eight-rank snapshot/restart set was observed and read through the
rank-aware analyzer; acceptance still requires terminal inspection and
accounting.  The legacy partial is not continued across the changed input
digest.
...
only as clean partial I/O-diagnostic evidence and its rank-local rerun has
been submitted, but quantitative MKS24 simulation-to-reference comparisons
require completed accepted bundles; none is claimed by this manuscript
version.
...
shared-I/O segment is a clean partial diagnostic record, and a fresh
rank-local segment has been submitted from \(t=0\).  Neither is counted as an
accepted comparison result.  Thus the present manuscript establishes the
method, target contract, and acceptance procedure, but does not yet claim
reproduction of the MKS24 turbulent conclusions.
...
the report is updated.  One production segment has been accounted as clean
partial and its rank-local replacement has been submitted, but
production/statistical and paper-reference comparison gates remain open until
accepted long-time output bundles are analyzed.
