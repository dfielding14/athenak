...
authorizes only `R02/s10_rankio_t4p5_t5` next with Slurm walltime `01:40:00`,
...
| Performance suitability | Representative timing/memory/I/O evidence; no uninvestigated prohibitive bottleneck | Yes for production use | Partial 2026-05-31. E03 `g030b` reaches reduced nonlinear `t = 2.0` in `1516` allocated seconds with fourteen MPI-I/O records. E03 `g031` launches the exact `192 x 192 x 384` ranked layout, fits one node, and reaches startup-only `t = 0.01` in `77` seconds while retaining `509685800` terminal snapshot bytes and `1381369688` terminal checkpoint bytes. E03 R02 continuation job `4747146` measures `2.452778` node-hours per simulated time unit across `t = 4.5`--`5.0`; retained corrected recost evidence projects at most `774.423334` matrix node-hours. The clean `4415`-second segment satisfies its reviewed `s10` cap. Independent review renews the same profile only for `R02/s11_rankio_t5_t5p5` from authenticated `s10` terminal siblings with Slurm walltime `01:40:00`, Athena timeout `01:30:00`, and a one-segment `4800`-second ceiling retaining `600` seconds on both timeout margins. It does not ratchet automatically. After `s11`, switch the next proposal to quarter units if elapsed time exceeds `4800` seconds, remaining cap headroom falls below `300` seconds, the trend-based next-half estimate reaches `4800`, or any scientific or reconciliation condition fails. Archive and catalog the current committed controller state, reconcile, prepare only `R02/s11_rankio_t5_t5p5`, then inspect, account, reconcile, and recost. |
...
| Production ready for supported CGL-LF use | Not established | Corrected E03 debug qualification, retained token, token-gated accounting tooling, committed epoch isolation and shared-root controls, immutable corrected build archival, retained focused serial/MPI CPU gates, historical E02 pipeline/cost evidence, accepted fresh E03 R02 jobs `4745922`, `4746154`, `4746182`, `4746356`, `4746435`, `4746663`, `4746773`, `4746953`, `4747015`, `4747087`, and `4747146` through exact `t = 5.0`, and archived F-079--F-084 controller provenance plus hardened clean reconciliation exist | Archive and catalog the current committed controller state, reconcile, prepare only the authenticated R02 continuation from authenticated `s10` terminal siblings through `t = 5.5` under the reviewed non-ratcheting `s11` profile, inspect, account, reconcile, and recost; switch the next proposal to quarter units after `s11` if elapsed time exceeds `4800` seconds, remaining cap headroom falls below `300` seconds, the trend-based next-half estimate reaches `4800`, or any scientific or reconciliation condition fails; then execute representative mapped production and complete production/statistical qualification |
...
`5256fb7fd9b75f175a16c2c16ee8a995b8d6d9281a92b70a979ea62171835bf2`.
Hardened reconciliation closes with `11/11/11` ledger
...
prepare only `R02/s11_rankio_t5_t5p5` from the authenticated `s10` terminal
siblings with Slurm walltime `01:40:00`, Athena timeout `01:30:00`, and its
reviewed one-segment `4800`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s11` and cannot ratchet
automatically. After `s11`, switch the next proposal to quarter units if
elapsed time exceeds `4800` seconds, remaining cap headroom falls below `300`
seconds, the trend-based next-half estimate reaches `4800`, or any scientific
or reconciliation condition fails.
Inspect, account, reconcile, and
recost before any further extension.
...
exact `t = 5.0`;
archive and catalog the current committed controller state, reconcile, then
continue only with `R02/s11_rankio_t5_t5p5` from its authenticated `s10`
restart siblings under the full F-093 profile: Slurm walltime `01:40:00`, Athena
timeout `01:30:00`, and a one-segment `4800`-second threshold retaining `600`
...
`R02/s11_rankio_t5_t5p5` from the authenticated `s10` terminal siblings under
the full F-093 profile: Slurm walltime `01:40:00`, Athena timeout `01:30:00`,
and a one-segment `4800`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s11` and cannot ratchet
automatically. After `s11`, switch the next proposal to quarter units if
elapsed time exceeds `4800` seconds, remaining cap headroom falls below `300`
seconds, the trend-based next-half estimate reaches `4800`, or any scientific
or reconciliation condition fails. Inspect, account, reconcile, and recost
...
2. **Preserve the accepted fresh E03 R02 pilot from `t = 0`.**
   ...
   then prepare only `R02/s11_rankio_t5_t5p5` from the authenticated `s10`
   terminal siblings with Slurm walltime `01:40:00`, Athena timeout `01:30:00`,
   and its reviewed one-segment `4800`-second threshold retaining `600`
   seconds on both timeout margins. The threshold is scoped only to `s11` and
   cannot ratchet automatically. After `s11`, switch the next proposal to
   quarter units if elapsed time exceeds `4800` seconds, remaining cap
   headroom falls below `300` seconds, the trend-based next-half estimate
   reaches `4800`, or any scientific or reconciliation condition fails.
