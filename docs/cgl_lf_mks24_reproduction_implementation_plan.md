...
| Performance suitability | Representative timing/memory/I/O evidence; no uninvestigated prohibitive bottleneck | Yes for production use | Partial 2026-05-31. E03 `g030b` reaches reduced nonlinear `t = 2.0` in `1516` allocated seconds with fourteen MPI-I/O records. E03 `g031` launches the exact `192 x 192 x 384` ranked layout, fits one node, and reaches startup-only `t = 0.01` in `77` seconds while retaining `509685800` terminal snapshot bytes and `1381369688` terminal checkpoint bytes. E03 R02 continuation job `4747087` measures `2.416111` node-hours per simulated time unit across `t = 4.0`--`4.5`; retained corrected recost evidence projects at most `769.290001` matrix node-hours. The clean `4349`-second segment satisfies its reviewed `s09` cap. Independent review renews the same profile only for `R02/s10_rankio_t4p5_t5` with Slurm walltime `01:40:00`, Athena timeout `01:30:00`, and a one-segment `4800`-second ceiling retaining `600` seconds on both timeout margins. It does not ratchet automatically. Archive and catalog the current committed controller state, reconcile, prepare only `R02/s10_rankio_t4p5_t5`, then inspect, account, reconcile, and recost. |
...
| Production ready for supported CGL-LF use | Not established | Corrected E03 debug qualification, retained token, token-gated accounting tooling, committed epoch isolation and shared-root controls, immutable corrected build archival, retained focused serial/MPI CPU gates, historical E02 pipeline/cost evidence, accepted fresh E03 R02 jobs `4745922`, `4746154`, `4746182`, `4746356`, `4746435`, `4746663`, `4746773`, `4746953`, `4747015`, and `4747087` through exact `t = 4.5`, and archived F-079--F-084 controller provenance plus hardened clean reconciliation exist | Archive and catalog the current committed controller state, reconcile, prepare only the authenticated R02 continuation through `t = 5.0` under the reviewed non-ratcheting `s10` profile, inspect, account, reconcile, and recost; then execute representative mapped production and complete production/statistical qualification |
...
`8b10b1786db520740b687d989207f3cda6e0123f9bf2b62e75a5df3849511561`.
Hardened reconciliation closes with `10/10/10` ledger
...
prepare only `R02/s10_rankio_t4p5_t5` with Slurm walltime `01:40:00`, Athena
timeout `01:30:00`, and its reviewed one-segment `4800`-second threshold
retaining `600` seconds on both timeout margins. The threshold is scoped only
to `s10` and cannot ratchet automatically.
...
exact `t = 4.5`;
archive and catalog the current committed controller state, reconcile, then
continue only with `R02/s10_rankio_t4p5_t5` from its authenticated restart
siblings under the full F-092 profile: Slurm walltime `01:40:00`, Athena
...
`R02/s10_rankio_t4p5_t5` from the authenticated `s09` terminal siblings under
the full F-092 profile: Slurm walltime `01:40:00`, Athena timeout `01:30:00`,
and a one-segment `4800`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s10` and cannot ratchet
...
2. **Preserve the accepted fresh E03 R02 pilot from `t = 0`.**
   ...
   then prepare only `R02/s10_rankio_t4p5_t5` with Slurm walltime `01:40:00`,
   ...
   scoped only to `s10` and cannot ratchet automatically. Inspect, account,
   reconcile,
