...
| Validated on Frontier GPU hardware | Supported only for archived pre-replacement reduced qualification cases through F-033 plus replacement-driver startup, rank-local modal restart identity, one-rank/eight-rank decomposition, and reduced passive/random smoke | Immutable historical HIP/MPI builds; G-001 through G-007 strict/restart/decomposition/reduced-paper records; post-F-026 restart jobs `4658072`/`4658163`, GPU AMR jobs `4658191`/`4658283`, managed-policy confirmation job `4659663`, G010b/G011 I/O evidence, G013 corrected nonlinear hard-wall job `4673404`, and replacement-driver `g015`/`g018`/`g019`/`g020`/`g021` | Replacement-driver nonlinear hard-wall, measured standard-layout I/O/runtime, and long-time statistical qualification remain open; G012 remains the retained failure of the superseded finite-rate hard-wall interpretation |
| Production ready for supported CGL-LF use | Not established | Debug policy/accounting tooling, committed epoch isolation and shared-root controls, immutable replacement-driver build archival, reduced `g015` startup, `g018` rank-local modal restart identity, `g019` one-rank/eight-rank decomposition, `g020`/`g021` passive/random smoke, archived reduced GPU qualification including F-033 G013, MPI CPU AMR evidence, historical shared-MPI-I/O measurements, an archived sequential normal-QOS Stage I protocol with accepted pre-replacement `R16` prefix evidence, and RK/AMR-consistent applied hyperbolic pressure traction histories exist | Complete replacement-driver nonlinear hard-wall and standard-layout gates; then run a fresh `R16` pilot, recost, execute representative mapped production runs, and complete production/statistical qualification |
...
2. **Qualify the replacement driver and archive one immutable production
   ...
   passive-Alfvénic and active-random startup coverage. Continue with nonlinear
   hard-wall and standard-layout rank-local I/O gates before production.
