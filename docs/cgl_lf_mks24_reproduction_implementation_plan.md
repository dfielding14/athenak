...
tiled raster and colorbar into excluded donor-pressure-coordinate surface
samples; those samples remain outside quantitative reference ingestion until
the donor-to-AthenaK pressure scaling and two-dimensional PDF Jacobian are
qualified. Figure 3's
...
| Paper observables and analysis | Synthetic analysis tests and archived reduced-run products | Yes for MKS24 claims | Partial 2026-05-25; reduced histories, restartable RK-integrated applied forcing work/global active residual, RKL2-applied heat-flux contractions with fine-side AMR ownership, AMR-corrected RK-applied total/anisotropic CGL pressure-work ledgers, operator-face cap counters, windowed snapshot PDF/spectral/transfer/alignment/local-strain products, Figure 2(a)-coordinate joint pressure-density PDFs, sampled-surface comparison support, and excluded color-calibrated donor-coordinate raster samples, Figure 3 compressive-flow spectra, Figure 4(b) normalized-density spectra, Figure 6(a) thermal/magnetic-pressure spectra, MKS24-normalized transfer and alignment-peak curve comparisons, opt-in local-field eddy-anisotropy structure functions, deduplicated threshold-volume history curves, manifest-qualified proxies and cadence-limited estimates, generic figures, checksum/uncertainty-qualified reference-data comparison plumbing, Figure 2(b) histories, Figure 4(a) normalized-density PDFs, Figure 5(b) normalized eddy scales, Figure 7 lower-panel transfer ratios, Figure 8 selected-shell alignment PDFs, Figure 9, Figure 11 lower-panel, and Figure 12 alignment curves, and Figure 13(b),(d) dimensionless curves are implemented; guarded Figure 3 case definitions are present under `paper-compressive`; Figure 2(a) donor-pressure conversion/Jacobian, dimensional/unmatched-panel conversion, standard-run comparison, and remaining panel references remain open |
...
| MKS24 reproduction complete | Not established | Guarded standard/limiter/heat-flux/compressive/scale-separation input matrices, paper analysis products including Figure 2(a)-coordinate joint PDFs with sampled-surface comparison support and excluded decoded donor-coordinate samples, Figure 3/4(b)/6(a) spectral fields, checksum/uncertainty-qualified reference-data comparison interface, populated Figure 2(b) histories, Figure 4(a) normalized-density PDFs, Figure 5(b) normalized eddy-scale curves, dimensionless Figure 7 transfer ratios, Figure 8 selected-shell alignment PDFs, Figure 9, Figure 11 lower-panel, and Figure 12 alignment curves, and Figure 13(b),(d) curves exist | Qualified Figure 2(a) donor-pressure conversion/Jacobian, paper-to-AthenaK transforms for dimensional or unmatched spectral panels, other remaining panel reference data, long-time statistical calibration, authorized standard/limiter/heat-flux/compressive/scale-separation runs, quantitative panel comparisons, and archived scientific interpretation remain required |
...
| Figure 2(a) excluded raw-surface extractor | `scripts/digitize_cgl_lf_mks24_fig2a.py` |
...
| --- | --- | --- | --- |
| Figure 2: pressure-density distributions and unstable-volume history | Active and passive, Alfvenic/random, beta 10 and 100 | PDFs of `delta p_parallel`, `delta p_perp`, density; volume fraction beyond thresholds versus time | Reduced threshold-volume, steady-window PDFs, panel-(a)-coordinate joint pressure-density products, sampled-surface comparison support, and excluded color-calibrated donor-coordinate raster samples are implemented; all eight panel-(b) histories are vector-extracted with checksummed provenance; panel (a) still needs a qualified donor-pressure conversion/Jacobian before quantitative comparison |
...
into excluded donor-coordinate samples, while its donor-pressure conversion
and PDF Jacobian remain open. Figure 13(b)
...
Under F-051, `scripts/digitize_cgl_lf_mks24_fig2a.py` decodes Figure 2(a)'s
labeled tiled raster into donor-pressure-coordinate samples with absolute
density uncertainty `0.5`, but writes `excluded_surfaces.json` rather than a
comparison manifest because its pressure conversion/Jacobian remains open.
...
and under F-051 its labeled raster/colorbar is decoded into explicitly
excluded donor-pressure-coordinate samples. It is not comparison data until
the donor-to-AthenaK pressure scale and the corresponding two-dimensional PDF
Jacobian are qualified. Under F-048, Figures 3, 4(b), and 6(a)
...
1. **Complete reference coverage and long-time calibration.**
   ...
   color-calibrated raster into excluded donor-coordinate samples, but
   comparison still requires a qualified donor-pressure conversion and PDF
   Jacobian. F-048
