...
authorizes only `R02/s13_rankio_t5p75_t6` on one node from the authenticated
...
| Performance suitability | Representative timing/memory/I/O evidence; no uninvestigated prohibitive bottleneck | Yes for production use | Partial 2026-05-31. E03 `g030b` reaches reduced nonlinear `t = 2.0` in `1516` allocated seconds with fourteen MPI-I/O records. E03 `g031` launches the exact `192 x 192 x 384` ranked layout, fits one node, and reaches startup-only `t = 0.01` in `77` seconds while retaining `509685800` terminal snapshot bytes and `1381369688` terminal checkpoint bytes. E03 R02 quarter-unit continuation job `4747500` measures `2.640000` node-hours per simulated time unit across `t = 5.5`--`5.75`; retained corrected recost evidence projects at most `814.945556` matrix node-hours. The clean `2376`-second segment leaves `324` seconds below its reviewed `s12` threshold, and the conservative `s13` estimator selects `2468 <= 2700` seconds. Independent review authorizes only `R02/s13_rankio_t5p75_t6` on one node from authenticated `s12` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a one-segment `2700`-second ceiling retaining `600` seconds on both timeout margins. It does not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Archive and catalog the current committed controller state, reconcile, prepare only `R02/s13_rankio_t5p75_t6` on one node from authenticated `s12` terminal siblings under the full post-s12 profile, then inspect, account, reconcile, and recost. |
...
prepare only `R02/s13_rankio_t5p75_t6` on one node from the authenticated `s12`
...
authorizes only `R02/s13_rankio_t5p75_t6`. This is a reviewed one-segment
...
exact `t = 5.75`;
archive and catalog the current committed controller state, reconcile, then
continue only with `R02/s13_rankio_t5p75_t6` on one node from its authenticated `s12`
restart siblings under the full F-095 profile: Slurm walltime `01:05:00`, Athena
...
`R02/s13_rankio_t5p75_t6` on one node from the authenticated `s12` terminal siblings under
the full F-095 profile: Slurm walltime `01:05:00`, Athena timeout `00:55:00`,
...
2. **Preserve the accepted fresh E03 R02 pilot from `t = 0`.**
   ...
   Archive and catalog the current committed controller state, reconcile,
   then prepare only `R02/s13_rankio_t5p75_t6` on one node from the authenticated `s12`
   terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`,
