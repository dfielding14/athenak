...
boundary. Accepted corrected-E03 `R02/s21_rankio_t7p75_t8` job `4758233`
reached exact `t = 8.0` in `2369` seconds. Formal and independently
regenerated inspection pass with `18` retained products, zero strict LF
failure counters, no restart-marker bypass, terminal
`lf_hwproj = 286108988748`, and forcing-work residual
`2.8542854449297643e-12`. Hardened reconciliation closes at `22/22/22`, with
...
F-105 is the latest hardened normal observed publication under the retained
...
`3cbf9c53d8fea066a67f334fe6393da7c8b31d3429d0c4582047b8174e6e7e7f`.
It authorizes only `R02/s22_rankio_t8_t8p25` on one node with Slurm
`01:05:00`, Athena `00:55:00`, and the reviewed one-segment `2700`-second
threshold. Before preparing `s22`, commit, push, archive, and catalog the
...
hardened reconciliation, `22/22/22`, no active reservation, no transaction,
...
| Standard MKS24 results | Required cases, durations, manifests, figure comparisons | Yes for reproduction claim | Only the admissible fresh E03 R02 prefix has run through exact `t = 8.0`; no corrected-epoch case is complete and long-time mapped results remain not run. The frozen sixteen-case mapped Stage I manifest remains the target matrix. Preserve E02 jobs `4743735` through `4744158` only as pipeline and cost evidence. |
| Operational workflow | Frontier scripts, budget ledger, failure recovery, storage plan | Yes for Frontier use | Corrected E03 entry gate passed. The debug-helper ledger records `1.847784` node-hours after `g031`; corrected E03 qualification accounts for `0.485834`. E02 records `15.628610` historical pipeline/cost node-hours and remains fail closed. Token SHA-256 `e3fec9f35da42121b902f41ef752021f375aab38bc34c5ff75ae8780bfbca635` is retained. Fresh E03 R02 jobs through `4758233` were formally inspected and recorded `accepted` through exact `t = 8.0` for `18.780279` displayed cumulative node-hours. F-102 hardening is committed at `f675bd677cb582a46bbda8b5f55c335e6f259dd3`; hardened reconciliation passed with `22/22/22` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. F-103 through F-105 are normal observed publications under the retained lifecycle companion. |
...
| Performance suitability | Representative timing/memory/I/O evidence; no uninvestigated prohibitive bottleneck | Yes for production use | Partial 2026-06-03. E03 `g030b` reaches reduced nonlinear `t = 2.0` in `1516` allocated seconds with fourteen MPI-I/O records. E03 `g031` launches the exact `192 x 192 x 384` ranked layout, fits one node, and reaches startup-only `t = 0.01` in `77` seconds while retaining `509685800` terminal snapshot bytes and `1381369688` terminal checkpoint bytes. Accepted E03 R02 quarter-unit continuation job `4758233` measures `0.658056` displayed node-hours across `t = 7.75`--`8.0`; canonical F-105 recost evidence projects `829.2566672222222` matrix node-hours with `70.74333277777782` margin. The clean `2369`-second segment leaves `331` seconds below its reviewed `s21` threshold, and the conservative `s22` estimator selects `2560 <= 2700` seconds. After the post-F-105 documentation and archive gate and completion of the durable handoff checklist items 12--14, prepare only `R02/s22_rankio_t8_t8p25` on one node from authenticated `s21` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. It does not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. |
...
| Production ready for supported CGL-LF use | Not established | Corrected E03 debug qualification, retained token, token-gated accounting tooling, committed epoch isolation and shared-root controls, immutable corrected build archival, retained focused serial/MPI CPU gates, historical E02 pipeline/cost evidence, accepted fresh E03 R02 jobs through `4758233` and exact `t = 8.0`, archived F-079--F-105 controller provenance, retained lifecycle-companion publications, and hardened clean reconciliation exist | Commit, push, archive, and catalog the post-F-105 documentation checkpoint, validate the archive ledger, and require completion of the durable handoff checklist items 12--14 before preparing only the authenticated R02 continuation from authenticated `s21` terminal siblings through `t = 8.25` under the reviewed non-ratcheting `s22` quarter profile; any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation; then execute representative mapped production and complete production/statistical qualification |
...
authorize preparation after accepted job `4758233`; the documentation
...
`242b4ae79b215690d2b552aba9e583cf209d70c822f8dd76da4e7af1b7ba6a5b`.
Hardened reconciliation closes with `22/22/22` ledger
rows/manifests/reservations, no active reservation, no transaction, and
`issues = []`. Commit, push, archive, and catalog the post-F-105 documentation
...
beta-25 record. Require completion of the durable handoff checklist items 12--14
before preparation. Then prepare only `R02/s22_rankio_t8_t8p25` on one node from
authenticated `s21` terminal siblings with Slurm walltime `01:05:00`, Athena
timeout `00:55:00`, and its reviewed one-segment `2700`-second threshold
retaining `600` seconds on both timeout margins. The threshold is scoped only
to `s22` and cannot ratchet automatically. Any scientific, provenance,
...
exact `t = 8.0`;
commit, push, archive, and catalog the post-F-105 documentation checkpoint,
...
record, and require completion of the durable handoff checklist items 12--14
before continuing only with `R02/s22_rankio_t8_t8p25` on one node from its
authenticated `s21` restart siblings under the full F-105 profile: Slurm
walltime `01:05:00`, Athena timeout `00:55:00`, and a one-segment
`2700`-second threshold retaining `600` seconds on both timeout margins. The
threshold is scoped only to `s22` and cannot ratchet automatically. Any
scientific, provenance, scheduler, storage, budget, or reconciliation failure
blocks successor preparation. Inspect, account, reconcile, and recost before
any further extension.
...
queue/shared-root audit with explicit acknowledgement of the stale beta-25
record, and require completion of the durable handoff checklist items 12--14
before preparing only `R02/s22_rankio_t8_t8p25` on one node from the
authenticated `s21` terminal siblings under the full F-105 profile: Slurm
walltime `01:05:00`, Athena timeout `00:55:00`, and a one-segment
`2700`-second threshold retaining `600` seconds on both timeout margins. The
threshold is scoped only to `s22` and cannot ratchet automatically. Any
scientific, provenance, scheduler, storage, budget, or reconciliation failure
...
2. **Preserve the accepted fresh E03 R02 pilot from `t = 0`.**
   ...
   only to `s22` and cannot ratchet automatically.
   Any scientific, provenance, scheduler, storage, budget, or reconciliation
