...
product now populate both Figure 12 panels for future comparisons. The
directly comparable Figure 13(a) kinetic spectra and Figure 13(b)
`beta Delta` PDFs are now vector-extracted with multi-panel checksum
provenance; Figure 13(c)-(d) remain excluded pending analyzer products for
their plotted normalizations. The
...
| Paper observables and analysis | Synthetic analysis tests and archived reduced-run products | Yes for MKS24 claims | Partial 2026-05-25; reduced histories, restartable RK-integrated applied forcing work/global active residual, RKL2-applied heat-flux contractions with fine-side AMR ownership, AMR-corrected RK-applied total/anisotropic CGL pressure-work ledgers, operator-face cap counters, windowed snapshot PDF/spectral/transfer/alignment/local-strain products, alignment-peak curve comparison, deduplicated threshold-volume history curves, manifest-qualified proxies and cadence-limited estimates, generic figures, checksum/uncertainty-qualified reference-curve comparison plumbing, vector-extracted Figure 2(b)/12 curves, and Figure 13(a)-(b) curves are implemented; standard-run comparison, Figure 13(c)-(d) normalized products, and remaining panel references remain open |
...
| MKS24 reproduction complete | Not established | Guarded standard/limiter/heat-flux input matrices, paper analysis products, checksum/uncertainty-qualified reference-curve comparison interface, populated Figure 2(b)/12 digitized curves, and directly comparable Figure 13(a)-(b) digitized curves exist | Figure 13(c)-(d) normalization products and other remaining panel reference data, long-time statistical calibration, authorized standard/limiter/heat-flux runs, quantitative panel comparisons, and archived scientific interpretation remain required |
...
| Figure 13(a)-(b) reference extractor | `scripts/digitize_cgl_lf_mks24_fig13.py` |
...
paper-standard/limiter-scan/heat-flux inputs explicitly select `parallel`.
Standard paper execution and analysis remain open.
...
| Figure 12: heat-flux sensitivity | Heat-flux parameter variants | Spectra/transfer/diagnostic products under heat-flux changes | Two nonnominal active beta-10 guarded definitions exist and nominal active/passive cases reuse standard definitions; both plotted panels are vector-extracted through `alignment_peak.cos_theta` and `spectra.grad_parallel_delta_p`; execution, steady nonlinear comparison, and exact interval/production budget remain missing |
| Figure 13: limiter-frequency sensitivity | Beta 100, Alfvenic, several `nu_lim` | Energy spectra, `beta Delta` PDFs, strain PDFs, pressure-stress transfer | Inputs and foundations exist; panel (a) velocity spectra and panel (b) `beta Delta` PDFs are vector-extracted for comparison, while panels (c)-(d) require normalized analysis products; case runs/comparisons are missing |
...
baseline, Figure 12 heat-flux, and beta-100 limiter-frequency input
definitions are present. The
...
standard cases and adds the two active nonnominal heat-flux definitions. The
standard, heat-flux, and limiter
...
and generic plots are implemented. Figure 13(b) `beta Delta` reference curves
are populated; the normalized Figure 13(c) strain product remains open.
...
archives reduced model/forcing choices. `paper-standard`, `paper-nulim`, and
`paper-heat-flux` now enumerate the defined production inputs, retain
...
and paper-standard/limiter/heat-flux-production decks under `debug`, emits an
...
- G011 standard-layout sizing job `4661434` uses an archived startup-only
  ...
  the expanded thirteen-unique-run proposal, yielding `403.162` GB
  sequential storage with 20 percent margin. G011 itself explicitly withholds
...
- F-036 Figure 12 reference extraction is retained outside git under
  ...
  The manifest validates through the reference-manifest reader. These
  likewise are reference data, not simulated comparison outcomes.
- F-037 Figure 13 partial reference extraction is retained outside git under
  ...
  Panels (c)-(d) are intentionally excluded pending analyzer products for
  their plotted strain and transfer normalizations.
...
`192 x 192 x 384` mesh and `32 x 32 x 64` MeshBlock layout used by all
...
heat-flux cases. The standard active Alfvenic beta-100 hard-wall definition
...
analysis roles and executes thirteen unique simulations.
...
| Raw generated output for thirteen unique cases | `13 x one case` | `534486712201` | `534.487` |
| Retained output after acceptance | all snapshots plus final two restarts per case | `319425293122` | `319.425` |
| Sequential peak before pruning accepted checkpoints | twelve retained cases plus one raw current case | `335968479205` | `335.968` |
| Sequential retained-policy allocation with 20 percent margin | `1.2 x sequential peak` | `403162175046` | `403.162` |
| No-pruning allocation with 20 percent margin | `1.2 x all raw output` | `641384054641` | `641.384` |
...
run directory; the thirteen-case arithmetic is the F-035 proposal update.
Executing all fourteen definitions independently would duplicate the equivalent
...
| Proposed runtime reservation with 2x contingency | `2 x 300.444444` | `600.888889` node-hours |
| Existing recorded debug qualification plus proposed ceiling | `0.851670 + 600.888889` | `601.740559` node-hours |
...
requires explicit user approval of the thirteen-run scope or a revised scope,
the `403.162` GB sequential-retention allocation (or the `641.384` GB
...
The defined Figure 12 heat-flux variants are included; any remaining
panel-reference-driven additions require a revised cost before execution.
...
For the fourteen guarded definitions and thirteen proposed unique executions,
the G011-measured file sizes define the candidate sequential retention plan
and a `403.162` GB allocation including margin. The candidate cost proposal
...
- `paper-smoke`, `paper-standard`, `paper-nulim`, `paper-heat-flux`, and
  analysis modes;
- runbook;
...
Figures 2(b), 12, and the directly comparable Figure 13(a)-(b) data are now
...
alignment shells spanning the reference coordinates. Figure 13(c)-(d)
remain excluded because their plotted normalized quantities do not yet map
to analyzer products. Data still required:
...
1. **Complete reference coverage and long-time calibration.**
   Figure 2(b)'s eight unstable-volume histories, both Figure 12 panels, and
   Figure 13(a)-(b) are populated through pinned vector-PDF extractors.
   Implement the normalized Figure 13(c)-(d) products and obtain or digitize
   remaining targeted panels with recorded uncertainty, define any resulting
   missing cases, revise the proposal when scope changes, and calibrate
   long-time injection/spectral behavior against the documented setup.
...
3. **Review and explicitly authorize or revise the costed run proposal.**
   ...
   counters and active `lf_hwproj`. The documented candidate executes thirteen
   unique defined cases sequentially, projects `300.444444` node-hours with
   a `600.888889` node-hour planning ceiling, and allocates `403.162` GB with
