...
| Frontier GPU numerical equivalence | CPU/GPU comparison suite, MPI/GPU restart checks, strict diagnostics | Yes for Frontier use | Partial 2026-05-30; archived pre-replacement HIP/MPI builds and G-001 through G013 evidence remain useful historical qualification. Reduced replacement-driver startup `g015`, natural-cycle eight-rank modal restart comparison `g018`, and one-rank versus eight-rank decomposition comparison `g019` pass with zero strict safety counters. `g019` evidence JSON SHA-256 is `509ec34d4f1f27dfdbb9b507d2ca663d7233d168a0b83775b0ab532dc283c752`. Complete reduced passive/random paper smoke, corrected hard-wall nonlinear, and standard-layout rank-local I/O gates before `E02` production. |
| Paper pgen and forcing fidelity | Input/pgen review, forcing metadata, restartable reduced smoke | Yes for MKS24 claims | Partial 2026-05-30; pre-replacement active/passive/random reduced smoke, forcing restart, OU cadence, multi-cycle RK-companion source-work checks, passive flow-decoupling checks, and Frontier G-007 reduced-paper matrix passed. The merged driver now stores authoritative modal OU coefficients with `TurbulenceRestartMetadata`, and `g018` closes reduced rank-local modal restart identity. Rerun MKS24 forcing orientation/cadence/work and reduced active/passive/random paper smoke before production. |
...
| Operational workflow | Frontier scripts, budget ledger, failure recovery, storage plan | Yes for Frontier use | Partial 2026-05-30; the debug ledger records `0.902227` node-hours after retained `g014` through `g019`, and the old Stage I ledger records `9.962778` historical node-hours through rejected job `4686032`. Commit `462b9dbd` makes `scripts/frontier/cgl_lf_stage_i.py` write new work beneath `E02-modal-driver`, use separate accounting files, reject cross-epoch ancestry, check all queued user jobs and active top-level shared-root campaign records, and permit only an initial `4.0` node-hour `R16` pilot. Commits `d210cdd5`, `eab6e12b`, and `7fae0bcf` retain bundle-backed source provenance, complete rank-local debug restart archival, matching debug shared-root checks, and absent-override-key rejection before reservation. |
...
| Validated on Frontier GPU hardware | Supported only for archived pre-replacement reduced qualification cases through F-033 plus replacement-driver startup, rank-local modal restart identity, and one-rank/eight-rank decomposition | Immutable historical HIP/MPI builds; G-001 through G-007 strict/restart/decomposition/reduced-paper records; post-F-026 restart jobs `4658072`/`4658163`, GPU AMR jobs `4658191`/`4658283`, managed-policy confirmation job `4659663`, G010b/G011 I/O evidence, G013 corrected nonlinear hard-wall job `4673404`, and replacement-driver `g015`/`g018`/`g019` | Replacement-driver reduced passive/random smoke, nonlinear hard-wall, measured standard-layout I/O/runtime, and long-time statistical qualification remain open; G012 remains the retained failure of the superseded finite-rate hard-wall interpretation |
| Production ready for supported CGL-LF use | Not established | Debug policy/accounting tooling, committed epoch isolation and shared-root controls, immutable replacement-driver build archival, reduced `g015` startup, `g018` rank-local modal restart identity, `g019` one-rank/eight-rank decomposition, archived reduced GPU qualification including F-033 G013, MPI CPU AMR evidence, historical shared-MPI-I/O measurements, an archived sequential normal-QOS Stage I protocol with accepted pre-replacement `R16` prefix evidence, and RK/AMR-consistent applied hyperbolic pressure traction histories exist | Complete replacement-driver reduced passive/random smoke, nonlinear hard-wall, and standard-layout gates; then run a fresh `R16` pilot, recost, execute representative mapped production runs, and complete production/statistical qualification |
...
and F-061 controls through `7fae0bcf` before broader qualification. Expand the
matrix reservation
...
1. **Isolate the fresh execution epoch and harden shared-root submission
   ...
   `4.0` node-hour `R16` pilot. Commits `462b9dbd`, `d210cdd5`, `eab6e12b`,
   and `7fae0bcf` retain those controls, including the F-061 prepare-time
   absent-override-key guard exposed by `g014`. The
...
2. **Qualify the replacement driver and archive one immutable production
   ...
   `g017`. Continue with reduced passive/random smoke, nonlinear hard-wall,
   and standard-layout rank-local I/O gates before production.
