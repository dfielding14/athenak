...
| Frontier GPU numerical equivalence | CPU/GPU comparison suite, MPI/GPU restart checks, strict diagnostics | Yes for Frontier use | Partial 2026-05-25; immutable HIP/MPI builds archived, G-001/G-002 strict eight-GPU active smoke and G-003 decomposition comparison passed, and G-004 analytic finite-collision GPU regression passed; initial G-005 preserved fields/work but exposed nonpersistent cumulative LF face/stage diagnostics, now under correction before rerun |
...
| Operational workflow | Frontier scripts, budget ledger, failure recovery, storage plan | Yes for Frontier use | Partial 2026-05-25; clean revisions `2a1b1fef` and `827394fb` were built on Frontier and sequential debug jobs through the rejected initial G-005 continuation are retained and recorded (`0.014168` node-hours total); corrected environment capture was verified live and diagnostic-restart remediation is in progress |
...
- G-005 reference job `4653460` passed on the same executable. Continuation
  ...
  pre-checkpoint cumulative baselines. F-022 requires a corrected rerun.
- The retained ledger therefore records `0.014168` node-hours used. Corrected
  G-005, multinode, reduced-science, and sizing evidence remain open.
