...
authorizes only `R02/s09_rankio_t4_t4p5` next with Slurm walltime `01:40:00`,
...
to `s09` and does not ratchet automatically.
...
| Performance suitability | Representative timing/memory/I/O evidence; no uninvestigated prohibitive bottleneck | Yes for production use | Partial 2026-05-31. E03 `g030b` reaches reduced nonlinear `t = 2.0` in `1516` allocated seconds with fourteen MPI-I/O records. E03 `g031` launches the exact `192 x 192 x 384` ranked layout, fits one node, and reaches startup-only `t = 0.01` in `77` seconds while retaining `509685800` terminal snapshot bytes and `1381369688` terminal checkpoint bytes. E03 R02 continuation job `4747015` measures `2.400000` node-hours per simulated time unit across `t = 3.5`--`4.0`; retained corrected recost evidence projects at most `767.034445` matrix node-hours. The clean `4320`-second segment satisfies its reviewed `s08` cap. A stricter independently reviewed next profile authorizes only `R02/s09_rankio_t4_t4p5` with Slurm walltime `01:40:00`, Athena timeout `01:30:00`, and a one-segment `4800`-second ceiling that retains `600` seconds on both timeout margins and does not ratchet automatically. Archive and catalog the current committed controller state, reconcile, prepare only `R02/s09_rankio_t4_t4p5`, then inspect, account, reconcile, and recost. |
...
| Production ready for supported CGL-LF use | Not established | Corrected E03 debug qualification, retained token, token-gated accounting tooling, committed epoch isolation and shared-root controls, immutable corrected build archival, retained focused serial/MPI CPU gates, historical E02 pipeline/cost evidence, accepted fresh E03 R02 jobs `4745922`, `4746154`, `4746182`, `4746356`, `4746435`, `4746663`, `4746773`, `4746953`, and `4747015` through exact `t = 4.0`, and archived F-079--F-084 controller provenance plus hardened clean reconciliation exist | Archive and catalog the current committed controller state, reconcile, prepare only the authenticated R02 continuation through `t = 4.5` under the reviewed non-ratcheting `s09` profile, inspect, account, reconcile, and recost; then execute representative mapped production and complete production/statistical qualification |
...
`dd5b0f5b82cf81005c1a481ee83d1127aa43861dcdd8c22765170e28b0ad6c99`.
Hardened reconciliation closes with `9/9/9` ledger
...
prepare only `R02/s09_rankio_t4_t4p5` with Slurm walltime `01:40:00`, Athena
timeout `01:30:00`, and its reviewed one-segment `4800`-second threshold
retaining `600` seconds on both timeout margins. The threshold is scoped only
to `s09` and cannot ratchet automatically.
...
exact `t = 4.0`;
archive and catalog the current committed controller state, reconcile, then
continue only with `R02/s09_rankio_t4_t4p5` from its authenticated restart
siblings under the full F-091 profile: Slurm walltime `01:40:00`, Athena
timeout `01:30:00`, and a one-segment `4800`-second threshold retaining `600`
seconds on both timeout margins. The threshold is scoped only to `s09` and
cannot ratchet automatically. Inspect, account, reconcile, and recost before
...
`R02/s09_rankio_t4_t4p5` from the authenticated `s08` terminal siblings under
the full F-091 profile: Slurm walltime `01:40:00`, Athena timeout `01:30:00`,
and a one-segment `4800`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s09` and cannot ratchet
automatically. Inspect, account,
reconcile, and recost before any further extension. Complete R02, then
...
2. **Preserve the accepted fresh E03 R02 pilot from `t = 0`.**
   ...
   then prepare only `R02/s09_rankio_t4_t4p5` with Slurm walltime `01:40:00`,
   Athena timeout `01:30:00`, and its reviewed one-segment `4800`-second
   threshold retaining `600` seconds on both timeout margins. The threshold is
   scoped only to `s09` and cannot ratchet automatically. Inspect, account,
   reconcile,
