...
joint PDFs. Its pressure-scaled raster remains excluded from quantitative
reference ingestion until the donor pressure/color-density transform is
...
| --- | --- | --- | --- |
| Figure 2: pressure-density distributions and unstable-volume history | Active and passive, Alfvenic/random, beta 10 and 100 | PDFs of `delta p_parallel`, `delta p_perp`, density; volume fraction beyond thresholds versus time | Reduced threshold-volume, steady-window PDFs, panel-(a)-coordinate joint pressure-density products, and sampled-surface comparison support are implemented; all eight panel-(b) histories are vector-extracted with checksummed provenance; panel (a)'s raster still needs a qualified pressure/color-density transform before quantitative comparison |
...
reference comparisons are now implemented, while its raster reference
conversion remains open under F-047/F-050. Figure 13(b)
...
- F-047 adds the missing Figure 2(a)-coordinate product in tracked analysis:
  ...
  pressure/color-density transform exists.
- F-050 extends the tracked reference-data interface to optional sampled
  ...
  route; it does not convert or admit the Figure 2(a) raster.
...
but its raster is not comparison data until its pressure/color-density
transform is qualified. Under F-048, Figures 3, 4(b), and 6(a)
...
1. **Complete reference coverage and long-time calibration.**
   ...
   comparison interface in AthenaK coordinates, but its raster comparison
   still requires a qualified pressure/color-density transform. F-048
