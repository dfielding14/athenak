...
| Frontier GPU numerical equivalence | CPU/GPU comparison suite, MPI/GPU restart checks, strict diagnostics | Yes for Frontier use | Partial 2026-05-30; archived pre-replacement HIP/MPI builds and G-001 through G013 evidence remain useful historical qualification. Reduced replacement-driver startup `g015` and natural-cycle eight-rank modal restart comparison `g018` pass with zero strict safety counters; `g018` evidence JSON SHA-256 is `4d898cf4aa4427b81164e4eb688f011adbbb277874b1ca7453a7a2c1e60b580f`. Complete GPU/MPI decomposition, reduced passive/random paper smoke, corrected hard-wall nonlinear, and standard-layout rank-local I/O gates before `E02` production. |
...
| Validated on Frontier GPU hardware | Supported only for archived pre-replacement reduced qualification cases through F-033 plus replacement-driver startup and rank-local modal restart identity | Immutable historical HIP/MPI builds; G-001 through G-007 strict/restart/decomposition/reduced-paper records; post-F-026 restart jobs `4658072`/`4658163`, GPU AMR jobs `4658191`/`4658283`, managed-policy confirmation job `4659663`, G010b/G011 I/O evidence, G013 corrected nonlinear hard-wall job `4673404`, and replacement-driver `g015`/`g018` | Replacement-driver GPU/MPI decomposition, reduced passive/random smoke, nonlinear hard-wall, measured standard-layout I/O/runtime, and long-time statistical qualification remain open; G012 remains the retained failure of the superseded finite-rate hard-wall interpretation |
| Production ready for supported CGL-LF use | Not established | Debug policy/accounting tooling, committed epoch isolation and shared-root controls, immutable replacement-driver build archival, reduced `g015` startup and `g018` rank-local modal restart identity, archived reduced GPU qualification including F-033 G013, MPI CPU AMR evidence, historical shared-MPI-I/O measurements, an archived sequential normal-QOS Stage I protocol with accepted pre-replacement `R16` prefix evidence, and RK/AMR-consistent applied hyperbolic pressure traction histories exist | Complete replacement-driver decomposition, reduced passive/random smoke, nonlinear hard-wall, and standard-layout gates; then run a fresh `R16` pilot, recost, execute representative mapped production runs, and complete production/statistical qualification |
...
2. **Qualify the replacement driver and archive one immutable production
   ...
   reduced rank-local modal restart identity. Continue with decomposition,
