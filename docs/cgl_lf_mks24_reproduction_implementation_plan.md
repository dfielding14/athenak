...
| Paper observables and analysis | Synthetic analysis tests and archived reduced-run products | Yes for MKS24 claims | Partial 2026-05-25; reduced histories, restartable RK-integrated applied forcing work/global active residual, RKL2-applied heat-flux contractions with fine-side AMR ownership, AMR-corrected RK-applied total/anisotropic CGL pressure-work ledgers, operator-face cap counters, windowed snapshot PDF/spectral/transfer/alignment/local-strain products, alignment-peak curve comparison, deduplicated threshold-volume history curves, manifest-qualified proxies and cadence-limited estimates, generic figures, checksum/uncertainty-qualified reference-curve comparison plumbing, and vector-extracted Figure 2(b)/12 reference curves are implemented; standard-run comparison and remaining panel references remain open |
...
| MKS24 reproduction complete | Not established | Guarded standard/limiter/heat-flux input matrices, paper analysis products, checksum/uncertainty-qualified reference-curve comparison interface, and populated Figure 2(b)/12 digitized curves exist | Remaining panel reference data, long-time statistical calibration, authorized standard/limiter/heat-flux runs, quantitative panel comparisons, and archived scientific interpretation remain required |
...
| 2026-05-24 | Stage official arXiv source version `2405.02418v2` into ignored result storage with SHA-256 provenance rather than vendoring paper contents. | A pinned source archive supports exact reference interpretation while leaving third-party redistribution outside the code branch. | Analysis bundles retain pinned checksum provenance; F-034/F-036 populate Figures 2(b)/12, while remaining panel curves/comparisons remain open |
...
Figures 2(b) and 12 data are now extracted from pinned vector PDFs by
`scripts/digitize_cgl_lf_mks24_fig2b.py` and
`scripts/digitize_cgl_lf_mks24_fig12.py`, with checksum-qualified external
...
alignment shells spanning the reference coordinates. Data still required:
...
1. **Complete reference coverage and long-time calibration.**
   Figure 2(b)'s eight unstable-volume histories and both Figure 12 panels
   are populated through pinned vector-PDF extractors. Obtain or digitize
   remaining targeted panels with recorded uncertainty, define any resulting
   missing cases, revise the proposal when scope changes, and calibrate long-time
