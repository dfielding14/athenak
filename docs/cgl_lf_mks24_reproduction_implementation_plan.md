...
until the donor pressure/color-density transform is qualified. The
...
| Paper observables and analysis | Synthetic analysis tests and archived reduced-run products | Yes for MKS24 claims | Partial 2026-05-25; reduced histories, restartable RK-integrated applied forcing work/global active residual, RKL2-applied heat-flux contractions with fine-side AMR ownership, AMR-corrected RK-applied total/anisotropic CGL pressure-work ledgers, operator-face cap counters, windowed snapshot PDF/spectral/transfer/alignment/local-strain products, Figure 2(a)-coordinate joint pressure-density PDFs, MKS24-normalized transfer and alignment-peak curve comparisons, opt-in local-field eddy-anisotropy structure functions, deduplicated threshold-volume history curves, manifest-qualified proxies and cadence-limited estimates, generic figures, checksum/uncertainty-qualified reference-curve comparison plumbing, Figure 2(b) histories, Figure 4(a) normalized-density PDFs, Figure 5(b) normalized eddy scales, Figure 7 lower-panel transfer ratios, Figure 8 selected-shell alignment PDFs, Figure 9, Figure 11 lower-panel, and Figure 12 alignment curves, and Figure 13(b),(d) dimensionless curves are implemented; the Figure 2(a) raster transform, dimensional/unmatched-panel conversion, standard-run comparison, and remaining panel references remain open |
...
| MKS24 reproduction complete | Not established | Guarded standard/limiter/heat-flux/scale-separation input matrices, paper analysis products including Figure 2(a)-coordinate joint PDFs, checksum/uncertainty-qualified reference-curve comparison interface, populated Figure 2(b) histories, Figure 4(a) normalized-density PDFs, Figure 5(b) normalized eddy-scale curves, dimensionless Figure 7 transfer ratios, Figure 8 selected-shell alignment PDFs, Figure 9, Figure 11 lower-panel, and Figure 12 alignment curves, and Figure 13(b),(d) curves exist | Qualified Figure 2(a) raster conversion, paper-to-AthenaK transforms for dimensional or unmatched spectral panels, other remaining panel reference data, long-time statistical calibration, authorized standard/limiter/heat-flux/scale-separation runs, quantitative panel comparisons, and archived scientific interpretation remain required |
...
| Figures 5-6: turbulent spectra and rate-of-strain suppression | Active/passive, Alfvenic/random | Velocity/magnetic/pressure spectra; parallel/perpendicular velocity-gradient spectra; local-field eddy scales | Local-field velocity-gradient/strain products plus opt-in conditioned three-point `eddy_anisotropy.velocity_perp`/`.magnetic_perp` products and generic rendering are implemented; Figure 5(b)'s four normalized eddy-scale curves are admitted under F-046, while the spectral panels retain dimensional/reference-transform gaps; target runs/comparison missing |
...
interface represents surfaces. Figure 4(b)'s `E_rho` spectrum remains
excluded because it does not yet map to a qualified supported normalization.
Data still required:
...
1. **Complete reference coverage and long-time calibration.**
   ...
   transform. Resolve the F-038 paper-to-AthenaK transform before admitting
   dimensional spectra or strain, and establish matching normalizations for
   remaining spectral panels. Obtain or digitize any remaining
