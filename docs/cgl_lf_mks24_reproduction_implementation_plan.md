...
boundary. Accepted corrected-E03 `R02/s20_rankio_t7p5_t7p75` job `4757761`
reached exact `t = 7.75` in `2370` seconds. Formal and independently
regenerated inspection pass with `18` retained products, zero strict LF
failure counters, no restart-marker bypass, terminal
`lf_hwproj = 282606449055`, and forcing-work residual
`3.7666708324783834e-12`. Hardened reconciliation closes at `21/21/21`, with
...
F-104 is the latest hardened normal observed publication under the retained
...
`625f41d9822a31ca5c9e5905a3d411885b56a03605d0c27f3107f11cc740bb5b`.
It authorizes only `R02/s21_rankio_t7p75_t8` on one node with Slurm
`01:05:00`, Athena `00:55:00`, and the reviewed one-segment `2700`-second
threshold. Before preparing `s21`, commit, push, archive, and catalog the
...
hardened reconciliation, `21/21/21`, no active reservation, no transaction,
...
| Standard MKS24 results | Required cases, durations, manifests, figure comparisons | Yes for reproduction claim | Only the admissible fresh E03 R02 prefix has run through exact `t = 7.75`; no corrected-epoch case is complete and long-time mapped results remain not run. The frozen sixteen-case mapped Stage I manifest remains the target matrix. Preserve E02 jobs `4743735` through `4744158` only as pipeline and cost evidence. |
| Operational workflow | Frontier scripts, budget ledger, failure recovery, storage plan | Yes for Frontier use | Corrected E03 entry gate passed. The debug-helper ledger records `1.847784` node-hours after `g031`; corrected E03 qualification accounts for `0.485834`. E02 records `15.628610` historical pipeline/cost node-hours and remains fail closed. Token SHA-256 `e3fec9f35da42121b902f41ef752021f375aab38bc34c5ff75ae8780bfbca635` is retained. Fresh E03 R02 jobs through `4757761` were formally inspected and recorded `accepted` through exact `t = 7.75` for `18.122223` displayed cumulative node-hours. F-102 hardening is committed at `f675bd677cb582a46bbda8b5f55c335e6f259dd3`; hardened reconciliation passed with `21/21/21` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. F-103 and F-104 are normal observed publications under the retained lifecycle companion. |
...
| Performance suitability | Representative timing/memory/I/O evidence; no uninvestigated prohibitive bottleneck | Yes for production use | Partial 2026-06-03. E03 `g030b` reaches reduced nonlinear `t = 2.0` in `1516` allocated seconds with fourteen MPI-I/O records. E03 `g031` launches the exact `192 x 192 x 384` ranked layout, fits one node, and reaches startup-only `t = 0.01` in `77` seconds while retaining `509685800` terminal snapshot bytes and `1381369688` terminal checkpoint bytes. Accepted E03 R02 quarter-unit continuation job `4757761` measures `0.658333` displayed node-hours across `t = 7.5`--`7.75`; canonical F-104 recost evidence projects `829.2566672222222` matrix node-hours with `70.74333277777782` margin. The clean `2370`-second segment leaves `330` seconds below its reviewed `s20` threshold, and the conservative `s21` estimator selects `2560 <= 2700` seconds. After the post-F-104 documentation and archive gate and completion of the durable handoff checklist items 9--11, prepare only `R02/s21_rankio_t7p75_t8` on one node from authenticated `s20` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. It does not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. |
...
| Production ready for supported CGL-LF use | Not established | Corrected E03 debug qualification, retained token, token-gated accounting tooling, committed epoch isolation and shared-root controls, immutable corrected build archival, retained focused serial/MPI CPU gates, historical E02 pipeline/cost evidence, accepted fresh E03 R02 jobs through `4757761` and exact `t = 7.75`, archived F-079--F-104 controller provenance, retained lifecycle-companion publications, and hardened clean reconciliation exist | Commit, push, archive, and catalog the post-F-104 documentation checkpoint, validate the archive ledger, and require completion of the durable handoff checklist items 9--11 before preparing only the authenticated R02 continuation from authenticated `s20` terminal siblings through `t = 8.0` under the reviewed non-ratcheting `s21` quarter profile; any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation; then execute representative mapped production and complete production/statistical qualification |
...
authorize preparation after accepted job `4757761`; the documentation
...
`f8d05a4483a902f21f49c9ebb8935dba3e21d4186c40eda050ae57d169f6bb41`.
Hardened reconciliation closes with `21/21/21` ledger
rows/manifests/reservations, no active reservation, no transaction, and
`issues = []`. Commit, push, archive, and catalog the post-F-104 documentation
...
beta-25 record. Require completion of the durable handoff checklist items 9--11
before preparation. Then prepare only `R02/s21_rankio_t7p75_t8` on one node from
authenticated `s20` terminal siblings with Slurm walltime `01:05:00`, Athena
timeout `00:55:00`, and its reviewed one-segment `2700`-second threshold
retaining `600` seconds on both timeout margins. The threshold is scoped only
to `s21` and cannot ratchet automatically. Any scientific, provenance,
...
exact `t = 7.75`;
commit, push, archive, and catalog the post-F-104 documentation checkpoint,
...
record, and require completion of the durable handoff checklist items 9--11
before continuing only with `R02/s21_rankio_t7p75_t8` on one node from its
authenticated `s20` restart siblings under the full F-104 profile: Slurm
walltime `01:05:00`, Athena timeout `00:55:00`, and a one-segment
`2700`-second threshold retaining `600` seconds on both timeout margins. The
threshold is scoped only to `s21` and cannot ratchet automatically. Any
scientific, provenance, scheduler, storage, budget, or reconciliation failure
blocks successor preparation. Inspect, account, reconcile, and recost before
any further extension.
...
queue/shared-root audit with explicit acknowledgement of the stale beta-25
record, and require completion of the durable handoff checklist items 9--11
before preparing only `R02/s21_rankio_t7p75_t8` on one node from the
authenticated `s20` terminal siblings under the full F-104 profile: Slurm
walltime `01:05:00`, Athena timeout `00:55:00`, and a one-segment
`2700`-second threshold retaining `600` seconds on both timeout margins. The
threshold is scoped only to `s21` and cannot ratchet automatically. Any
scientific, provenance, scheduler, storage, budget, or reconciliation failure
...
2. **Preserve the accepted fresh E03 R02 pilot from `t = 0`.**
   ...
   only to `s21` and cannot ratchet automatically.
