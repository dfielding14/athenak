...
| Performance suitability | Representative timing/memory/I/O evidence; no uninvestigated prohibitive bottleneck | Yes for production use | Partial 2026-05-30; G-008 records reduced debug-scale evidence, G010b records reduced shared-MPI-I/O timing, G011 measures startup-only historical standard-layout memory/file size, and corrected replacement-driver nonlinear `g022` reaches `t = 2.0` in `1554` allocated seconds with retained shared MPI-I/O timing. Its terminal modal restart is `227140784` bytes. Pre-replacement `R16` timing is archival only after the turbulent-driver contract change; replacement-driver standard-layout rank-local sizing and production/statistical qualification remain open. |
...
| Validated on Frontier GPU hardware | Supported only for archived pre-replacement reduced qualification cases through F-033 plus replacement-driver startup, rank-local modal restart identity, one-rank/eight-rank decomposition, reduced passive/random smoke, and reduced nonlinear hard-wall execution | Immutable historical HIP/MPI builds; G-001 through G-007 strict/restart/decomposition/reduced-paper records; post-F-026 restart jobs `4658072`/`4658163`, GPU AMR jobs `4658191`/`4658283`, managed-policy confirmation job `4659663`, G010b/G011 I/O evidence, G013 corrected nonlinear hard-wall job `4673404`, and replacement-driver `g015`/`g018`/`g019`/`g020`/`g021`/`g022` | Replacement-driver measured standard-layout rank-local I/O/runtime and long-time statistical qualification remain open; G012 remains the retained failure of the superseded finite-rate hard-wall interpretation |
| Production ready for supported CGL-LF use | Not established | Debug policy/accounting tooling, committed epoch isolation and shared-root controls, immutable replacement-driver build archival, reduced `g015` startup, `g018` rank-local modal restart identity, `g019` one-rank/eight-rank decomposition, `g020`/`g021` passive/random smoke, `g022` nonlinear hard-wall completion, archived reduced GPU qualification including F-033 G013, MPI CPU AMR evidence, historical shared-MPI-I/O measurements, an archived sequential normal-QOS Stage I protocol with accepted pre-replacement `R16` prefix evidence, and RK/AMR-consistent applied hyperbolic pressure traction histories exist | Complete the replacement-driver standard-layout gate; then run a fresh `R16` pilot, recost, execute representative mapped production runs, and complete production/statistical qualification |
...
2. **Qualify the replacement driver and archive one immutable production
   ...
   MPI-I/O evidence. Continue with the standard-layout rank-local I/O gate
   before production.
