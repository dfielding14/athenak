...
qualified spectral ordinate definition. Figure 5(b)'s normalized eddy-scale
relation is also not a curve-ingestion omission: it requires a validated
local-field-conditioned structure-function product that the analyzer does
not currently compute. The
...
| Paper observables and analysis | Synthetic analysis tests and archived reduced-run products | Yes for MKS24 claims | Partial 2026-05-25; reduced histories, restartable RK-integrated applied forcing work/global active residual, RKL2-applied heat-flux contractions with fine-side AMR ownership, AMR-corrected RK-applied total/anisotropic CGL pressure-work ledgers, operator-face cap counters, windowed snapshot PDF/spectral/transfer/alignment/local-strain products, MKS24-normalized transfer and alignment-peak curve comparisons, deduplicated threshold-volume history curves, manifest-qualified proxies and cadence-limited estimates, generic figures, checksum/uncertainty-qualified reference-curve comparison plumbing, Figure 2(b) histories, Figure 4(a) normalized-density PDFs, Figure 7 lower-panel transfer ratios, Figure 8 selected-shell alignment PDFs, Figure 9, Figure 11 lower-panel, and Figure 12 alignment curves, and Figure 13(b),(d) dimensionless curves are implemented; dimensional/unmatched-panel conversion, Figure 5(b) structure-function analysis, standard-run comparison, and remaining panel references remain open |
...
| MKS24 reproduction complete | Not established | Guarded standard/limiter/heat-flux/scale-separation input matrices, paper analysis products, checksum/uncertainty-qualified reference-curve comparison interface, populated Figure 2(b) histories, Figure 4(a) normalized-density PDFs, dimensionless Figure 7 transfer ratios, Figure 8 selected-shell alignment PDFs, Figure 9, Figure 11 lower-panel, and Figure 12 alignment curves, and Figure 13(b),(d) curves exist | Paper-to-AthenaK transforms for dimensional or unmatched joint/spectral panels, Figure 5(b) conditioned-structure-function analysis, other remaining panel reference data, long-time statistical calibration, authorized standard/limiter/heat-flux/scale-separation runs, quantitative panel comparisons, and archived scientific interpretation remain required |
...
| 2026-05-25 | Retain Figure 5(b) outside admitted references until its conditioned-structure-function observable is implemented. | Unlike the corrected Figure 4(a) PDF and Figure 8 alignment slices, Figure 5(b)'s dimensionless axes do not map to an existing analyzer curve: its characteristic eddy lengths are derived from local-field-conditioned structure functions. | F-045 documents the product boundary; it authorizes neither a speculative digitization nor production execution |
...
| --- | --- | --- | --- |
| Figure 2: pressure-density distributions and unstable-volume history | Active and passive, Alfvenic/random, beta 10 and 100 | PDFs of `delta p_parallel`, `delta p_perp`, density; volume fraction beyond thresholds versus time | Reduced threshold-volume and steady-window PDF machinery plus `history.unstable_fraction` comparison implemented; all eight panel-(b) histories vector-extracted with checksummed provenance; panel (a) is raster and needs a qualified joint-product/normalization reference before comparison |
| Figures 3-4: compressive/density behavior | Active/passive; beta and forcing variants | Density PDFs and density/compressive spectra | Steady-window density PDF/spectrum machinery implemented; Figure 4(a)'s explicit `rho/<rho>` coordinate is mapped to eight admitted `pdf.density_fluctuation` curves under F-044; Figure 4(b)'s `E_rho` normalization and Figure 3 spectra remain unqualified; target runs/reference comparison remain open |
| Figures 5-6: turbulent spectra and rate-of-strain suppression | Active/passive, Alfvenic/random | Velocity/magnetic/pressure spectra; parallel/perpendicular velocity-gradient spectra; local-field eddy scales | Local-field velocity-gradient and strain products plus generic rendering implemented; Figure 5(b)'s normalized eddy-scale relation requires a new conditioned-structure-function product under F-045, while the spectral panels retain dimensional/reference-transform gaps; target runs/comparison missing |
...
and generic plots are implemented. Figure 13(b) dimensionless `beta Delta`
...
Figure 2(b), Figure 4(a) normalized-density PDFs, dimensionless Figure 7
...
Figure 4(b)'s `E_rho` spectrum and Figure 2(a)'s joint distribution remain
excluded because they do not yet map to a qualified supported product.
Figure 5(b)'s normalized eddy-scale relation also remains excluded: it
requires the local-field-conditioned structure-function observable described
by the paper, which is not an implemented analysis product.
Data still required:
...
1. **Complete reference coverage and long-time calibration.**
   ...
   extractors.
   Resolve the F-038 paper-to-AthenaK transform before admitting dimensional
   spectra or strain, and establish matching products/normalizations for
   remaining joint or spectral panels. Implement and validate a
   local-field-conditioned structure-function product before targeting Figure
   5(b). Obtain or digitize any remaining targeted panels with recorded
