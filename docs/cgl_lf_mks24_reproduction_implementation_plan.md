...
through Frontier `debug`.  Stage I execution has begun under that protocol;
`R16` is accepted only as a prefix through `t = 7.5`, with job `4686032`
submitted toward `t = 8.5`.
...
3. **Continue the explicitly revised and recorded Stage I run proposal.**
   ...
   execution is now retained for `R16` through an inspected `t = 7.5` prefix,
   with sequential continuation job `4686032` submitted toward `t = 8.5`.
   The completed `t = 10` case, remaining mapped runs, quantitative panel
   comparisons, and paper/statistical qualification remain open.
