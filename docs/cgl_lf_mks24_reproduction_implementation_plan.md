...
| Production ready for supported CGL-LF use | Not established | Corrected E03 debug qualification, retained token, token-gated accounting tooling, committed epoch isolation and shared-root controls, immutable corrected build archival, retained focused serial/MPI CPU gates, historical E02 pipeline/cost evidence, accepted fresh E03 R02 pilot job `4745922` through exact `t = 0.1`, and archived F-079/F-080 controller provenance exist | Continue from the authenticated R02 checkpoint, execute representative mapped production, and complete production/statistical qualification |
...
`R02/s00_rankio_t0_t0p1` job `4745922` was submitted from `t = 0`, formally
inspected, and recorded `accepted` at exact `t = 0.1` for `0.188889`
node-hours. F-079/F-080 controller provenance is archived; prepare the
authenticated R02 continuation.
...
continue only from its authenticated restart siblings.
...
2. **Preserve the accepted fresh E03 R02 pilot from `t = 0`.**
   ...
   valid. F-079/F-080 controller provenance is archived; prepare only an
