...
| Frontier GPU numerical equivalence | CPU/GPU comparison suite, MPI/GPU restart checks, strict diagnostics | Yes for Frontier use | Partial 2026-05-30; archived pre-replacement HIP/MPI builds and G-001 through G013 evidence remain useful historical qualification. They do not qualify the replacement modal OU driver or its restart schema. A fresh immutable replacement-driver build must pass rank-local restart identity, GPU/MPI decomposition, reduced paper smoke, corrected hard-wall nonlinear, and standard-layout rank-local I/O gates before `E02` production. |
...
| Operational workflow | Frontier scripts, budget ledger, failure recovery, storage plan | Yes for Frontier use | Partial 2026-05-30; reduced debug qualification remains archived (`0.851670` node-hours), and the old Stage I ledger records `9.962778` historical node-hours through rejected job `4686032`. Commit `462b9dbd` makes `scripts/frontier/cgl_lf_stage_i.py` write new work beneath `E02-modal-driver`, use separate accounting files, reject cross-epoch ancestry, check all queued user jobs and active top-level shared-root campaign records, and permit only an initial `4.0` node-hour `R16` pilot. The follow-up provenance hardening requires a retained in-root Git bundle while allowing a clean external source checkout; commit and review that control before submission. |
...
| F-060 | 2026-05-30 | Retained source-provenance audit | The campaign-root working checkout was intentionally replaced by Git bundles on 2026-05-29, but both Frontier preparation helpers still required a source checkout beneath the root. The build script now correctly refuses such a checkout, so real preparation would fail or encourage restoring mutable campaign-root source state. The debug helper also copied only one selected restart file, which is insufficient for a rank-local MPI continuation because AthenaK rewrites the `rank_00000000` path for each rank. | `source-archives/README.md`; `scripts/frontier/build_cgl_lf_frontier.sh`; `scripts/frontier/cgl_lf_frontier.py`; `scripts/frontier/cgl_lf_stage_i.py`; `src/main.cpp:235-262` | High | Permit a clean external source checkout, require and checksum an in-root Git bundle containing each prepared revision, record bundle provenance in manifests, and archive complete rank-local restart sets for debug continuation qualification. | Debug helper self-test including bundle and rank-local restart-set checks; focused Stage I bundle-provenance regression; retained replacement-driver bundle through the committed follow-up revision. | Implemented locally; commit the helpers and retain the new source bundle before scheduler submission. |
...
complete sibling set when a rank-local restart is supplied.
...
the committed input and executable revisions. Commit and review the F-060
provenance follow-up before any `E02` submission. Expand the matrix reservation
...
1. **Isolate the fresh execution epoch and harden shared-root submission
   ...
   `4.0` node-hour `R16` pilot. Commit and review those controls before any
   submission. The
...
2. **Qualify the replacement driver and archive one immutable production
   ...
   Retain the source bundle and archive the exact input digest and environment
   before production.
