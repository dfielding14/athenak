...
historically authorized only `R02/s12_rankio_t5p5_t5p75` on one node from the authenticated
...
historically authorized only `R02/s13_rankio_t5p75_t6` on one node from the authenticated
`s12` terminal siblings, with Slurm walltime `01:05:00`, Athena timeout
`00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600`
seconds on both timeout margins. The threshold is scoped only to `s13` and
...
historically authorized only `R02/s14_rankio_t6_t6p25` on one node from the authenticated
`s13` terminal siblings, with Slurm walltime `01:05:00`, Athena timeout
`00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600`
seconds on both timeout margins. The threshold is scoped only to `s14` and
does not ratchet automatically. Any scientific, provenance, scheduler,
storage, budget, or reconciliation failure blocks successor preparation.
...
`0.655278` node-hours. Corrected E03 Stage I use was then `14.165279`
node-hours. Retained recost evidence SHA-256
`5ba7aec52d3e7824cf4ccf52f95ce1e46368c84ed59f0525e2bbbcfc167fde91`
historically authorized only `R02/s15_rankio_t6p25_t6p5` on one node from the
authenticated `s14` terminal siblings, with Slurm walltime `01:05:00`, Athena
timeout `00:55:00`, and a reviewed one-segment `2700`-second threshold retaining
`600` seconds on both timeout margins. The threshold was scoped only to `s15`
and did not ratchet automatically. Any scientific, provenance, scheduler,
storage, budget, or reconciliation failure blocks successor preparation.
...
historically authorized only `R02/s17_rankio_t6p75_t7` on one node from the
authenticated `s16` terminal siblings under the full F-099 non-ratcheting
profile. Authenticated `R02/s17_rankio_t6p75_t7` job `4754008` is formally and
independently inspected and recorded `accepted` through exact `t = 7.0` for
`0.667222` displayed node-hours. Retained F-100 recost
evidence SHA-256
`63438d8843b86815a2e25d4d6de6ef78ce9756c561fe9db3e762965ada73202f`
historically authorized only `R02/s18_rankio_t7_t7p25`. Authenticated `s18`
job `4754394` is accepted through exact `t = 7.25` for `0.650000` node-hours.
Historical F-101 recost evidence SHA-256
`df208829aec85c88fcc2caa757a21ad3ff3372aeb82628cbfe7bd41851faed9a`
was truthfully adopted under the retained companion and historically authorized
only `R02/s19_rankio_t7p25_t7p5`. Authenticated `s19` job `4757300` is formally
and independently inspected, recorded `accepted`, and reconciled through exact
`t = 7.5` for `0.680556` displayed node-hours, bringing corrected-E03 use to
`17.463890` displayed node-hours. Canonical F-103 recost evidence SHA-256
`ab7fa80746fa4d2c65f515b3ace71d5668586e27fde91bbdeb578fc3a4509d98`
normally published under the retained companion and historically authorized
only `R02/s20_rankio_t7p5_t7p75`. Authenticated `s20` job `4757761` is
formally and independently inspected, recorded `accepted`, and reconciled
through exact `t = 7.75` for `0.658333` displayed node-hours, bringing
corrected-E03 use to `18.122223` displayed node-hours. Canonical F-104 recost
evidence SHA-256
`f8d05a4483a902f21f49c9ebb8935dba3e21d4186c40eda050ae57d169f6bb41`
normally published under the retained companion. After the post-F-104
documentation checkpoint is committed, pushed, archived, and cataloged, rerun
hardened reconciliation and repeat the queue/shared-root audit with explicit
stale beta-25 acknowledgement. Require completion of the durable handoff
checklist items 9--11 before preparation. Then prepare only
`R02/s21_rankio_t7p75_t8` on one node from authenticated `s20` terminal
...
timeout margins. The threshold is scoped only to `s21` and does not ratchet
automatically. Any scientific, provenance, scheduler, storage, budget, or
reconciliation failure blocks successor preparation.

That authorization is historical. Authenticated `s21` job `4758233` is
formally and independently inspected, recorded `accepted`, and reconciled
through exact `t = 8.0` for `0.658056` displayed node-hours, bringing
corrected-E03 use to `18.780279` displayed node-hours. Canonical F-105 recost
evidence SHA-256
`242b4ae79b215690d2b552aba9e583cf209d70c822f8dd76da4e7af1b7ba6a5b`
normally published under the retained companion. After the post-F-105
documentation checkpoint is committed, pushed, archived, and cataloged, rerun
hardened reconciliation and repeat the queue/shared-root audit with explicit
stale beta-25 acknowledgement. Require completion of the durable handoff
checklist items 12--14 before preparation. Then prepare only
`R02/s22_rankio_t8_t8p25` on one node from authenticated `s21` terminal
siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s22` and does not ratchet
automatically. Any scientific, provenance, scheduler, storage, budget, or
reconciliation failure blocks successor preparation.

That authorization is historical. Authenticated `s22` job `4758398` is
formally and independently inspected, recorded `accepted`, and reconciled
through exact `t = 8.25` for `0.651389` displayed node-hours, bringing
corrected-E03 use to `19.431668` displayed node-hours. Canonical F-106 recost
evidence SHA-256
`fe78232b5396d9d3ee6a5ef0e39f7aa445cf2c17125691a7d3e63d4a67978bc2`
normally published under the retained companion. After the post-F-106
documentation checkpoint is committed, pushed, archived, and cataloged, rerun
hardened reconciliation and repeat the queue/shared-root audit with explicit
stale beta-25 acknowledgement. Require completion of the durable handoff
checklist items 15--17 before preparation. Then prepare only
`R02/s23_rankio_t8p25_t8p5` on one node from authenticated `s22` terminal
siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s23` and does not ratchet
automatically. Any scientific, provenance, scheduler, storage, budget, or
reconciliation failure blocks successor preparation.

That authorization is historical. Authenticated `s23` job `4758475` is
formally and independently inspected, recorded `accepted`, and reconciled
through exact `t = 8.5` for `0.634722` displayed node-hours, bringing
corrected-E03 use to `20.066390` displayed node-hours. Canonical F-107 recost
evidence SHA-256
`1e4db459d7c0b5211108dee07fe1efa643ac7a655adadb907deb57f36822d699`
normally published under the retained companion. After the post-F-107
documentation checkpoint is committed, pushed, archived, and cataloged, rerun
hardened reconciliation and repeat the queue/shared-root audit with explicit
stale beta-25 acknowledgement. Require completion of the durable handoff
checklist items 19--21 before preparation. Then prepare only
`R02/s24_rankio_t8p5_t8p75` on one node from authenticated `s23` terminal
siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s24` and does not ratchet
automatically. Any scientific, provenance, scheduler, storage, budget, or
reconciliation failure blocks successor preparation.

That authorization is historical. Authenticated `s24` job `4758576` is
formally and independently inspected, recorded `accepted`, and reconciled
through exact `t = 8.75` for `0.651667` displayed node-hours, bringing
corrected-E03 use to `20.718057` displayed node-hours. Canonical F-108 recost
evidence SHA-256
`dbd3f5e3b9f21532da546caca4bdc66263d69c3b60b35938972edd04b2ab3e19`
normally published under the retained companion. After the post-F-108
documentation checkpoint is committed, pushed, archived, and cataloged, rerun
hardened reconciliation and repeat the queue/shared-root audit with explicit
stale beta-25 acknowledgement. Require completion of the durable handoff
checklist items 22--24 before preparation. Then prepare only
`R02/s25_rankio_t8p75_t9` on one node from authenticated `s24` terminal
siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s25` and does not ratchet
automatically. Any scientific, provenance, scheduler, storage, budget, or
reconciliation failure blocks successor preparation.

That authorization is historical. Authenticated `s25` job `4758713` is
formally and independently inspected, recorded `accepted`, and reconciled
through exact `t = 9` for `0.627500` displayed node-hours, bringing
corrected-E03 use to `21.345557` displayed node-hours. Canonical F-109 recost
evidence SHA-256
`67f9096900b5ec3ce8368cd39c6f6a8c649dcff2436261fcf72aa328cd3ac0ea`
normally published under the retained companion. After the post-F-109
documentation checkpoint is committed, pushed, archived, and cataloged, rerun
hardened reconciliation and repeat the queue/shared-root audit with explicit
stale beta-25 acknowledgement. Require completion of the durable handoff
checklist items 25--27 before preparation. Then prepare only
`R02/s26_rankio_t9_t9p25` on one node from authenticated `s25` terminal
siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a
reviewed one-segment `2700`-second threshold retaining `600` seconds on both
timeout margins. The threshold is scoped only to `s26` and does not ratchet
automatically. Any scientific, provenance, scheduler, storage, budget, or
reconciliation failure blocks successor preparation.
...
| Standard MKS24 results | Required cases, durations, manifests, figure comparisons | Yes for reproduction claim | Only the admissible fresh E03 R02 prefix has run through exact `t = 9`; no corrected-epoch case is complete and long-time mapped results remain not run. The frozen sixteen-case mapped Stage I manifest remains the target matrix. Preserve E02 jobs `4743735` through `4744158` only as pipeline and cost evidence. |
| Operational workflow | Frontier scripts, budget ledger, failure recovery, storage plan | Yes for Frontier use | Corrected E03 entry gate passed. The debug-helper ledger records `1.847784` node-hours after `g031`; corrected E03 qualification accounts for `0.485834`. E02 records `15.628610` historical pipeline/cost node-hours and remains fail closed. Token SHA-256 `e3fec9f35da42121b902f41ef752021f375aab38bc34c5ff75ae8780bfbca635` is retained. Fresh E03 R02 jobs through `4758713` were formally inspected and recorded `accepted` through exact `t = 9` for `21.345557` displayed cumulative node-hours. F-102 hardening is committed at `f675bd677cb582a46bbda8b5f55c335e6f259dd3`; hardened reconciliation passed with `26/26/26` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. F-103 through F-109 are normal observed publications under the retained lifecycle companion. |
...
| Performance suitability | Representative timing/memory/I/O evidence; no uninvestigated prohibitive bottleneck | Yes for production use | Partial 2026-06-03. E03 `g030b` reaches reduced nonlinear `t = 2.0` in `1516` allocated seconds with fourteen MPI-I/O records. E03 `g031` launches the exact `192 x 192 x 384` ranked layout, fits one node, and reaches startup-only `t = 0.01` in `77` seconds while retaining `509685800` terminal snapshot bytes and `1381369688` terminal checkpoint bytes. Accepted E03 R02 quarter-unit continuation job `4758713` measures `0.627500` displayed node-hours across `t = 8.75`--`9`; canonical F-109 recost evidence projects `829.2566672222222` matrix node-hours with `70.74333277777782` margin. The clean `2259`-second segment leaves `441` seconds below its reviewed `s25` threshold, and the conservative `s26` estimator selects `2560 <= 2700` seconds. After the post-F-109 documentation and archive gate and completion of the durable handoff checklist items 25--27, prepare only `R02/s26_rankio_t9_t9p25` on one node from authenticated `s25` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. It does not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. |
...
| Production ready for supported CGL-LF use | Not established | Corrected E03 debug qualification, retained token, token-gated accounting tooling, committed epoch isolation and shared-root controls, immutable corrected build archival, retained focused serial/MPI CPU gates, historical E02 pipeline/cost evidence, accepted fresh E03 R02 jobs through `4758713` and exact `t = 9`, archived F-079--F-109 controller provenance, retained lifecycle-companion publications, and hardened clean reconciliation exist | Commit, push, archive, and catalog the post-F-109 documentation checkpoint, validate the archive ledger, and require completion of the durable handoff checklist items 25--27 before preparing only the authenticated R02 continuation from authenticated `s25` terminal siblings through `t = 9.25` under the reviewed non-ratcheting `s26` quarter profile; any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation; then execute representative mapped production and complete production/statistical qualification |
...
only the newest F-100 row together with the current handoff sections governs
...
| F-095 | 2026-05-31 | Fresh corrected E03 R02 quarter-unit continuation through exact `t = 5.75`, retained quarter validator, and reviewed bounded quarter continuation | F-094 authorized only one quarter-unit continuation under the reviewed `2700`-second threshold scoped to `s12`. The accepted clean interval remains inside that threshold, but later quarter units still require explicit recosting and cannot ratchet automatically. | accepted `R02/s12_rankio_t5p5_t5p75`; job `4747500`; source bundle `athenak-feature-cgl-through-8a0a406b6.bundle` SHA-256 `b6e3188aba94f0f48e3d129b455bc28ea67e1cfac63eb55ba2101fc19e0c1fb5`; record-time inspection SHA-256 `24dbc74639d09222b321f805af0d47adf5b917f2cd83d765bb75f69ddadb5680`; recorded manifest SHA-256 `56a364e52f158e6049a7dfb269a8e331d74028b6788177b28b5997656a766ebb`; retained independent validation SHA-256 `7d27cd3a67eb06fdde1b96cb0c5809fef524eeb08d71a745fca2927f0d203a0e`; retained quarter validator SHA-256 `0ec3004247f62e2266939c94ef07cdb94513d6e8edc2ceddcf0504e3ac812789`; retained one-use recost generator SHA-256 `eb54488fed0b321f6dd935e35fb88c67706283a0b0b2e03092d369e00436d2ab`; retained scheduler evidence SHA-256 `c982bbc266668c0205153f1237d94ed196cc0895dbdd78c553bd26bc4b404e02`; retained corrected R02 recost evidence JSON SHA-256 `d0a60e222138971e1bc9aae978bb3d62aee460f09000ee62f7f9ffdfe534165f` | High | Continue from authenticated E03 `t = 5.5` restart siblings through exact `t = 5.75`; require formal and retained independent quarter inspection, exactly one eight-rank snapshot group, complete terminal restart siblings, zero strict LF failure counters, finite synchronized histories, forcing-work closure, accounting, reconciliation, staged recost review, and byte-preserving promotion. Retain utility bytes durably before use. | Job `4747500` reaches exact `t = 5.75` in `2376` seconds (`0.660000` node-hours). Strict LF failure counters remain zero; terminal `lf_hwproj = 235196163710`; one eight-rank snapshot group and terminal eight-rank restart siblings are retained; sampled-history forcing-work relative residual is `6.6019081979506825e-12`; accepted-prefix residual is `2.5099635566634386e-12`; reconciliation closes with `13/13/13` ledger rows/manifests/reservations, no active reservation, and no transaction. The quarter-unit rate is `2.640000` node-hours per simulated unit. Authorization-facing projection is `814.945556` matrix node-hours with `85.054444` margin. The reviewed estimator selects `2468 <= 2700` seconds for `s13`. Lustre rejected `RENAME_NOREPLACE` for directory and file publication with `EINVAL`; reviewed lock-held atomic POSIX-directory-rename and hard-link no-clobber recoveries retained exact staged bytes. Carver, Linnaeus, Godel, and Newton independently reviewed the retained toolchain or staged evidence. | Archive and catalog the current committed controller state, reconcile, then prepare only `R02/s13_rankio_t5p75_t6` on one node from authenticated `s12` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. Do not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any further extension. Keep R17 last. |
...
| F-099 | 2026-06-01 | Fresh corrected E03 R02 quarter-unit continuation through exact `t = 6.75`, retained hardened recost generator, and reviewed bounded quarter continuation | F-098 historically authorized only one quarter-unit continuation under the reviewed `2700`-second threshold scoped to `s16`. The accepted clean interval remains inside that threshold, but later quarter units still require explicit recosting and cannot ratchet automatically. | accepted `R02/s16_rankio_t6p5_t6p75`; job `4753294`; exact scheduler row `4753294\|cgl_mks24_E03_forcing_policy_R02_s16_rankio_t6p5_t6p75\|COMPLETED\|0:0\|1\|2330\|2026-06-01T20:40:10\|2026-06-01T21:19:07`; source bundle `athenak-feature-cgl-through-6c0739806.bundle` SHA-256 `a837ebe60d62a922dde3fc3ddd87350eec8a9e3f439f671f783346ee6290617f`; previously audited, non-retained submitted-state manifest SHA-256 `10c469c424159ecc3ca283a2c951aa2c3456d9b7fbd01841926ee55b973c42f0`; record-time inspection SHA-256 `1149de945447eae3b274052985363c1c821720a117254662657e33d8b21467de`; recorded manifest SHA-256 `a87787fdaf4a0a8491bdf42e834fba7399f084ffef1cd9ff5e9c4aae84037089`; retained independent validation SHA-256 `4889d08083004d3d89f6cc8beede3521ce7f8143983c7b4bb233209d59844b05`; retained quarter validator SHA-256 `0ec3004247f62e2266939c94ef07cdb94513d6e8edc2ceddcf0504e3ac812789`; retained one-use recost generator SHA-256 `e9991ec75559113fb0da5b5fae5d117f7fe6701160316237227169dde04d05af`; reviewed one-shot, non-retained recost promoter SHA-256 `08fae00aa1cc365cb39d0b935f059a732955465eb26977e7fd459791ae762883`; retained scheduler evidence SHA-256 `11b3d7bbae54e4c51fa7548134ae0e7e837fe8b624af698a2e32f30c9cd82fc9`; retained corrected R02 recost evidence JSON SHA-256 `5b997623a3f8d83c200034f42e0c9f1b9a09c811bcb46e2fc1a0f7c1eb58815e` | High | Continue from authenticated E03 `t = 6.5` restart siblings through exact `t = 6.75`; require formal and retained independent quarter inspection, exactly one eight-rank snapshot group, complete terminal restart siblings, zero strict LF failure counters, finite synchronized histories, forcing-work closure, accounting, reconciliation, staged recost review, and byte-preserving promotion. Retain utility bytes durably before use. | Job `4753294` reaches exact `t = 6.75` in `2330` seconds (`0.647222` displayed node-hours). Strict LF failure counters remain zero; terminal `lf_hwproj = 259077916291`; one eight-rank snapshot group and terminal eight-rank restart siblings are retained; sampled-history forcing-work relative residual is `6.437727059090897e-12`; the accepted prefix totals `55678` seconds (`15.466111111111111` exact and `15.466112` displayed node-hours) with residual `3.1385227986388927e-12`; reconciliation closes with `17/17/17` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. The quarter-unit rate is `2.588889` node-hours per simulated unit. Authorization-facing projection is `829.1011116666666` exact (`829.101112` displayed) matrix node-hours with `70.89888833333339` exact (`70.898888` displayed) margin. With prior selected `2559`, previous observed `2353`, observed `2330`, and local `2330`, the reviewed estimator selects `2559 <= 2700` seconds for `s17`; the accepted segment leaves `370` seconds of threshold headroom. | Commit the post-F-099 documentation state, archive and catalog its resulting actual controller bundle and SHA-256, rerun reconciliation, repeat the queue/shared-root audit with explicit acknowledgement of the stale beta-25 record, then prepare only `R02/s17_rankio_t6p75_t7` on one node from authenticated `s16` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. Do not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any further extension. Keep R17 last. |
| F-100 | 2026-06-02T03:10:59+00:00 | Fresh corrected E03 R02 quarter-unit continuation through exact `t = 7.0`, retained hardened recost generator, and reviewed bounded quarter continuation | F-099 historically authorized only one quarter-unit continuation under the reviewed `2700`-second threshold scoped to `s17`. The accepted clean interval remains inside that threshold, but later quarter units still require explicit recosting and cannot ratchet automatically. | accepted `R02/s17_rankio_t6p75_t7`; job `4754008`; exact scheduler row `4754008\|cgl_mks24_E03_forcing_policy_R02_s17_rankio_t6p75_t7\|COMPLETED\|0:0\|1\|2402\|2026-06-01T22:24:23\|2026-06-01T23:04:53`; source launch bundle `athenak-feature-cgl-through-e119e2dcf.bundle` SHA-256 `25ffc9cd279a02debf8b55e318f0674ac3c14bf9a05cf05827218ae2056a5f14`; previously audited, non-retained submitted-state manifest SHA-256 `a1d986135c397f68a986accff0a4db6621caf644f57cc8c5ae720ef35dcaeea6`; record-time inspection SHA-256 `f5028f8d6c6157e837b34e06712d68360040bfebdfb5bb1bfdc0221cd8431db8`; recorded manifest SHA-256 `73b970e5014ac425f805013ad2901935b7a884bbbcde54ba9a785a53222d63ea`; retained independent validation SHA-256 `af4e188ae27904e41b6bf7d517784688ac2b46be03de70d7285c96a4b1e8ccdd`; retained quarter validator SHA-256 `0ec3004247f62e2266939c94ef07cdb94513d6e8edc2ceddcf0504e3ac812789`; retained one-use recost generator SHA-256 `b55c741516ccd7e5be236856599e6abf6adf859bb2ebacc801e6164d994db4aa`; reviewed one-shot, non-retained recost promoter SHA-256 `b3d62c75bd175a73df8dcf42a607a37551774261c2bca1193b0a2be125e5d8e4`; retained scheduler evidence SHA-256 `4ca296c0eb44863dbde4fec34e35b145eba7691df6938688f0c55f2752560fae`; retained corrected R02 recost evidence JSON SHA-256 `63438d8843b86815a2e25d4d6de6ef78ce9756c561fe9db3e762965ada73202f` | High | Continue from authenticated E03 `t = 6.75` restart siblings through exact `t = 7.0`; require formal and retained independent quarter inspection, exactly one eight-rank snapshot group, complete terminal restart siblings, zero strict LF failure counters, finite synchronized histories, forcing-work closure, accounting, reconciliation, staged recost review, and byte-preserving promotion. Retain utility bytes durably before use. | Job `4754008` reaches exact `t = 7.0` in `2402` seconds (`0.667222` displayed node-hours). Strict LF failure counters remain zero; terminal `lf_hwproj = 265802141460`; one eight-rank snapshot group and terminal eight-rank restart siblings are retained; sampled-history forcing-work relative residual is `7.10881221809853e-12`; the accepted prefix totals `58080` seconds (`16.133333333333333` exact and `16.133334` displayed node-hours) with residual `3.2704293797689304e-12`; reconciliation closes with `18/18/18 ledger rows/manifests/reservations, no active reservation, no transaction, and issues = []`. With prior selected `2559`, previous observed `2330`, observed `2402`, and local `2474`, the reviewed estimator selects `2559 <= 2700` seconds for `s18`; the accepted segment leaves `298` seconds of threshold headroom. Authorization-facing projection is `829.1011116666666` exact (`829.101112` displayed) matrix node-hours with `70.89888833333339` exact (`70.898888` displayed) margin. | Commit the post-F-100 documentation state, archive and catalog its resulting actual controller bundle and its SHA-256, rerun reconciliation, repeat the queue/shared-root audit with explicit acknowledgement of the stale beta-25 record, then prepare only `R02/s18_rankio_t7_t7p25` on one node from authenticated `s17` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. Do not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any further extension. Keep R17 last. |
| F-101 | 2026-06-02T23:35:28+00:00 | Truthfully adopt externally canonical F-101 recost evidence under the retained companion | The reviewed F-101 artifact appeared canonical-only before the retained companion observed publication. Normal promotion cannot retroactively claim a transition it did not execute. | `accounting/mks24_stage_i_E03_forcing_policy_R02_t7p25_recost_evidence.json` SHA-256 `df208829aec85c88fcc2caa757a21ad3ff3372aeb82628cbfe7bd41851faed9a`; publication audit SHA-256 `9bbfc6ee1007b0e17a4408444acb1bcdb2f59cbde9183569d506fe2606c1837d`; forensic copy with the same artifact SHA-256; retained companion commit `89ba4143c448c26fd8a66111d0409b4bcd3eea89`; post-adoption docs commit `958829bb84929c3d806c0c8d7971eff1c27df743` | Blocker | Under a fresh empty-queue and free-lock boundary, use `adopt-legacy-canonical` exactly once, then separately run `verify-promoted-recost`. Preserve `original_publication_transition_observed = false`, unknown original publication method, unknown original publisher, and the prior Stage I helper binding. | Closed. The retained mode-`0444` forensic copy, mode-`0644` audit, canonical-only artifact, empty recost transaction directory, empty queue, and free strict lock were separately inspected. Do not rerun adoption. |
| F-102 | 2026-06-02T23:58:06+00:00 | Harden canonical Stage I lock opening and Slurm control-plane routing before `s19` | The retained Stage I helper predated the companion's strict lock profile and allowed production scheduler fixtures, `--skip-slurm-test`, caller `PATH`, inherited `SLURM_*`, and caller `$USER` to influence production control-plane paths. | commit `f675bd677cb582a46bbda8b5f55c335e6f259dd3`; helper SHA-256 `54ec671bb45aa27735a174d40b4b2e6009070716346ea09699bbe62421bbfada`; source bundle `athenak-feature-cgl-through-f675bd677.bundle` SHA-256 `eb32cd98a97ff57311d1a783ca6b80d3c22192370c09e85fc72843fc59ae7c42`; exact-staged independent review; focused helper slice `10 passed, 35 deselected`; retained companion suite `67 passed` | Blocker | Require `O_NOFOLLOW`, regular file, effective-UID owner, one link, restricted mode, exact `0644` normalization, post-lock pathname identity, fixed `/usr/bin/{sacct,sbatch,scontrol,squeue}`, effective-UID scheduler account lookup, stripped `SLURM_*`, and production rejection of `--squeue-file` and `--skip-slurm-test`. Cover direct scheduler command construction and lock-profile failures with focused regressions. | Closed before any `s19` mutation. F-101 remains bound to the prior helper bytes. Require a fresh live queue/free-lock boundary and authenticated hardened reconciliation before preparing only `R02/s19_rankio_t7p25_t7p5`. |
| F-103 | 2026-06-03T01:28:52+00:00 | Accept corrected E03 R02 quarter-unit continuation through exact `t = 7.5` and normally publish the first hardened retained-companion recost checkpoint | F-101 required truthful legacy adoption and F-102 hardened lock and scheduler routing before `s19`. The reviewed `s19` packet was then prepared and submitted only after fresh queue, strict free-lock, hardened reconciliation, and independent readiness gates. Its clean endpoint still requires a new one-segment recost rather than automatic threshold ratcheting. | accepted `R02/s19_rankio_t7p25_t7p5`; job `4757300`; exact scheduler row `4757300\|cgl_mks24_E03_forcing_policy_R02_s19_rankio_t7p25_t7p5\|COMPLETED\|0:0\|1\|2450\|2026-06-02T20:18:10\|2026-06-02T20:59:12`; source launch bundle `athenak-feature-cgl-through-28113fd06.bundle` SHA-256 `e1910db1dcb008a6b39888536e7b6fabf527fac525fb3550b466a7801ffafaf0`; submitted-state manifest SHA-256 `205205c8934c04c28775097514ae5e83bf30c05013ab3e2897b449506d7e8f1a`; record-time inspection SHA-256 `8b9cfb6f3d0ec215234d9481e031299da1286117424d756f654270930881f744`; recorded manifest SHA-256 `30445e1c64dc35c64db38f119ff6838c904ee4d71f78ec75b219c4d476bb7b30`; retained independent validation SHA-256 `6ebd5054e8d8082d75343b9cd176d7faf8c3b773774d8e145302bf3bfd859dcf`; retained scheduler evidence SHA-256 `9a45c253369f87f8f7a31ce1189ac2df5b312b293c058a6f5bd9aa2c9e953f48`; retained recost generator SHA-256 `131ab294888fd70bad7140a781ebc43ea6cb05bc4eddcb15dbee91c2a445ba28`; canonical recost artifact SHA-256 `ab7fa80746fa4d2c65f515b3ace71d5668586e27fde91bbdeb578fc3a4509d98`; observed-publication audit SHA-256 `28e8526eb3c225cde028476dea95657b170264ea7f77156b8a839e2fe03bbf6b`; forensic copy with the canonical artifact SHA-256 | High | Continue from authenticated E03 `t = 7.25` restart siblings through exact `t = 7.5`; require formal and retained independent quarter inspection, exactly one eight-rank snapshot group, complete terminal restart siblings, zero strict LF failure counters, finite synchronized histories, forcing-work closure, live scheduler accounting, hardened reconciliation, retained-generator review, staged recost verification, independent staged review, normal companion promotion, and separate promoted verification. | Job `4757300` reaches exact `t = 7.5` in `2450` seconds (`0.680556` displayed node-hours). Strict LF failure counters remain zero; terminal `lf_hwproj = 279167641195`; one eight-rank snapshot group and terminal eight-rank restart siblings are retained; sampled-history forcing-work relative residual is `8.520423303056633e-12`; accepted corrected-E03 prefix totals `62870` seconds (`17.46388888888889` exact and `17.463890` displayed node-hours); hardened reconciliation closes with `20/20/20` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. The reviewed estimator uses prior selected `2559`, previous observed `2340`, observed `2450`, local acceleration `2560`, and selects `2560 <= 2700` seconds for `s20`; `s19` leaves `250` seconds of threshold headroom. Authorization-facing projection is `829.2566672222222` matrix node-hours with `70.74333277777782` margin. The companion normally published canonical-only bytes with record type `observed-publication`, same-directory `link/fsync/unlink/fsync`, a mode-`0444` forensic copy, empty recost transaction directory, empty queue, and free strict lock. | Commit, push, archive, and catalog the post-F-103 documentation state, validate the full archive checksum ledger, and require completion of the durable handoff checklist items 6--8 before preparing only `R02/s20_rankio_t7p5_t7p75` on one node from authenticated `s19` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. Do not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any further extension. Keep R17 last. |
| F-104 | 2026-06-03T03:21:44+00:00 | Accept corrected E03 R02 quarter-unit continuation through exact `t = 7.75` and normally publish the next hardened retained-companion recost checkpoint | F-103 authorized only `s20`. Its clean endpoint still requires a new one-segment recost rather than automatic threshold ratcheting. | accepted `R02/s20_rankio_t7p5_t7p75`; job `4757761`; exact scheduler row `4757761\|cgl_mks24_E03_forcing_policy_R02_s20_rankio_t7p5_t7p75\|COMPLETED\|0:0\|1\|2370\|2026-06-02T22:01:48\|2026-06-02T22:41:35`; source launch bundle `athenak-feature-cgl-through-07e044440.bundle` SHA-256 `4dc5c3820b385401e0d60bfaecde9bf91fbb16467ae4ff5c6604c35652beec15`; record-time inspection SHA-256 `4094abc01fb9fa2f4e690f17c549270d519a1fbbeab63c49ff8856bb4cfba791`; recorded manifest SHA-256 `06e70f554196b0da40cb343bf0c5a7bd8e9b7ee2fafdf0e88e4019a9980a6ea0`; retained independent validation SHA-256 `eec64e5b065414912385549c2b42406683ec6fce4787bd525c357a4226ef958f`; retained scheduler evidence SHA-256 `0d726cffe32b00f14ccad5627bf4823eb9092da491c2c5aba878a52b25040573`; retained recost generator SHA-256 `ca3eebaaf0d04dd82a3daa9ee40006a888e0ac82ca74872e8bc63afb8a54e971`; canonical recost artifact SHA-256 `f8d05a4483a902f21f49c9ebb8935dba3e21d4186c40eda050ae57d169f6bb41`; observed-publication audit SHA-256 `625f41d9822a31ca5c9e5905a3d411885b56a03605d0c27f3107f11cc740bb5b`; forensic copy with the canonical artifact SHA-256 | High | Continue from authenticated E03 `t = 7.5` restart siblings through exact `t = 7.75`; require formal and retained independent quarter inspection, exactly one eight-rank snapshot group, complete terminal restart siblings, zero strict LF failure counters, finite synchronized histories, forcing-work closure, live scheduler accounting, hardened reconciliation, retained-generator review, staged recost verification, independent staged review, normal companion promotion, and separate promoted verification. | Job `4757761` reaches exact `t = 7.75` in `2370` seconds (`0.658333` displayed node-hours). Strict LF failure counters remain zero; terminal `lf_hwproj = 282606449055`; one eight-rank snapshot group and terminal eight-rank restart siblings are retained; sampled-history forcing-work relative residual is `3.7666708324783834e-12`; accepted corrected-E03 prefix totals `65240` seconds (`18.122222222222224` exact and `18.122223` displayed node-hours); hardened reconciliation closes with `21/21/21` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. The reviewed estimator uses prior selected `2560`, previous observed `2450`, observed `2370`, local acceleration `2370`, and selects `2560 <= 2700` seconds for `s21`; `s20` leaves `330` seconds of threshold headroom. Authorization-facing projection is `829.2566672222222` matrix node-hours with `70.74333277777782` margin. The companion normally published canonical-only bytes with record type `observed-publication`, same-directory `link/fsync/unlink/fsync`, a mode-`0444` forensic copy, empty recost transaction directory, empty queue, and free strict lock. | Commit, push, archive, and catalog the post-F-104 documentation state, validate the full archive checksum ledger, and require completion of the durable handoff checklist items 9--11 before preparing only `R02/s21_rankio_t7p75_t8` on one node from authenticated `s20` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. Do not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any further extension. Keep R17 last. |
| F-105 | 2026-06-03T05:27:00+00:00 | Accept corrected E03 R02 quarter-unit continuation through exact `t = 8.0` and normally publish the next hardened retained-companion recost checkpoint | F-104 authorized only `s21`. Its clean endpoint still requires a new one-segment recost rather than automatic threshold ratcheting. | accepted `R02/s21_rankio_t7p75_t8`; job `4758233`; exact scheduler row `4758233\|cgl_mks24_E03_forcing_policy_R02_s21_rankio_t7p75_t8\|COMPLETED\|0:0\|1\|2369\|2026-06-02T23:59:21\|2026-06-03T00:46:02`; source launch bundle `athenak-feature-cgl-through-aa3e2a59d.bundle` SHA-256 `e8a0f2028d67f908efd3ef0390f2994321ef11a6a1aa6f0d1cbb78732c18182b`; record-time inspection SHA-256 `6c1fe59719c109b011b9c2db4d8e6ff5af57b028c818c5f29b2b370203112199`; recorded manifest SHA-256 `69bd7db74380862fb887f218499879774a3a3c6e94ecbc3ba9ccf855a06b4ca2`; retained independent validation SHA-256 `11d20c2cf311563ea36b366f472cee90323d4835d59e9d9ab4f9776cefe1ab44`; retained scheduler evidence SHA-256 `24a54707bcf8de9cf7fd85f7a975d58900c35bdfe0289d354e612299120f4f21`; retained recost generator SHA-256 `0d06691c0fcd30dbd8796f3fdd3dcab70486f772638cfcdcfabbb2b5298b3896`; canonical recost artifact SHA-256 `242b4ae79b215690d2b552aba9e583cf209d70c822f8dd76da4e7af1b7ba6a5b`; observed-publication audit SHA-256 `3cbf9c53d8fea066a67f334fe6393da7c8b31d3429d0c4582047b8174e6e7e7f`; forensic copy with the canonical artifact SHA-256 | High | Continue from authenticated E03 `t = 7.75` restart siblings through exact `t = 8.0`; require formal and retained independent quarter inspection, exactly one eight-rank snapshot group, complete terminal restart siblings, zero strict LF failure counters, finite synchronized histories, forcing-work closure, live scheduler accounting, hardened reconciliation, retained-generator review, staged recost verification, independent staged review, normal companion promotion, and separate promoted verification. | Job `4758233` reaches exact `t = 8.0` in `2369` seconds (`0.658056` displayed node-hours). Strict LF failure counters remain zero; terminal `lf_hwproj = 286108988748`; one eight-rank snapshot group and terminal eight-rank restart siblings are retained; sampled-history forcing-work relative residual is `2.8542854449297643e-12`; accepted corrected-E03 prefix totals `67609` seconds (`18.780277777777776` exact and `18.780279` displayed node-hours); hardened reconciliation closes with `22/22/22` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. The reviewed estimator uses prior selected `2560`, observed `2369`, local acceleration `2369`, and selects `2560 <= 2700` seconds for `s22`; `s21` leaves `331` seconds of threshold headroom. Authorization-facing projection is `829.2566672222222` matrix node-hours with `70.74333277777782` margin. The companion normally published canonical-only bytes with record type `observed-publication`, same-directory `link/fsync/unlink/fsync`, a mode-`0444` forensic copy, empty recost transaction directory, empty queue, and free strict lock. | Commit, push, archive, and catalog the post-F-105 documentation state, validate the full archive checksum ledger, and require completion of the durable handoff checklist items 12--14 before preparing only `R02/s22_rankio_t8_t8p25` on one node from authenticated `s21` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. Do not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any further extension. Keep R17 last. |
| F-106 | 2026-06-03T07:36:16+00:00 | Accept corrected E03 R02 quarter-unit continuation through exact `t = 8.25` and normally publish the next hardened retained-companion recost checkpoint | F-105 authorized only `s22`. Its clean endpoint still requires a new one-segment recost rather than automatic threshold ratcheting. | accepted `R02/s22_rankio_t8_t8p25`; job `4758398`; exact scheduler row `4758398\|cgl_mks24_E03_forcing_policy_R02_s22_rankio_t8_t8p25\|COMPLETED\|0:0\|1\|2345\|2026-06-03T02:25:21\|2026-06-03T03:06:00`; source launch bundle `athenak-feature-cgl-through-4bb580e22.bundle` SHA-256 `8e4f6dbc5a7318b3e17db33348ca8fdd346c5ba8d161faaf20c23a0fdeb49411`; record-time inspection SHA-256 `77479a51cda10b1a399a9060c5a7153dec115389cd4f0ba5a6fec167c462be49`; recorded manifest SHA-256 `b02e754cf521a0b0e2205bcd13947c0c12b6a443adb268f9ea91a2eeeff5a803`; retained independent validation SHA-256 `e8854e4b233d7b1b64fcc53e69975041e052e98477b85e74b8e0dc1d5a332d08`; retained scheduler evidence SHA-256 `e6af51d0cbbd1aef3e83df0fa99f64da77d7aa991a59c0375b21b4fe07c44ea1`; retained recost generator SHA-256 `341fda3812143288806285b6f8ec3969d43d0e0bd22f150190e5e4f44c57b3b6`; canonical recost artifact SHA-256 `fe78232b5396d9d3ee6a5ef0e39f7aa445cf2c17125691a7d3e63d4a67978bc2`; observed-publication audit SHA-256 `a8289b06194f93492f20fdc965ef6e709d3b1179fc251de29efa9af01c524b99`; forensic copy with the canonical artifact SHA-256 | High | Continue from authenticated E03 `t = 8.0` restart siblings through exact `t = 8.25`; require formal and retained independent quarter inspection, exactly one eight-rank snapshot group, complete terminal restart siblings, zero strict LF failure counters, finite synchronized histories, forcing-work closure, live scheduler accounting, hardened reconciliation, retained-generator review, staged recost verification, independent staged review, normal companion promotion, and separate promoted verification. | Job `4758398` reaches exact `t = 8.25` in `2345` seconds (`0.651389` displayed node-hours). Strict LF failure counters remain zero; terminal `lf_hwproj = 290890469421`; one eight-rank snapshot group and terminal eight-rank restart siblings are retained; sampled-history forcing-work relative residual is `3.021717626579328e-12`; accepted corrected-E03 prefix totals `69954` seconds (`19.43166666666667` exact and `19.431668` displayed node-hours); hardened reconciliation closes with `23/23/23` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. The reviewed estimator uses prior selected `2560`, previous observed `2369`, observed `2345`, local acceleration `2345`, and selects `2560 <= 2700` seconds for `s23`; `s22` leaves `355` seconds of threshold headroom. Authorization-facing projection is `829.2566672222222` matrix node-hours with `70.74333277777782` margin. The companion normally published canonical-only bytes with record type `observed-publication`, same-directory `link/fsync/unlink/fsync`, a mode-`0444` forensic copy, empty recost transaction directory, empty queue, and free strict lock. | Commit, push, archive, and catalog the post-F-106 documentation state, validate the full archive checksum ledger, and require completion of the durable handoff checklist items 15--17 before preparing only `R02/s23_rankio_t8p25_t8p5` on one node from authenticated `s22` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. Do not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any further extension. Keep R17 last. |
| F-107 | 2026-06-03T09:28:03+00:00 | Accept corrected E03 R02 quarter-unit continuation through exact `t = 8.5` and normally publish the next hardened retained-companion recost checkpoint | F-106 authorized only `s23`. Its clean endpoint still requires a new one-segment recost rather than automatic threshold ratcheting. | accepted `R02/s23_rankio_t8p25_t8p5`; job `4758475`; exact scheduler row `4758475\|cgl_mks24_E03_forcing_policy_R02_s23_rankio_t8p25_t8p5\|COMPLETED\|0:0\|1\|2285\|2026-06-03T04:11:20\|2026-06-03T04:49:40`; source launch bundle `athenak-feature-cgl-through-849b052dd.bundle` SHA-256 `adf897b8f60149bdcf89bc0a42f69f224ec6486155341411df1ccd8bd2d7b8ed`; record-time inspection SHA-256 `318786f1be49ef894606f579709942bdb38f0de37c4ab23b357569b0deb36b2b`; recorded manifest SHA-256 `aa01c14794007564e708125c4be3ce1f683c3e33a92084b0f65e67944d454149`; retained independent validation SHA-256 `08cd3c894106f295eda79571faca85dc23c1aaf621ba1a5c1eb09c253bb152fe`; retained scheduler evidence SHA-256 `86bc48723ad2365729c415623ab4fc7baf47a69ccc83cf10410f84b01440e4fe`; retained recost generator SHA-256 `94548cf12706bc4abe9449bcb0d917d09ab58fa241cc3f3ec48f730ed701c479`; canonical recost artifact SHA-256 `1e4db459d7c0b5211108dee07fe1efa643ac7a655adadb907deb57f36822d699`; observed-publication audit SHA-256 `7dfca5ec22cc1bdd265a75e9aec275c81848c193562f5d7661bae1a79751e7f4`; forensic copy with the canonical artifact SHA-256 | High | Continue from authenticated E03 `t = 8.25` restart siblings through exact `t = 8.5`; require formal and retained independent quarter inspection, exactly one eight-rank snapshot group, complete terminal restart siblings, zero strict LF failure counters, finite synchronized histories, forcing-work closure, live scheduler accounting, hardened reconciliation, retained-generator review, staged recost verification, independent staged review, normal companion promotion, and separate promoted verification. | Job `4758475` reaches exact `t = 8.5` in `2285` seconds (`0.634722` displayed node-hours). Strict LF failure counters remain zero; terminal `lf_hwproj = 295614667845`; one eight-rank snapshot group and terminal eight-rank restart siblings are retained; sampled-history forcing-work relative residual is `2.3508932051548193e-12`; accepted corrected-E03 prefix totals `72239` seconds (`20.066388888888888` exact and `20.066390` displayed node-hours); hardened reconciliation closes with `24/24/24` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. The reviewed estimator uses prior selected `2560`, previous observed `2345`, observed `2285`, local acceleration `2285`, and selects `2560 <= 2700` seconds for `s24`; `s23` leaves `415` seconds of threshold headroom. Authorization-facing projection is `829.2566672222222` matrix node-hours with `70.74333277777782` margin. The companion normally published canonical-only bytes with record type `observed-publication`, same-directory `link/fsync/unlink/fsync`, a mode-`0444` forensic copy, empty recost transaction directory, empty queue, and free strict lock. | Commit, push, archive, and catalog the post-F-107 documentation state, validate the full archive checksum ledger, and require completion of the durable handoff checklist items 19--21 before preparing only `R02/s24_rankio_t8p5_t8p75` on one node from authenticated `s23` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. Do not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any further extension. Keep R17 last. |
| F-108 | 2026-06-03T11:24:40+00:00 | Accept corrected E03 R02 quarter-unit continuation through exact `t = 8.75` and normally publish the next hardened retained-companion recost checkpoint | F-107 authorized only `s24`. Its clean endpoint still requires a new one-segment recost rather than automatic threshold ratcheting. | accepted `R02/s24_rankio_t8p5_t8p75`; job `4758576`; exact scheduler row `4758576\|cgl_mks24_E03_forcing_policy_R02_s24_rankio_t8p5_t8p75\|COMPLETED\|0:0\|1\|2346\|2026-06-03T06:00:56\|2026-06-03T06:40:27`; source launch bundle `athenak-feature-cgl-through-6d1f032a2.bundle` SHA-256 `62587378e8480cbe94caa174734f6d3cf6e4c56c2755b2b96ffe0e2bba525ef4`; submitted-state manifest SHA-256 `68e51623df49fa4612be49d578790023031bfbbb06b41a9df910e3737a1fc2ff`; record-time inspection SHA-256 `24a7013abca332a65196552f0ce1a6031314deb56e8ddbebb130b75be4b887de`; recorded manifest SHA-256 `24f407dcc74011479f783ccb16d08536ce87a35314d1637f93b78135adb27ed8`; retained independent validation SHA-256 `0a271b1f53f3363c2598a76356a7ca3063a38e5d8c0ab63a64e15b4ebcfa2229`; retained scheduler evidence SHA-256 `06b86448d2c4eb8c3765d9c3912ee1cb9d946f0d07ad606dfae6456ab1075f7f`; retained recost generator SHA-256 `c488c4d6f51a2d73e00e212f0e485d920876eb2ed79e8e18988788969e344d05`; canonical recost artifact SHA-256 `dbd3f5e3b9f21532da546caca4bdc66263d69c3b60b35938972edd04b2ab3e19`; observed-publication audit SHA-256 `503dad9d617456a3154f77931726b8d1edf3ccdc1602802e0db553670d0ca79c`; forensic copy with the canonical artifact SHA-256 | High | Continue from authenticated E03 `t = 8.5` restart siblings through exact `t = 8.75`; require formal and retained independent quarter inspection, exactly one eight-rank snapshot group, complete terminal restart siblings, zero strict LF failure counters, finite synchronized histories, forcing-work closure, live scheduler accounting, hardened reconciliation, retained-generator review, staged recost verification, independent staged review, normal companion promotion, and separate promoted verification. | Job `4758576` reaches exact `t = 8.75` in `2346` seconds (`0.651667` displayed node-hours). Strict LF failure counters remain zero; terminal `lf_hwproj = 299627446834`; one eight-rank snapshot group and terminal eight-rank restart siblings are retained; sampled-history forcing-work relative residual is `1.4737883591844796e-12`; accepted corrected-E03 prefix totals `74585` seconds (`20.718055555555555` exact and `20.718057` displayed node-hours); hardened reconciliation closes with `25/25/25` ledger rows/manifests/reservations, no active reservation, no transaction, and `issues = []`. The reviewed estimator uses prior selected `2560`, previous observed `2285`, observed `2346`, local acceleration `2407`, and selects `2560 <= 2700` seconds for `s25`; `s24` leaves `354` seconds of threshold headroom. Authorization-facing projection is `829.2566672222222` matrix node-hours with `70.74333277777782` margin. The companion normally published canonical-only bytes with record type `observed-publication`, same-directory `link/fsync/unlink/fsync`, a mode-`0444` forensic copy, empty recost transaction directory, empty queue, and free strict lock. | Commit, push, archive, and catalog the post-F-108 documentation state, validate the full archive checksum ledger, and require completion of the durable handoff checklist items 22--24 before preparing only `R02/s25_rankio_t8p75_t9` on one node from authenticated `s24` terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and a reviewed one-segment `2700`-second threshold retaining `600` seconds on both timeout margins. Do not ratchet automatically. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any further extension. Keep R17 last. |
...
authorize preparation after accepted job `4758713`; the documentation
checkpoint archive, fresh queue/free-lock, authenticated hardened
...
| 2026-06-03T09:28:03+00:00 | Accept corrected E03 R02 quarter-unit continuation job `4758475` through exact `t = 8.5` and conditionally authorize only the reviewed quarter-unit continuation through `t = 8.75` after the post-F-107 documentation and archive gate. | Job `4758475` passes formal and retained independent inspection, records `0.634722` displayed node-hours, and brings corrected E03 use to `20.066390` displayed node-hours. Canonical F-107 recost evidence SHA-256 `1e4db459d7c0b5211108dee07fe1efa643ac7a655adadb907deb57f36822d699` projects at most `829.2566672222222` matrix node-hours inside the `900.000000` envelope. The accepted `2285`-second quarter unit leaves `415` seconds of threshold headroom; the reviewed `s24` estimator selects `2560 <= 2700` seconds. The `s24` profile uses Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and threshold `2700` seconds, retaining `600` seconds on both timeout margins. It is scoped only to `s24` and cannot ratchet automatically. | Commit, push, archive, and catalog the post-F-107 documentation state, validate the archive checksum ledger, and require completion of the durable handoff checklist items 19--21 before preparing only `R02/s24_rankio_t8p5_t8p75` on one node from authenticated `s23` terminal siblings under the full F-107 profile. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any extension. Finish R02, execute R03--R16 sequentially, and execute R17 last. |
| 2026-06-03T07:36:16+00:00 | Accept corrected E03 R02 quarter-unit continuation job `4758398` through exact `t = 8.25` and conditionally authorize only the reviewed quarter-unit continuation through `t = 8.5` after the post-F-106 documentation and archive gate. | Job `4758398` passes formal and retained independent inspection, records `0.651389` displayed node-hours, and brings corrected E03 use to `19.431668` displayed node-hours. Canonical F-106 recost evidence SHA-256 `fe78232b5396d9d3ee6a5ef0e39f7aa445cf2c17125691a7d3e63d4a67978bc2` projects at most `829.2566672222222` matrix node-hours inside the `900.000000` envelope. The accepted `2345`-second quarter unit leaves `355` seconds of threshold headroom; the reviewed `s23` estimator selects `2560 <= 2700` seconds. The `s23` profile uses Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and threshold `2700` seconds, retaining `600` seconds on both timeout margins. It is scoped only to `s23` and cannot ratchet automatically. | Commit, push, archive, and catalog the post-F-106 documentation state, validate the archive checksum ledger, and require completion of the durable handoff checklist items 15--17 before preparing only `R02/s23_rankio_t8p25_t8p5` on one node from authenticated `s22` terminal siblings under the full F-106 profile. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any extension. Finish R02, execute R03--R16 sequentially, and execute R17 last. |
| 2026-06-03T05:27:00+00:00 | Accept corrected E03 R02 quarter-unit continuation job `4758233` through exact `t = 8.0` and conditionally authorize only the reviewed quarter-unit continuation through `t = 8.25` after the post-F-105 documentation and archive gate. | Job `4758233` passes formal and retained independent inspection, records `0.658056` displayed node-hours, and brings corrected E03 use to `18.780279` displayed node-hours. Canonical F-105 recost evidence SHA-256 `242b4ae79b215690d2b552aba9e583cf209d70c822f8dd76da4e7af1b7ba6a5b` projects at most `829.2566672222222` matrix node-hours inside the `900.000000` envelope. The accepted `2369`-second quarter unit leaves `331` seconds of threshold headroom; the reviewed `s22` estimator selects `2560 <= 2700` seconds. The `s22` profile uses Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and threshold `2700` seconds, retaining `600` seconds on both timeout margins. It is scoped only to `s22` and cannot ratchet automatically. | Commit, push, archive, and catalog the post-F-105 documentation state, validate the archive checksum ledger, and require completion of the durable handoff checklist items 12--14 before preparing only `R02/s22_rankio_t8_t8p25` on one node from authenticated `s21` terminal siblings under the full F-105 profile. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any extension. Finish R02, execute R03--R16 sequentially, and execute R17 last. |
| 2026-06-03T03:21:44+00:00 | Accept corrected E03 R02 quarter-unit continuation job `4757761` through exact `t = 7.75` and conditionally authorize only the reviewed quarter-unit continuation through `t = 8.0` after the post-F-104 documentation and archive gate. | Job `4757761` passes formal and retained independent inspection, records `0.658333` displayed node-hours, and brings corrected E03 use to `18.122223` displayed node-hours. Canonical F-104 recost evidence SHA-256 `f8d05a4483a902f21f49c9ebb8935dba3e21d4186c40eda050ae57d169f6bb41` projects at most `829.2566672222222` matrix node-hours inside the `900.000000` envelope. The accepted `2370`-second quarter unit leaves `330` seconds of threshold headroom; the reviewed `s21` estimator selects `2560 <= 2700` seconds. The `s21` profile uses Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and threshold `2700` seconds, retaining `600` seconds on both timeout margins. It is scoped only to `s21` and cannot ratchet automatically. | Commit, push, archive, and catalog the post-F-104 documentation state, validate the archive checksum ledger, and require completion of the durable handoff checklist items 9--11 before preparing only `R02/s21_rankio_t7p75_t8` on one node from authenticated `s20` terminal siblings under the full F-104 profile. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any extension. Finish R02, execute R03--R16 sequentially, and execute R17 last. |
| 2026-06-03T01:28:52+00:00 | Accept corrected E03 R02 quarter-unit continuation job `4757300` through exact `t = 7.5` and conditionally authorize only the reviewed quarter-unit continuation through `t = 7.75` after the post-F-103 documentation and archive gate. | Job `4757300` passes formal and retained independent inspection, records `0.680556` displayed node-hours, and brings corrected E03 use to `17.463890` displayed node-hours. Canonical F-103 recost evidence SHA-256 `ab7fa80746fa4d2c65f515b3ace71d5668586e27fde91bbdeb578fc3a4509d98` projects at most `829.2566672222222` matrix node-hours inside the `900.000000` envelope. The accepted `2450`-second quarter unit leaves `250` seconds of threshold headroom; the reviewed `s20` estimator selects `2560 <= 2700` seconds. The `s20` profile uses Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and threshold `2700` seconds, retaining `600` seconds on both timeout margins. It is scoped only to `s20` and cannot ratchet automatically. | Commit, push, archive, and catalog the post-F-103 documentation state, validate the archive checksum ledger, and require completion of the durable handoff checklist items 6--8 before preparing only `R02/s20_rankio_t7p5_t7p75` on one node from authenticated `s19` terminal siblings under the full F-103 profile. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any extension. Finish R02, execute R03--R16 sequentially, and execute R17 last. |
| 2026-06-02T03:10:59+00:00 | Accept corrected E03 R02 quarter-unit continuation job `4754008` through exact `t = 7.0` and narrowly and conditionally authorize only the reviewed quarter-unit continuation through `t = 7.25` after the post-F-100 archive gate. | Job `4754008` passes formal and retained independent inspection, records `0.667222` displayed node-hours, and brings corrected E03 use to `16.133334` displayed node-hours. Retained corrected F-100 evidence SHA-256 `63438d8843b86815a2e25d4d6de6ef78ce9756c561fe9db3e762965ada73202f` projects at most `829.101112` displayed matrix node-hours inside the `900.000000` envelope. The accepted `2402`-second quarter unit leaves `298` seconds of threshold headroom; the reviewed `s18` estimator selects `2559 <= 2700` seconds. The `s18` profile uses Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and threshold `2700` seconds, retaining `600` seconds on both timeout margins. It is scoped only to `s18` and cannot ratchet automatically. | Commit the post-F-100 documentation state, archive and catalog its resulting actual controller bundle and its SHA-256, rerun reconciliation, repeat the queue/shared-root audit with explicit acknowledgement of the stale beta-25 record, then prepare only `R02/s18_rankio_t7_t7p25` on one node from authenticated `s17` terminal siblings under the full F-100 profile. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any extension. Finish R02, execute R03--R16 sequentially, and execute R17 last. |
...
| 2026-05-31 | Accept corrected E03 R02 quarter-unit continuation job `4747500` through exact `t = 5.75` and authorize only the reviewed quarter-unit continuation through `t = 6.0`. | Job `4747500` passes formal and retained independent inspection, records `0.660000` node-hours, and brings corrected E03 use to `12.910279`. Retained corrected F-095 evidence SHA-256 `d0a60e222138971e1bc9aae978bb3d62aee460f09000ee62f7f9ffdfe534165f` projects at most `814.945556` matrix node-hours inside the `900.000000` envelope. The accepted `2376`-second quarter unit leaves `324` seconds of threshold headroom; the reviewed `s13` estimator selects `2468 <= 2700` seconds. The `s13` profile uses Slurm walltime `01:05:00`, Athena timeout `00:55:00`, and threshold `2700` seconds, retaining `600` seconds on both timeout margins. It is scoped only to `s13` and cannot ratchet automatically. | Archive and catalog the current committed controller state, reconcile, then prepare only `R02/s13_rankio_t5p75_t6` on one node from authenticated `s12` terminal siblings under the full F-095 profile. Any scientific, provenance, scheduler, storage, budget, or reconciliation failure blocks successor preparation. Inspect, account, reconcile, and recost before any extension. Finish R02, execute R03--R16 sequentially, and execute R17 last. |
...
Authenticated `R02/s12_rankio_t5p5_t5p75` job `4747500` is accepted through
exact `t = 5.75` for `0.660000` node-hours, bringing corrected E03 use to
`12.910279`. Retained corrected F-095 recost evidence JSON SHA-256 is
`d0a60e222138971e1bc9aae978bb3d62aee460f09000ee62f7f9ffdfe534165f`.
...
on both timeout margins. The threshold is scoped only to `s13` and cannot
ratchet automatically. Any scientific, provenance, scheduler, storage,
budget, or reconciliation failure blocks successor preparation. Inspect,
account, reconcile, and recost before any further extension.
Authenticated `R02/s13_rankio_t5p75_t6` job `4747834` is accepted through
exact `t = 6.0` for `0.599722` node-hours, bringing corrected E03 use to
`13.510001`. Retained corrected F-096 recost evidence JSON SHA-256 is
`4bc19f5b44587e73869982425b1cf0ad459547c3e0c5de1ab1b2b5cb366c6322`.
...
terminal siblings with Slurm walltime `01:05:00`, Athena timeout `00:55:00`,
and its reviewed one-segment `2700`-second threshold retaining `600` seconds
on both timeout margins. The threshold is scoped only to `s14` and cannot
ratchet automatically. Any scientific, provenance, scheduler, storage,
budget, or reconciliation failure blocks successor preparation. Inspect,
account, reconcile, and recost before any further extension.
Authenticated `R02/s14_rankio_t6_t6p25` job `4748138` is accepted through
exact `t = 6.25` for `0.655278` node-hours, bringing corrected E03 use to
`14.165279`. Retained corrected F-097 recost evidence JSON SHA-256 is
`5ba7aec52d3e7824cf4ccf52f95ce1e46368c84ed59f0525e2bbbcfc167fde91`.
...
`2700`-second threshold retaining `600` seconds on both timeout margins. The
threshold was scoped only to `s15` and could not ratchet automatically. Any
scientific, provenance, scheduler, storage, budget, or reconciliation failure
...
exact `t = 6.5` for `0.653611` node-hours, bringing corrected E03 use to
...
issue. Historical authorization was only `R02/s17_rankio_t6p75_t7` on one
node from the authenticated `s16` terminal siblings under the full F-099
profile. Authenticated `R02/s17_rankio_t6p75_t7` job `4754008` is accepted
through exact `t = 7.0` for `0.667222` displayed
node-hours, bringing corrected E03 use to `16.133334`
displayed node-hours. Retained corrected F-100 recost evidence JSON SHA-256 is
`63438d8843b86815a2e25d4d6de6ef78ce9756c561fe9db3e762965ada73202f`.
Hardened reconciliation closes with `18/18/18` ledger
rows/manifests/reservations, no active reservation, no transaction, and
`issues = []`. Historical F-100 authorization was only
`R02/s18_rankio_t7_t7p25`. Authenticated `s18` job `4754394` is accepted
through exact `t = 7.25` for `0.650000` node-hours. Historical F-101 recost
evidence SHA-256
`df208829aec85c88fcc2caa757a21ad3ff3372aeb82628cbfe7bd41851faed9a`
was truthfully adopted under the retained companion and historically authorized
only `R02/s19_rankio_t7p25_t7p5`. F-102 hardened the Stage I helper before any
`s19` mutation. Authenticated `s19` job `4757300` is accepted through exact
`t = 7.5` for `0.680556` displayed node-hours, bringing corrected-E03 use to
`17.463890` displayed node-hours. Canonical F-103 recost evidence SHA-256
`ab7fa80746fa4d2c65f515b3ace71d5668586e27fde91bbdeb578fc3a4509d98`
is the first normal observed publication under the retained companion and
historically authorized only `R02/s20_rankio_t7p5_t7p75`. Authenticated `s20`
job `4757761` is accepted through exact `t = 7.75` for `0.658333` displayed
node-hours, bringing corrected-E03 use to `18.122223` displayed node-hours.
Canonical F-104 recost evidence SHA-256 is
`f8d05a4483a902f21f49c9ebb8935dba3e21d4186c40eda050ae57d169f6bb41`.
Authenticated `s21` job `4758233` is accepted through exact `t = 8.0` for
`0.658056` displayed node-hours, bringing corrected-E03 use to `18.780279`
displayed node-hours. Canonical F-105 recost evidence SHA-256 is
`242b4ae79b215690d2b552aba9e583cf209d70c822f8dd76da4e7af1b7ba6a5b`.
Authenticated `s22` job `4758398` is accepted through exact `t = 8.25` for
`0.651389` displayed node-hours, bringing corrected-E03 use to `19.431668`
displayed node-hours. Canonical F-106 recost evidence SHA-256 is
`fe78232b5396d9d3ee6a5ef0e39f7aa445cf2c17125691a7d3e63d4a67978bc2`.
Authenticated `s23` job `4758475` is accepted through exact `t = 8.5` for
`0.634722` displayed node-hours, bringing corrected-E03 use to `20.066390`
displayed node-hours. Canonical F-107 recost evidence SHA-256 is
`1e4db459d7c0b5211108dee07fe1efa643ac7a655adadb907deb57f36822d699`.
Authenticated `s24` job `4758576` is accepted through exact `t = 8.75` for
`0.651667` displayed node-hours, bringing corrected-E03 use to `20.718057`
displayed node-hours. Canonical F-108 recost evidence SHA-256 is
`dbd3f5e3b9f21532da546caca4bdc66263d69c3b60b35938972edd04b2ab3e19`.
Authenticated `s25` job `4758713` is accepted through exact `t = 9` for
`0.627500` displayed node-hours, bringing corrected-E03 use to `21.345557`
displayed node-hours. Canonical F-109 recost evidence SHA-256 is
`67f9096900b5ec3ce8368cd39c6f6a8c649dcff2436261fcf72aa328cd3ac0ea`.
Hardened reconciliation closes with `26/26/26` ledger
rows/manifests/reservations, no active reservation, no transaction, and
`issues = []`. Commit, push, archive, and catalog the post-F-109 documentation
checkpoint, validate the archive ledger, rerun hardened reconciliation, and
repeat the queue/shared-root audit with explicit acknowledgement of the stale
beta-25 record. Require completion of the durable handoff checklist items 25--27
before preparation. Then prepare only `R02/s26_rankio_t9_t9p25` on one node from
authenticated `s25` terminal siblings with Slurm walltime `01:05:00`, Athena
timeout `00:55:00`, and its reviewed one-segment `2700`-second threshold
retaining `600` seconds on both timeout margins. The threshold is scoped only
to `s26` and cannot ratchet automatically. Any scientific, provenance,
scheduler, storage, budget, or reconciliation failure blocks successor
preparation. Inspect, account, reconcile, and recost before any further
extension.
...
After accepted `s12`, retain quarter units and recost the immediately following
quarter only. Let `E12` be the top-level Slurm allocation `ElapsedRaw` in
integer seconds. The reviewed `s13` estimator is:

```text
prior_half_derived_quarter = 2361
normalized_prior_quarter = 4568 // 2 = 2284
local_next_quarter = max(E12, 2 * E12 - normalized_prior_quarter)
selected_next_quarter = max(prior_half_derived_quarter, local_next_quarter)
```

For accepted `s12`, `E12 = 2376`, so `local_next_quarter = 2468` and
`selected_next_quarter = 2468 <= 2700`. Retained F-095 evidence therefore
historically authorized only `R02/s13_rankio_t5p75_t6`. This is a reviewed one-segment
authorization, not an automatic quarter-unit ratchet.

After accepted `s13`, retain quarter units and recost the immediately following
quarter only. Let `E13` be the top-level Slurm allocation `ElapsedRaw` in
integer seconds. The reviewed `s14` estimator is:

```text
prior_selected_next_quarter = 2468
previous_observed_quarter = 2376
local_next_quarter = max(E13, 2 * E13 - previous_observed_quarter)
selected_next_quarter = max(prior_selected_next_quarter, local_next_quarter)
```

For accepted `s13`, `E13 = 2159`, so `local_next_quarter = 2159` and
`selected_next_quarter = 2468 <= 2700`. Retained F-096 evidence therefore
historically authorized only `R02/s14_rankio_t6_t6p25`. This is a reviewed one-segment
authorization, not an automatic quarter-unit ratchet.

After accepted `s14`, retain quarter units and recost the immediately following
quarter only. Let `E14` be the top-level Slurm allocation `ElapsedRaw` in
integer seconds. The reviewed `s15` estimator is:

```text
prior_selected_next_quarter = 2468
previous_observed_quarter = 2159
local_next_quarter = max(E14, 2 * E14 - previous_observed_quarter)
selected_next_quarter = max(prior_selected_next_quarter, local_next_quarter)
```

For accepted `s14`, `E14 = 2359`, so `local_next_quarter = 2559` and
`selected_next_quarter = 2559 <= 2700`. Retained F-097 evidence therefore
historically authorized only `R02/s15_rankio_t6p25_t6p5`. This was a reviewed
one-segment authorization, not an automatic quarter-unit ratchet.

After accepted `s15`, retain quarter units and recost the immediately following
quarter only. Let `E15` be the top-level Slurm allocation `ElapsedRaw` in
integer seconds. The reviewed `s16` estimator is:

```text
prior_selected_next_quarter = 2559
previous_observed_quarter = 2359
local_next_quarter = max(E15, 2 * E15 - previous_observed_quarter)
selected_next_quarter = max(prior_selected_next_quarter, local_next_quarter)
```
...
`selected_next_quarter = 2559 <= 2700`. Retained F-099 evidence historically
authorized only `R02/s17_rankio_t6p75_t7` after the post-F-099 documentation
...
After accepted `s17`, retain the same conservative envelope for `s18` only.
Let `E17` be the top-level Slurm allocation `ElapsedRaw` in integer seconds.
The reviewed `s18` estimator is:

```text
prior_selected_next_quarter = 2559
previous_observed_quarter = 2330
local_next_quarter = max(E17, 2 * E17 - previous_observed_quarter)
selected_next_quarter = max(prior_selected_next_quarter, local_next_quarter)
```

For accepted `s17`, `E17 = 2402`, so
`local_next_quarter = 2474` and
`selected_next_quarter = 2559 <= 2700`.
Retained F-100 evidence narrowly and conditionally authorizes only
`R02/s18_rankio_t7_t7p25` after the post-F-100 documentation state is committed,
its resulting actual controller bundle and its SHA-256 are archived and
cataloged, reconciliation is rerun, and the queue/shared-root
audit repeats explicit acknowledgement of the stale beta-25 record. This is a
reviewed one-segment authorization, not an automatic quarter-unit ratchet.

After accepted `s19`, the same conservative envelope authorized `s20` only.
The reviewed F-103 estimator uses prior selected `2559`, previous observed
`2340`, accepted `s19` elapsed `2450`, and local acceleration `2560`; it selects
`2560 <= 2700` seconds. Canonical F-103 evidence conditionally authorizes only
`R02/s20_rankio_t7p5_t7p75` after the post-F-103 documentation checkpoint is
committed, pushed, archived, and cataloged, the full archive ledger is
validated, reconciliation is rerun, and the queue/shared-root audit repeats
explicit acknowledgement of the stale beta-25 record. Require completion of
the durable handoff checklist items 6--8 before preparation. This is a
reviewed one-segment authorization, not an automatic quarter-unit ratchet.

After accepted `s20`, retain the same conservative envelope for `s21` only.
The reviewed F-104 estimator uses prior selected `2560`, previous observed
`2450`, accepted `s20` elapsed `2370`, and local acceleration `2370`; it selects
`2560 <= 2700` seconds. Canonical F-104 evidence conditionally authorizes only
`R02/s21_rankio_t7p75_t8` after the post-F-104 documentation checkpoint is
committed, pushed, archived, and cataloged, the full archive ledger is
validated, reconciliation is rerun, and the queue/shared-root audit repeats
explicit acknowledgement of the stale beta-25 record. Require completion of
the durable handoff checklist items 9--11 before preparation. This is a
reviewed one-segment authorization, not an automatic quarter-unit ratchet.

After accepted `s21`, retain the same conservative envelope for `s22` only.
The reviewed F-105 estimator uses prior selected `2560`, accepted `s21`
elapsed `2369`, and local acceleration `2369`; it selects `2560 <= 2700`
seconds. Canonical F-105 evidence conditionally authorizes only
`R02/s22_rankio_t8_t8p25` after the post-F-105 documentation checkpoint is
committed, pushed, archived, and cataloged, the full archive ledger is
validated, reconciliation is rerun, and the queue/shared-root audit repeats
explicit acknowledgement of the stale beta-25 record. Require completion of
the durable handoff checklist items 12--14 before preparation. This is a
reviewed one-segment authorization, not an automatic quarter-unit ratchet.

After accepted `s22`, retain the same conservative envelope for `s23` only.
The reviewed F-106 estimator uses prior selected `2560`, previous observed
`2369`, accepted `s22` elapsed `2345`, and local acceleration `2345`; it selects
`2560 <= 2700` seconds. Canonical F-106 evidence conditionally authorizes only
`R02/s23_rankio_t8p25_t8p5` after the post-F-106 documentation checkpoint is
committed, pushed, archived, and cataloged, the full archive ledger is
validated, reconciliation is rerun, and the queue/shared-root audit repeats
explicit acknowledgement of the stale beta-25 record. Require completion of
the durable handoff checklist items 15--17 before preparation. This is a
reviewed one-segment authorization, not an automatic quarter-unit ratchet.

After accepted `s23`, retain the same conservative envelope for `s24` only.
The reviewed F-107 estimator uses prior selected `2560`, previous observed
`2345`, accepted `s23` elapsed `2285`, and local acceleration `2285`; it selects
`2560 <= 2700` seconds. Canonical F-107 evidence conditionally authorizes only
`R02/s24_rankio_t8p5_t8p75` after the post-F-107 documentation checkpoint is
committed, pushed, archived, and cataloged, the full archive ledger is
validated, reconciliation is rerun, and the queue/shared-root audit repeats
explicit acknowledgement of the stale beta-25 record. Require completion of
the durable handoff checklist items 19--21 before preparation. This is a
reviewed one-segment authorization, not an automatic quarter-unit ratchet.

After accepted `s24`, retain the same conservative envelope for `s25` only.
The reviewed F-108 estimator uses prior selected `2560`, previous observed
`2285`, accepted `s24` elapsed `2346`, and local acceleration `2407`; it selects
`2560 <= 2700` seconds. Canonical F-108 evidence conditionally authorizes only
`R02/s25_rankio_t8p75_t9` after the post-F-108 documentation checkpoint is
committed, pushed, archived, and cataloged, the full archive ledger is
validated, reconciliation is rerun, and the queue/shared-root audit repeats
explicit acknowledgement of the stale beta-25 record. Require completion of
the durable handoff checklist items 22--24 before preparation. This is a
reviewed one-segment authorization, not an automatic quarter-unit ratchet.

After accepted `s25`, retain the same conservative envelope for `s26` only.
The reviewed F-109 estimator uses prior selected `2560`, previous observed
`2346`, accepted `s25` elapsed `2259`, and local acceleration `2259`; it selects
`2560 <= 2700` seconds. Canonical F-109 evidence conditionally authorizes only
`R02/s26_rankio_t9_t9p25` after the post-F-109 documentation checkpoint is
committed, pushed, archived, and cataloged, the full archive ledger is
validated, reconciliation is rerun, and the queue/shared-root audit repeats
explicit acknowledgement of the stale beta-25 record. Require completion of
the durable handoff checklist items 25--27 before preparation. This is a
reviewed one-segment authorization, not an automatic quarter-unit ratchet.
...
authenticated `R02/s16_rankio_t6p5_t6p75` job `4753294` is accepted through
exact `t = 6.75`;
authenticated `R02/s17_rankio_t6p75_t7` job `4754008` is accepted through exact
`t = 7.0`;
authenticated `R02/s18_rankio_t7_t7p25` job `4754394` is accepted through exact
`t = 7.25`;
authenticated `R02/s19_rankio_t7p25_t7p5` job `4757300` is accepted through
exact `t = 7.5`;
authenticated `R02/s20_rankio_t7p5_t7p75` job `4757761` is accepted through
exact `t = 7.75`;
authenticated `R02/s21_rankio_t7p75_t8` job `4758233` is accepted through
exact `t = 8.0`;
authenticated `R02/s22_rankio_t8_t8p25` job `4758398` is accepted through
exact `t = 8.25`;
authenticated `R02/s23_rankio_t8p25_t8p5` job `4758475` is accepted through
exact `t = 8.5`;
authenticated `R02/s24_rankio_t8p5_t8p75` job `4758576` is accepted through
exact `t = 8.75`;
authenticated `R02/s25_rankio_t8p75_t9` job `4758713` is accepted through
exact `t = 9`;
commit, push, archive, and catalog the post-F-109 documentation checkpoint,
...
record, and require completion of the durable handoff checklist items 25--27
before continuing only with `R02/s26_rankio_t9_t9p25` on one node from its
authenticated `s25` restart siblings under the full F-109 profile: Slurm
walltime `01:05:00`, Athena timeout `00:55:00`, and a one-segment
`2700`-second threshold retaining `600` seconds on both timeout margins. The
threshold is scoped only to `s26` and cannot ratchet automatically. Any
scientific, provenance, scheduler, storage, budget, or reconciliation failure
blocks successor preparation. Inspect, account, reconcile, and recost before
any further extension.
...
queue/shared-root audit with explicit acknowledgement of the stale beta-25
record, and require completion of the durable handoff checklist items 25--27
before preparing only `R02/s26_rankio_t9_t9p25` on one node from the
authenticated `s25` terminal siblings under the full F-109 profile: Slurm
walltime `01:05:00`, Athena timeout `00:55:00`, and a one-segment
`2700`-second threshold retaining `600` seconds on both timeout margins. The
threshold is scoped only to `s26` and cannot ratchet automatically. Any
scientific, provenance, scheduler, storage, budget, or reconciliation failure
...
2. **Preserve the accepted fresh E03 R02 pilot from `t = 0`.**
   ...
   `323d7cb3cbe108eee944045c189cff75ffc07d06833c49bcfa8315f07fad8d51`.
   Authenticated `R02/s12_rankio_t5p5_t5p75` job `4747500` passes formal and
   retained independent inspection through exact `t = 5.75`, records
   `0.660000` node-hours, and leaves corrected E03 cumulative use at
   `12.910279`. Retained corrected F-095 recost evidence JSON SHA-256 is
   `d0a60e222138971e1bc9aae978bb3d62aee460f09000ee62f7f9ffdfe534165f`.
   Authenticated `R02/s13_rankio_t5p75_t6` job `4747834` passes formal and
   retained independent inspection through exact `t = 6.0`, records
   `0.599722` node-hours, and leaves corrected E03 cumulative use at
   `13.510001`. Retained corrected F-096 recost evidence JSON SHA-256 is
   `4bc19f5b44587e73869982425b1cf0ad459547c3e0c5de1ab1b2b5cb366c6322`.
   Authenticated `R02/s14_rankio_t6_t6p25` job `4748138` passes formal and
   retained independent inspection through exact `t = 6.25`, records
   `0.655278` node-hours, and leaves corrected E03 cumulative use at
   `14.165279`. Retained corrected F-097 recost evidence JSON SHA-256 is
   `5ba7aec52d3e7824cf4ccf52f95ce1e46368c84ed59f0525e2bbbcfc167fde91`.
   Authenticated `R02/s15_rankio_t6p25_t6p5` job `4752118` passes formal and
   retained independent inspection through exact `t = 6.5`, records
   ...
   Authenticated `R02/s17_rankio_t6p75_t7` job `4754008` passes formal and
   retained independent inspection through exact `t = 7.0`, records
   `0.667222` displayed node-hours, and leaves corrected
   E03 cumulative use at `16.133334` displayed
   node-hours. Retained corrected F-100 recost evidence JSON SHA-256 is
   `63438d8843b86815a2e25d4d6de6ef78ce9756c561fe9db3e762965ada73202f`.
   Authenticated `R02/s18_rankio_t7_t7p25` job `4754394` reaches exact
   `t = 7.25`; historical F-101 was truthfully adopted under the retained
   companion. Authenticated `R02/s19_rankio_t7p25_t7p5` job `4757300` passes
   formal and independently regenerated inspection through exact `t = 7.5`,
   records `0.680556` displayed node-hours, and leaves corrected-E03 cumulative
   use at `17.463890` displayed node-hours. Canonical F-103 recost evidence
   SHA-256 is
   `ab7fa80746fa4d2c65f515b3ace71d5668586e27fde91bbdeb578fc3a4509d98`.
   Authenticated `R02/s20_rankio_t7p5_t7p75` job `4757761` passes formal and
   independently regenerated inspection through exact `t = 7.75`, records
   `0.658333` displayed node-hours, and leaves corrected-E03 cumulative use at
   `18.122223` displayed node-hours. Canonical F-104 recost evidence SHA-256 is
   `f8d05a4483a902f21f49c9ebb8935dba3e21d4186c40eda050ae57d169f6bb41`.
   Authenticated `R02/s21_rankio_t7p75_t8` job `4758233` passes formal and
   independently regenerated inspection through exact `t = 8.0`, records
   `0.658056` displayed node-hours, and leaves corrected-E03 cumulative use at
   `18.780279` displayed node-hours. Canonical F-105 recost evidence SHA-256 is
   `242b4ae79b215690d2b552aba9e583cf209d70c822f8dd76da4e7af1b7ba6a5b`.
   Authenticated `R02/s22_rankio_t8_t8p25` job `4758398` passes formal and
   independently regenerated inspection through exact `t = 8.25`, records
   `0.651389` displayed node-hours, and leaves corrected-E03 cumulative use at
   `19.431668` displayed node-hours. Canonical F-106 recost evidence SHA-256 is
   `fe78232b5396d9d3ee6a5ef0e39f7aa445cf2c17125691a7d3e63d4a67978bc2`.
   Authenticated `R02/s23_rankio_t8p25_t8p5` job `4758475` passes formal and
   independently regenerated inspection through exact `t = 8.5`, records
   `0.634722` displayed node-hours, and leaves corrected-E03 cumulative use at
   `20.066390` displayed node-hours. Canonical F-107 recost evidence SHA-256 is
   `1e4db459d7c0b5211108dee07fe1efa643ac7a655adadb907deb57f36822d699`.
   Authenticated `R02/s24_rankio_t8p5_t8p75` job `4758576` passes formal and
   independently regenerated inspection through exact `t = 8.75`, records
   `0.651667` displayed node-hours, and leaves corrected-E03 cumulative use at
   `20.718057` displayed node-hours. Canonical F-108 recost evidence SHA-256 is
   `dbd3f5e3b9f21532da546caca4bdc66263d69c3b60b35938972edd04b2ab3e19`.
   Authenticated `R02/s25_rankio_t8p75_t9` job `4758713` passes formal and
   independently regenerated inspection through exact `t = 9`, records
   `0.627500` displayed node-hours, and leaves corrected-E03 cumulative use at
   `21.345557` displayed node-hours. Canonical F-109 recost evidence SHA-256 is
   `67f9096900b5ec3ce8368cd39c6f6a8c649dcff2436261fcf72aa328cd3ac0ea`.
   Commit, push, archive, and catalog the post-F-109 documentation checkpoint,
   validate the archive ledger, rerun hardened reconciliation, repeat the
   queue/shared-root audit with explicit acknowledgement of the stale beta-25
   ...
   authenticated `s25` terminal siblings with Slurm walltime `01:05:00`, Athena
   timeout `00:55:00`, and its reviewed one-segment `2700`-second threshold
   retaining `600` seconds on both timeout margins. The threshold is scoped
   only to `s26` and cannot ratchet automatically.
   Any scientific, provenance, scheduler, storage, budget, or reconciliation
