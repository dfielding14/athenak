...
authorizes only `R02/s05_rankio_t2_t2p5` next.
...
| Production ready for supported CGL-LF use | Not established | Corrected E03 debug qualification, retained token, token-gated accounting tooling, committed epoch isolation and shared-root controls, immutable corrected build archival, retained focused serial/MPI CPU gates, historical E02 pipeline/cost evidence, accepted fresh E03 R02 jobs `4745922`, `4746154`, `4746182`, `4746356`, and `4746435` through exact `t = 2.0`, and archived F-079--F-084 controller provenance plus hardened clean reconciliation exist | Archive and catalog the current committed controller state, reconcile, prepare only the authenticated R02 continuation through `t = 2.5`, inspect, account, reconcile, and recost; then execute representative mapped production and complete production/statistical qualification |
...
prepare only `R02/s05_rankio_t2_t2p5`. Inspect, account, reconcile, and
...
exact `t = 2.0`;
archive and catalog the current committed controller state, reconcile, then
continue only with `R02/s05_rankio_t2_t2p5` from its authenticated restart
siblings. Inspect, account, reconcile, and recost before any further
extension.
...
accepted through exact `t = 2.0`. Archive and catalog the current committed
controller state, reconcile, then prepare only `R02/s05_rankio_t2_t2p5`
next. Inspect, account,
...
2. **Preserve the accepted fresh E03 R02 pilot from `t = 0`.**
   ...
   then prepare only `R02/s05_rankio_t2_t2p5`. Inspect, account, reconcile,
