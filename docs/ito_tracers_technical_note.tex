...
cell-centered mean displacements and central covariance tensors from the same
final Runge--Kutta-weighted mass fluxes used by the Monte Carlo tracer,
communicates those quantities across MeshBlocks and refinement boundaries,
interpolates them to each particle, and advances particles with a correlated
stochastic step.

The evidence supports a precise conclusion: the implemented \Ito{}-2 particles
match the full first and central second displacement moments of the finite-step
Monte Carlo kernel in controlled two- and three-dimensional tests, preserve
continuous subcell positions through AMR motion, and provide useful
...
\begin{enumerate}
  ...
  \item preserve the Monte Carlo process's vector mean and full finite-step
        covariance while producing continuous particle paths; and
...
displacement chosen to have the same vector mean and covariance.
...
cross moment vanishes. AthenaK factors
\(\mathbf Q=\mathbf L\mathbf L^{\mathsf T}\) and advances
\begin{equation}
...
variables with zero mean and unit variance. The resulting physical coordinate
kicks are correlated by \(\mathbf L\).
...
& Continuous paths with the same vector mean and finite-step covariance as the
...
\begin{enumerate}
  ...
  \item After the fluid integrator, each cell converts its outward mass-flux
        probabilities into the finite-step mean \(\mathbf m\) and central
        covariance \(\mathbf Q\). Invalid probabilities, non-positive-
        semidefinite covariance beyond tolerance, and non-finite values
        terminate the run.
  ...
  \item A pivoted semidefinite factorization computes
...
& Two- and three-dimensional covariance tests include mixed signs,
...
\begin{itemize}
  ...
  \item Controlled tests support full finite-step vector-mean and covariance
        matching in two and three dimensions, plus continuous-path behavior.
