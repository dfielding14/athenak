...
Run from the repository root:
  ...
  python docs/scripts/generate_selfgravity_validation.py --build-cpu --with-mpi --build-mpi
...
def build(extra_flags: list[str] | None = None) -> None:
  ...
  run_command(["cmake", "-S", str(REPO_ROOT), "-B", str(TST_DIR / "build")] + flags,
              REPO_ROOT)
  run_command(["cmake", "--build", str(TST_DIR / "build"), "-j", str(os.cpu_count() or 4)],
              REPO_ROOT)
...
@in_build_dir
def expect_failure(input_file: str | Path, extra_args: list[str] | None, expected: str) -> str:
...
def add_image(ax, data, x, y, title, cmap="viridis", symmetric=False):
  ...
  if symmetric and finite.size:
    ...
    vmin = -vmax
  mesh = ax.pcolormesh(centers_to_edges(x), centers_to_edges(y), data,
...
def periodic_jeans(metrics: dict[str, Any]) -> str:
  ...
  try:
    run_athena("inputs/tests/selfgravity.athinput", basename, expect_jeans_banner=True)
    ...
    metrics["periodic_jeans"] = {"relative_l2": rel_l2}
...
def hydro_mhd_equivalence(metrics: dict[str, Any]) -> str:
  ...
  try:
    ...
    h_grid, x, y, z = sg.uniform_grid_field(parse_latest_binary(hydro, "grav_phi"), "grav_phi")
    m_grid, _, _, _ = sg.uniform_grid_field(parse_latest_binary(mhd, "grav_phi"), "grav_phi")
    diff = h_grid - m_grid
    metrics["hydro_mhd_equivalence"] = {"max_abs_phi_difference": float(np.max(np.abs(diff)))}
...
def source_oracle(metrics: dict[str, Any]) -> str:
  ...
  try:
    run_athena("inputs/tests/selfgravity_oracle.athinput", on,
               expect_jeans_banner=True, max_final_defect=2.0e-8)
    run_athena("inputs/tests/selfgravity_oracle.athinput", off,
    ...
    accel = (four_pi_g * 1.0e-4 / k_wave) * np.cos(k_wave * sg.jeans_phase(xgrid, ygrid, zgrid))
    ...
    fig, axes = plt.subplots(1, 3, figsize=(12, 3.8))
    add_image(axes[0], actual_fields[0][mid], x, y, "Actual mom1 increment", "viridis")
    add_image(axes[1], expected[0][mid], x, y, "Analytic mom1 increment", "viridis")
    add_image(axes[2], residual[mid], x, y, f"Residual; rel. L2 = {rel['mom1']:.3e}",
              "RdBu_r", True)
...
def binary_multipole(metrics: dict[str, Any]) -> str:
  ...
  try:
    run_athena("inputs/tests/selfgravity_binary.athinput", basename,
    ...
    analytic = (uniform_sphere_potential(xgrid, ygrid, zgrid, center1, 1.0, radius, gconst) +
    ...
    ax1a, ax2a, ax3a = point_mass_acceleration(xgrid, ygrid, zgrid, center1, 1.0, gconst)
    bx1a, bx2a, bx3a = point_mass_acceleration(xgrid, ygrid, zgrid, center2, 0.5, gconst)
    ...
    mask[:1, :, :] = False
    mask[-1:, :, :] = False
    force_cosine = float(np.sum(num[:, mask] * ref[:, mask]) /
                         (np.linalg.norm(num[:, mask]) * np.linalg.norm(ref[:, mask])))
    metrics["binary_multipole"] = {"potential_relative_l2": rel_l2,
                                   "force_cosine": force_cosine}

    mid = len(z) // 2
    fig, axes = plt.subplots(2, 2, figsize=(10, 8))
    add_image(axes[0, 0], phi[mid], x, y, "Numerical phi", "viridis")
    add_image(axes[0, 1], analytic[mid], x, y, "Uniform-sphere analytic phi", "viridis")
    add_image(axes[1, 0], (phi - analytic)[mid], x, y, "Potential residual", "RdBu_r", True)
    dist = np.minimum(r1[far], r2[far])
    rel_err = np.abs(phi[far] - analytic[far]) / np.maximum(np.abs(analytic[far]), 1.0e-300)
...
def restart_equivalence(metrics: dict[str, Any]) -> str:
  ...
  try:
    run_athena("inputs/tests/selfgravity_oracle.athinput", straight,
               extra_args=["time/nlim=2"], expect_jeans_banner=True,
               max_final_defect=2.0e-8)
    run_athena("inputs/tests/selfgravity_oracle.athinput", split,
               extra_args=["time/nlim=1"], expect_jeans_banner=True,
               max_final_defect=2.0e-8)
    ...
    mid = len(z) // 2
    fig, axes = plt.subplots(1, 3, figsize=(12, 3.8))
    add_image(axes[0], diff_grids["dens"][mid], x, y, "density difference", "RdBu_r", True)
    add_image(axes[1], diff_grids["mom1"][mid], x, y, "mom1 difference", "RdBu_r", True)
...
def controls_profile(metrics: dict[str, Any]) -> str:
  ...
  try:
    threshold_out = run_athena("inputs/tests/selfgravity.athinput", threshold,
                               extra_args=["gravity/profile=true"],
                               expect_jeans_banner=True)
    fixed_out = run_athena("inputs/tests/selfgravity.athinput", fixed,
                           extra_args=["gravity/profile=true", "gravity/threshold=-1",
                                       "gravity/niteration=4"],
                           expect_jeans_banner=True, max_final_defect=None)
    run_athena("inputs/tests/selfgravity.athinput", root_device, expect_jeans_banner=True)
    run_athena("inputs/tests/selfgravity.athinput", root_host,
               extra_args=["gravity/root_on_host=true"], expect_jeans_banner=True)
    device_phi, _, _, _ = sg.uniform_grid_field(parse_latest_binary(root_device, "grav_phi"),
                                                "grav_phi")
    host_phi, _, _, _ = sg.uniform_grid_field(parse_latest_binary(root_host, "grav_phi"),
                                              "grav_phi")
    ...
    axes[0].plot(range(1, len(threshold_defects) + 1), threshold_defects, marker="o",
                 label="threshold")
    axes[0].plot(range(1, len(fixed_defects) + 1), fixed_defects, marker="o",
    ...
    axes[0].legend()
    keys = [k for k in ("source_load", "setup", "root_transfer", "smooth", "boundary",
                       "restrict_prolong", "result_retrieve") if k in profile]
...
def be_static_refined(metrics: dict[str, Any]) -> str:
  ...
  try:
    out = run_athena("inputs/tests/selfgravity_be_amr.athinput", basename,
    ...
    phi, x, y, z = sg.uniform_grid_field(parse_latest_binary(basename, "grav_phi"), "grav_phi")
    dens, _, _, _ = sg.uniform_grid_field(parse_latest_binary(basename, "hydro_u"), "dens")
...
def failure_cases(metrics: dict[str, Any]) -> None:
  failures = [
      ("non-cubic MeshBlock", "inputs/tests/selfgravity.athinput",
       ["meshblock/nx3=8"], "logically cubic MeshBlocks"),
      ("2D mesh", "inputs/tests/selfgravity.athinput",
       ["mesh/nx3=1", "meshblock/nx3=1"], "requires a 3D mesh"),
      ("bad mg_bc", "inputs/tests/selfgravity.athinput",
       ["gravity/mg_bc=bogus"], "Unknown gravity/mg_bc"),
      ("bad mporder", "inputs/tests/selfgravity_binary.athinput",
       ["gravity/mporder=3"], "gravity/mporder must be 2 or 4"),
  ]
  table = []
  ...
  with tempfile.TemporaryDirectory() as tmp:
    ...
    table.append({"case": "missing gravity block",
                  "expected": "self_gravity source terms require"})

    no_controls = tmp_path / "selfgravity_no_controls.athinput"
    no_controls.write_text(remove_parameters(source, "gravity", {"threshold", "niteration"}))
    expect_failure(no_controls, None, "Set either gravity/threshold or gravity/niteration")
    table.append({"case": "missing convergence control",
...
def mpi_summary(metrics: dict[str, Any]) -> str:
  ...
  for ranks in (2, 4):
    ...
    try:
      run_athena("inputs/tests/selfgravity.athinput", basename, mpi_ranks=ranks,
                 expect_jeans_banner=True)
      output = parse_latest_binary(basename, "grav_phi")
  ...
  try:
    run_athena("inputs/tests/selfgravity.athinput", device, mpi_ranks=2,
               expect_jeans_banner=True)
    run_athena("inputs/tests/selfgravity.athinput", host, mpi_ranks=2,
               extra_args=["gravity/root_on_host=true"], expect_jeans_banner=True)
    dphi, _, _, _ = sg.concatenate_field(parse_latest_binary(device, "grav_phi"), "grav_phi")
    hphi, _, _, _ = sg.concatenate_field(parse_latest_binary(host, "grav_phi"), "grav_phi")
    mpi_metrics["root_host_device_max_abs_phi_difference"] = float(np.max(np.abs(dphi - hphi)))
  finally:
    cleanup(device, host)

  basename = "docs_selfgravity_mpi_binary"
  try:
    run_athena("inputs/tests/selfgravity_binary.athinput", basename, mpi_ranks=2,
               max_final_defect=2.0e-6)
    output = parse_latest_binary(basename, "grav_phi")
    phi, x1, x2, x3 = sg.concatenate_field(output, "grav_phi")
    gconst = 1.0 / (4.0 * math.pi)
    ...
    mask = (r1 > 2.0 * radius) & (r2 > 2.0 * radius)
    mpi_metrics["binary_multipole_relative_l2"] = sg.relative_l2(phi[mask] - analytic[mask],
                                                                 analytic[mask])
  finally:
    cleanup(basename)
...
def gpu_summary(metrics: dict[str, Any]) -> str:
  ...
  try:
    run_athena("inputs/tests/selfgravity.athinput", basename,
               expect_jeans_banner=True)
    output = parse_latest_binary(basename, "grav_phi")
    phi, x1, x2, x3 = sg.concatenate_field(output, "grav_phi")
    analytic, _ = jeans_analytic(x1, x2, x3, 1.0e-3)
  ...
  try:
    run_athena("inputs/tests/selfgravity.athinput", device,
               expect_jeans_banner=True)
    run_athena("inputs/tests/selfgravity.athinput", host,
               extra_args=["gravity/root_on_host=true"],
               expect_jeans_banner=True)
    dphi, _, _, _ = sg.concatenate_field(parse_latest_binary(device, "grav_phi"), "grav_phi")
    hphi, _, _, _ = sg.concatenate_field(parse_latest_binary(host, "grav_phi"), "grav_phi")
    gpu_metrics["root_host_device_max_abs_phi_difference"] = float(np.max(np.abs(dphi - hphi)))
  finally:
    cleanup(device, host)
    ...
    for key, value in values.items():
      if isinstance(value, dict):
        for subkey, subvalue in value.items():
          rows.append(f"| `{group}` | `{key}.{subkey}` | {format_metric(subvalue)} |")
      else:
...
def write_results_page(figures: dict[str, str], metrics: dict[str, Any], with_mpi: bool,
                       with_gpu: bool) -> None:
  mpi_text = ""
...
def main() -> None:
  ...
  parser.add_argument("--build-cpu", action="store_true",
                      help="configure and build a serial CPU AthenaK binary before generating CPU figures")
  parser.add_argument("--with-mpi", action="store_true",
                      help="also generate MPI validation figures")
  parser.add_argument("--build-mpi", action="store_true",
  ...
  if args.with_mpi:
    if args.build_mpi:
      build(["-D", "Athena_ENABLE_MPI=ON"])
    figures["mpi_summary"] = mpi_summary(metrics)

  if args.with_gpu:
    if args.build_gpu:
      build(["-D", "Kokkos_ENABLE_CUDA=On"])
