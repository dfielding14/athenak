...
def test_periodic_jeans_potential_matches_analytic_solution():
    ...
    try:
        sg.run_athena("inputs/tests/selfgravity.athinput", basename,
...
def test_zero_field_mhd_matches_hydro_gravity_solution():
    ...
    try:
        sg.run_athena("inputs/tests/selfgravity.athinput", hydro_basename,
                      expect_jeans_banner=True)
        sg.run_athena("inputs/tests/selfgravity_mhd.athinput", mhd_basename,
                      expect_jeans_banner=True)
...
def test_source_terms_follow_analytic_jeans_force():
    ...
    try:
        sg.run_athena("inputs/tests/selfgravity_oracle.athinput", on_basename,
...
def test_binary_multipole_potential_and_force_are_physical():
    ...
    try:
        sg.run_athena("inputs/tests/selfgravity_binary.athinput", basename,
        ...
        analytic = (
            _uniform_sphere_potential(x1, x2, x3, center1, 1.0, radius, gconst)
        ...
        assert sg.relative_l2(phi[far_source] - analytic[far_source],
                              analytic[far_source]) < 3.5e-1

        phi_grid, gx1, gx2, gx3 = sg.uniform_grid_field(output, "grav_phi")
        dphidz, dphidy, dphidx = np.gradient(phi_grid, gx3, gx2, gx1,
                                             edge_order=2)
        x3g, x2g, x1g = np.meshgrid(gx3, gx2, gx1, indexing="ij")
        ax1a, ax2a, ax3a = _point_mass_acceleration(
            x1g, x2g, x3g, center1, 1.0, gconst)
        bx1a, bx2a, bx3a = _point_mass_acceleration(
            x1g, x2g, x3g, center2, 0.5, gconst)
...
def test_restart_equivalence_for_self_gravity_cycle():
    ...
    try:
        ...
        straight_out = sg.parse_binary_output(sg.latest_binary_output(straight, "hydro_u"))
...
def test_profile_diagnostics_report_gravity_phases():
    ...
    try:
        ...
        for field in ("source_load=", "setup=", "root_transfer=", "smooth=",
                      "boundary=", "restrict_prolong=", "result_retrieve="):
...
def test_missing_convergence_control_fails(tmp_path):
    ...
    try:
        sg.expect_failure(path, expected_text="Set either gravity/threshold or gravity/niteration")
    finally:
        testutils.cleanup()
