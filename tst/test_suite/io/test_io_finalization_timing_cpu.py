"""Regression tests for final-output policy and opt-in performance timing."""
...
def test_default_policy_retains_terminal_outputs(tmp_path):
    ...
    assert "PERFORMANCE_TIMING" not in stdout
...
def test_timing_is_opt_in(tmp_path):
    stdout, _, _ = _run_case(tmp_path, "time/performance_timing=true")
    assert stdout.count("PERFORMANCE_TIMING synchronized=true") == 1
    assert stdout.count("PERFORMANCE_REGION outputs ") == 1
