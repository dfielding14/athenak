# PIC Readiness Registry

This directory stores compact, source-controlled control records for the MHD-PIC
production-readiness program. It does not contain simulation output and does not
qualify any scientific claim by itself.
...
- `external_artifacts.json`: provenance and redistribution inventory for paper,
  Entity, reference-data, and plotting artifacts.
- `entity_snapshot.json`: frozen accessible Entity comparison tree and
  shared-kernel file checksums.
- `host_verification_2026-05-30.json`: bounded descriptive host-build and
...
- `q022_independent_comparison_preregistration_2026-05-30.json` and
  `q023_statistical_qualification_preregistration_2026-05-30.json`: fail-closed
  preregistration scaffolds for future comparison and statistical campaigns.
- `q018_claim_links_successor_2026-05-30.json`: per-claim applicability,
  limitations and current evidence-link scaffold with named-reviewer closure
  intentionally open.
- `q022_sun_bai_repository_reference_manifest_2026-05-30.json`,
...
- `q028_frontier_f0_admission_and_manifest_anchor_2026-05-30.json`: registered
...
- `q008_q033_expanding_coupled_source_successor_2026-05-30.json`: successor
...
- `q024_schema7_restart_resilience_successor_2026-05-30.json`: successor
  bounded serial-host schema-7 provenance, restart-resilience and fault-
  injection chronology record. The schema-6 sidecar remains preserved as
...
- `q042_architecture_style_conformance_2026-05-30.json`: touched-file style
  evidence, repository baseline debt, and architecture-review residuals.
- `schemas/validation_manifest.schema.json`: minimum machine-readable record
  required for a qualifying validation run.
...
1. Keep large artifacts outside Git under the authorized Frontier root.
2. Initialize and use the Frontier ledger only through the installed immutable
   control plane after Orion and the Project Home ledger mirror pass preflight.
3. Do not promote a claim to `qualified` while its reviewer is
   `pending external review`.
4. Run the readiness registry regression after editing these records:
