#!/usr/bin/env python3
"""Prepare the six non-authorizing Q019 Q023-carrier calibration runs."""
...
from tst.publication import (
    q019_q023_carrier_nonlinear_bell_redesign_v1 as design,
)
from tst.publication import q019_physics_first_nonlinear_bell_successor_v2 as deck_parser
from tst.publication import (
    q019_q023_carrier_resource_calibration_runtime_controller_v1 as controller,
)
from tst.publication import (
...
RUN_NAMESPACE = "runs/q019_q023_carrier_resource_calibration_v1"
PROJECT_HOME_RECEIPT_NAMESPACE = "ledger/q019_registered_execution_receipts"
CAMPAIGN = "q019_q023_carrier_resource_calibration_v1"
...
SUCCESSOR_ID = "q019_q023_carrier_calibration_launch_policy_preparation_v1"
SCHEMA_VERSION = 1
MANIFEST_RECORD_TYPE = "q019_q023_carrier_calibration_launch_policy_manifest_v1"
LAUNCH_RECORD_TYPE = "q019_q023_carrier_calibration_launch_candidate_v1"
POLICY_RECORD_TYPE = "q019_q023_carrier_calibration_policy_candidate_v1"
POLICY_FRAGMENT_RECORD_TYPE = "q019_q023_carrier_calibration_policy_fragment_v1"
FINAL_BINDING_RECORD_TYPE = "q019_q023_carrier_calibration_final_bindings_v1"
EXPECTED_ATTEMPT_COUNT = 6
MAXIMUM_ATTEMPTS_PER_PILOT = 1
MAXIMUM_RETRIES_PER_PILOT = 0
MAXIMUM_LIVE_Q019_SUBMISSIONS = 1
TASKS_PER_NODE = 8
MAXIMUM_BATCH_NODE_HOURS = 7.5
MAXIMUM_BATCH_STORAGE_BYTES = 576 * 1024**3
...
PILOT_RESOURCES = {
    ...
    3: {
        ...
        "maximum_storage_bytes": 128 * 1024**3,
    },
}
...
QUALIFICATION_EFFECT = (
    "q023_carrier_resource_calibration_only_excluded_from_q019_saturation_"
    "qualification_and_production_resource_freeze_no_science_no_publication_authority"
)
...
def _pilot_members() -> list[dict[str, object]]:
    ...
    runtime_manifest = controller.validate_checked_in_packet()
    _require(
        json.loads(controller.CHECKED_IN_MANIFEST.read_text(encoding="utf-8"))
        == runtime_manifest,
        "Q019 carrier calibration manifest drifted",
    )
    base_by_id = {item["case_id"]: item for item in base_manifest["decks"]}
    archived_by_id = {
        str(item["artifact_id"]): item for item in runtime_manifest["artifacts"]
    }
    pilots = []
    for overlay in controller.expected_overlays():
        artifact_id = str(overlay["artifact_id"])
        archived = archived_by_id.get(artifact_id)
        _require(
            type(archived) is dict
            and {key: archived.get(key) for key in overlay} == overlay,
            f"{artifact_id}: Q019 carrier calibration manifest entry drifted",
        )
        source_case_id = str(overlay["source_case_id"])
        _require(
            overlay["authority"] == "excluded_pilot_only"
            and source_case_id in base_by_id,
            "Q019 carrier calibration inventory drifted",
        )
        path = controller.CHECKED_IN_ROOT / str(archived["filename"])
        _, payload = q043_prep._stable_regular_bytes(
            path,
            label=f"Q019 carrier calibration deck {artifact_id}",
            require_read_only=False,
        )
        _require(
            payload.decode("utf-8") == controller.render_overlay(overlay)
            and _sha256_bytes(payload) == archived["rendered_sha256"],
            f"{artifact_id}: Q019 carrier calibration deck drifted",
        )
        base = dict(base_by_id[source_case_id])
        _require(
            int(base["dimension"]) == int(overlay["dimension"])
            and int(overlay["nodes"]) == PILOT_RESOURCES[int(base["dimension"])]["nodes"]
            and int(overlay["tasks"]) == PILOT_RESOURCES[int(base["dimension"])]["tasks"],
            f"{artifact_id}: Q019 carrier calibration resources drifted",
        )
        pilots.append(
            {
                ...
                "pair_role": str(overlay["instrumentation"]),
                "calibration_role": str(overlay["calibration_role"]),
                "cycle_limit": int(overlay["cycle_limit"]),
                "overlay": dict(archived),
                "base_case": base,
                "deck_path": str(path.relative_to(REPO_ROOT)),
                "deck_sha256": archived["rendered_sha256"],
    ...
    _require(
        ...
        == [str(item["artifact_id"]) for item in controller.expected_overlays()],
        "Q019 carrier calibration ordered matrix drifted",
    )
    return pilots
...
def _authorization_id(index: int, member: Mapping[str, object]) -> str:
    ...
    cycle_limit = int(member["cycle_limit"])
    return (
        f"q019-carrier-cal-{index:02d}-{dimension}d-"
        f"c{cycle_limit:04d}-{role}-v1"
    )
...
def _deck_binding(member: Mapping[str, object]) -> dict[str, object]:
    ...
    _, payload = q043_prep._stable_regular_bytes(
        path,
        label=f"Q019 carrier calibration deck {member['artifact_id']}",
        require_read_only=False,
    )
    _require(
        _sha256_bytes(payload) == member["deck_sha256"],
        f"{member['artifact_id']}: Q019 carrier calibration deck digest drifted",
    )
    return {
...
def _output_topology(member: Mapping[str, object]) -> dict[str, object]:
    ...
    blocks = deck_parser.parse_athinput_text(text)
...
def _wrapper_evidence(member: Mapping[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "required_controller_trigger_cycle": int(member["cycle_limit"]),
...
def build_materialization(
    ...
    for index, member in enumerate(members, 1):
        ...
        launch = {
            ...
            "cycle_limit": member["cycle_limit"],
            ...
            "controller_contract": {
                ...
                "pilot_cycle_limit": member["cycle_limit"],
            ...
            "exclusion_boundary": {
                ...
                "required_measurements": list(
                    member["overlay"]["required_measurements"]
                ),
            },
    ...
    manifest = {
        ...
        "calibration_matrix_contract": {
            "dimensions": [2, 3],
            "ordered_cycle_limits": [32, 256, 256, 16, 64, 64],
            "ordered_instrumentation": [
                "baseline",
                "baseline",
                "instrumented",
                "baseline",
            ...
            "one_attempt_per_matrix_member": True,
            "short_baseline_measures_startup": True,
            "all_runs_measure_peak_memory": True,
            "maximum_dimension_peak_memory_controls_headroom": True,
            "long_baseline_instrumented_pair_measures_overhead": True,
            "only_controller_instrumentation_differs_within_long_pairs": True,
            "expected_stop_reason": 1903,
            "trustworthy_peak_memory_requires_bound_sacct_step_rows": True,
            "all_required_measurements_must_be_present": True,
            "saturation_evidence_eligible": False,
