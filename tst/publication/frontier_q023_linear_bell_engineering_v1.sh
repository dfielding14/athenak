#!/bin/bash
# Compact corrected-current linear Bell engineering run.
#SBATCH --account=AST207
#SBATCH --partition=batch
#SBATCH --qos=debug
#SBATCH --nodes=1
#SBATCH --time=00:30:00
#SBATCH --job-name=pic-linear-bell-engineering
...
deck="${repo_root}/inputs/tests/q023_paper_bell_linear_joverc_predecessor/q023-joverc-d1-fine-reference_x1-eps0p4.athinput"
output_root="${PIC_OUTPUT_ROOT:-/lustre/orion/ast207/proj-shared/dfielding/PIC/engineering_readiness/linear-bell-${SLURM_JOB_ID}}"
...
/usr/bin/time -p -o "${output_root}/run.time" \
  srun -N1 -n2 --ntasks-per-node=2 --cpus-per-task=1 \
...
/opt/cray/pe/python/3.11.7/bin/python3 -B -m \
  ...
  linear "${output_root}" --output "${output_root}/quicklook.json" --require-pass
