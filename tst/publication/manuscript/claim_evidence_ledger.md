...
|---|---|---|
| Authoritative source baseline for this revision | `PIC` commit `9db61e545adc4c6c7558bac1e6288b510849fd87`; unintegrated successors are review context only | Positive methods statements must describe the integrated baseline; unintegrated work may narrow claims or identify blockers but cannot support a result |
...
| Q009 dynamic AMR/load-balance pilot | A reviewed fail-closed five-case contract specifies paired load-balance off/on migration, restart through `t>45`, and a held-out full-geometry engineering pilot under 100 node-hours and 200 GiB. An unsafe execution-preparation predecessor was rejected after adversarial review demonstrated forged-receipt, run-root-replacement, telemetry, pairwise-acceptance, and checkpoint-binding gaps. Independent review also blocked preparation-only successor `e2abdca7` as-is because fabricated or nonexistent final bindings can become installed-schema-compatible launch candidates, `control_plane_common` can be module-poisoned while repository bytes are claimed, and output-root escape or symlink traversal is permitted. A repair successor is in progress | Contract context only; the blocked preparation successor grants no launch/admission authority, and trusted telemetry/admission, live policy slices, execution, and scientific-AMR qualification remain open |
| Q009 conservative AMR receiver policy | A source-local `conservative_partition` candidate demonstrates serial partition-of-unity deposition across unequal receiver volumes and leaves `paper_smooth` default behavior distinct. Independent review found production-scale global PTAG all-to-all audit, serial host allocation and topology scans per particle, an unnecessary incompletely established PTAG-uniqueness precondition, and no multi-rank/dynamic-AMR/restart/HIP runtime evidence. The reviewed and integrated `paper_smooth` stage-local identity repair now deduplicates by source rank, stage-local source index, destination MeshBlock GID, and periodic image rather than persistent particle tag | `conservative_partition` remains open and must not be described as a production option or as qualifying Section 5.4 AMR conservation. The integrated identity repair prevents tag collisions from conflating distinct stage-local contributions but does not qualify dynamic AMR or broaden conservation claims |
| Q011 exact-accounting repair | Independent review accepted the ledger signs and staging in clean numerical-only successor `0464fd85` but blocked the successor as-is because its pure reducer accepts malformed or incomplete logical topology, transitive reducer dependencies are unbound, the ledger-disabled path introduces an unconditional new 3D restriction, and readiness overstates 2D3V enforcement. A repair successor is in progress | The blocked successor does not close Q011 numerical or conservation evidence; a reviewed repair, trusted registered execution, and independent recomputation remain required |
...
- Source-local preparation, bounded injection/provenance diagnostics, restart
  ...
  explanation.
- The active `paper_smooth` AMR policy targets receiver-resolution smoothness
  but is not individually conservative at mixed levels. The reviewed
...
- Independent review accepted the clean Q011 numerical-only successor's ledger
  signs and staging but blocked it as-is because its reducer accepts malformed
  or incomplete logical topology, transitive reducer dependencies are unbound,
  its ledger-disabled path adds an unconditional new 3D restriction, and
  readiness overstates 2D3V enforcement. A repair successor is in progress; the
  blocked successor cannot establish numerical closure or trusted execution.
...
| Q003/Q004/Q009 prerequisites | Pusher, coupling, multi-rank AMR migration/restart, HIP, and scientific-AMR gates close for required scope | Open; five-case dynamic-AMR pilot is unexecuted; preparation-only successor `e2abdca7` and conservative policy candidate are blocked and do not qualify or replace `paper_smooth` | Complete reviewed repairs and prerequisite registered simulations while retaining explicit AMR-policy scope |
...
| Restart and provenance | Registered restart carrier, particle provenance, cohort spectra, MPI/GPU, load balance, runtime, memory, and trusted exact-accounting evidence close | Open; clean numerical-only successor `0464fd85` is blocked as-is and supplies no trusted registered execution evidence | Complete and review the repair successor, then execute and review provenance-bound evidence |
...
| Paper coupling methods evidence | Explain opposite-impulse exchange on its qualified uniform-mesh scope and ideal-MHD induction isolation | Bounded host and selected registered GPU evidence described in production plan; independent review accepted Q011 successor `0464fd85` ledger signs/staging but blocked its reducer, dependency binding, ledger-disabled dimensional restriction, and readiness claims | A reviewed repair, exact source audit, trusted execution evidence, full intended decomposition/GPU scope, explicit boundary/AMR/injection/removal exclusions, and claim-specific review | `draftable_method_context` |
...
| Can the Q011 repair successor preserve the independently accepted ledger signs/staging while rejecting malformed or incomplete topology, binding transitive reducer dependencies, removing the unconditional ledger-disabled 3D restriction, and correcting the 2D3V readiness claim before trusted execution? | Blocks shock conservation evidence | Complete and independently review the repair, then obtain trusted registered execution and independent recomputation |
