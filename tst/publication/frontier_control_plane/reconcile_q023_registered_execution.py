...
"""Reconcile Q023 and publish deterministic registered-execution evidence."""
...
ENTRYPOINT_NAME = "reconcile_q023_registered_execution.py"
...
AUTHORIZATION_PREFIX = "q023-"
EXECUTION_RECEIPT_NAME = "q023_registered_execution_receipt.json"
EXECUTION_RECEIPT_RECORD_TYPE = "q023_reconciled_registered_execution_receipt"
TERMINAL_RECEIPT_NAME = "q023_terminal_receipt.json"
TERMINAL_RECEIPT_RECORD_TYPE = "q023_registered_execution_terminal_receipt"
PROJECT_HOME_MIRROR_NAMESPACE = Path("ledger/q023_registered_execution_receipts")
REQUIRED_FIELDS = ("mhd_w_bcc", "prtcl_rho", "prtcl_jx", "prtcl_jy", "prtcl_jz")
REQUIRED_OUTPUT_INDICES = tuple(range(89))
REQUIRED_TLIM = 1.75
REQUIRED_NLIM = 2_000_000
...
_MEMBER_ID = re.compile(
    r"d[123]-(?:coarse|fine)-"
    r"(?:reference_x1|split_x2|split_x3|split_x1x2|split_xyz)-eps0p[12468]"
)
...
def _producer_binding(inventory: dict[str, object]) -> dict[str, object]:
    ...
    if not isinstance(version, str) or _SHA256.fullmatch(version) is None:
        raise ValueError("Q023 producer control-plane version is malformed")
    return {
...
def _require_bootstrapped_installed_producer(
    ...
    *,
    ...
    if not getattr(_sys, "_pic_control_plane_bootstrapped", False):
        raise ValueError("Q023 reconciliation requires bootstrapped installed execution")
    ...
    if (
        ...
        raise ValueError(
            "Q023 captured executing entrypoint differs from verified generation"
        )
...
def _q023_event(
    ...
    *,
    ...
    if len(matches) != 1:
        raise ValueError("Expected one Q023 reconciliation event for the scheduler job")
    event = matches[0]
    ...
    if (
        ...
        raise ValueError("Q023 reconciliation event is not a successful registered execution")
    member_id = event.get("test_id")
    if not isinstance(member_id, str) or _MEMBER_ID.fullmatch(member_id) is None:
        raise ValueError("Q023 reconciliation member identity is malformed")
    return event
...
def _trusted_wrapper_evidence(
    ...
    try:
        ...
        raise ValueError("Q023 retained stdout is not UTF-8") from error
    rank_line = (
        f"Q023_REGISTERED_EXECUTION case_id={member_id} "
        f"mpi_world_size={tasks} rank_ids={','.join(str(rank) for rank in range(tasks))}"
    )
    exit_line = "Q023_REGISTERED_EXECUTION_EXIT exit_code=0 signal=0"
    if lines.count(rank_line) != 1 or lines.count(exit_line) != 1:
        raise ValueError("Q023 retained stdout lacks exact trusted rank/exit evidence")
    ...
    if (
        lines.count("Terminating on time limit") != 1
        or len(terminal) != 1
        or len(limits) != 1
        or not math.isclose(terminal[0][0], REQUIRED_TLIM, rel_tol=0.0, abs_tol=1e-12)
        or terminal[0][1] <= 0
        or not math.isclose(limits[0][0], REQUIRED_TLIM, rel_tol=0.0, abs_tol=1e-12)
        or limits[0][1] != REQUIRED_NLIM
    ):
        raise ValueError("Q023 retained stdout lacks exact terminal-time evidence")
    return {
...
def _structured_launch_evidence(
    ...
    *,
    ...
    if len(actions) != 1:
        raise ValueError("Q023 requires one trusted trampoline Athena action")
    action = actions[0]
    ...
    if len(raw_directories) != 1 or input_decks != 1:
        raise ValueError("Q023 launch contract lacks one raw directory and input deck")
    raw_relative = secure._safe_relative(
        raw_directories[0], label="Q023 raw artifact directory"
    )
    raw_root = artifact_dir.joinpath(*PurePosixPath(raw_relative).parts)
    tasks = action["resources"]["tasks"]
    if type(tasks) is not int or tasks not in {2, 4, 8}:
        raise ValueError("Q023 launch contract has unsupported MPI task count")
    records = sealed_inventory["records"]
    if not isinstance(records, dict):
        raise ValueError("Q023 sealed artifact records are malformed")
    stdout_record = records.get("athena_stdout.txt")
    if not isinstance(stdout_record, dict):
        raise ValueError("Q023 sealed artifact records lack trusted stdout")
    ...
    expected = [
        ...
        for index in REQUIRED_OUTPUT_INDICES
    ...
    observed_bins = sorted(
        ...
        if relative.startswith(prefix) and relative.endswith(".bin")
    )
    if observed_bins != sorted(expected):
        raise ValueError("Q023 sealed binary output differs from exact expected inventory")
    raw_inventory = []
    for index in REQUIRED_OUTPUT_INDICES:
        for field in REQUIRED_FIELDS:
            relative = f"bin/{basename}.{field}.{index:05d}.bin"
...
def derive_q023_registered_execution_evidence(
    ...
    *,
    ...
    """Derive exact Q023 receipts from controller-owned immutable evidence."""
    ...
    if (
        ...
        raise ValueError("Q023 producer, reconciliation, and mirror acknowledgment differ")
    ...
    manifest, manifest_payload = secure._read_json(
        manifest_path, root=root, label="Q023 pre-submit manifest"
    )
    if hashlib.sha256(manifest_payload).hexdigest() != event.get("manifest_sha256"):
        raise ValueError("Q023 pre-submit manifest digest differs from reconciliation")
    ...
    if (
        ...
        raise ValueError("Q023 pre-submit manifest differs from reconciliation")
    ...
    if _UUID.fullmatch(submission_id) is None or _JOB_ID.fullmatch(job_id) is None:
        raise ValueError("Q023 reconciliation has malformed scheduler identity")
    ...
    candidate, candidate_payload = secure._read_json(
        candidate_manifest_path, root=root, label="Q023 clean-candidate manifest"
    )
    ...
    if (
        ...
        raise ValueError("Q023 clean candidate differs from pre-submit manifest")
    receipt = {
        "schema_version": 1,
...
def publish_q023_registered_execution_evidence(
    event: dict[str, object],
    mirror_ack: dict[str, object],
    inventory: dict[str, object],
    *,
    ...
    derived = derive_q023_registered_execution_evidence(
        event,
        mirror_ack,
        inventory,
    ...
    if identities[0] == identities[2] or identities[1] == identities[3]:
        raise ValueError("Q023 Orion and Project Home evidence reuse one filesystem object")
...
def reconcile_q023(
    *,
    ...
    if paired != inventory:
        raise ValueError("Q023 producer installed control-plane pair differs")
    ...
    if len(completion_events) != 1:
        raise ValueError(
            "Q023 reconciliation requires one canonical trampoline-completion anchor"
        )
    ...
    completion_manifest, completion_payload = secure._read_json(
        ...
        root=Path(os.path.abspath(authorized_pic_root)),
        label="Q023 trampoline-completion pre-submit manifest",
    )
    if (
        hashlib.sha256(completion_payload).hexdigest()
        != completion_event.get("manifest_sha256")
    ):
        raise ValueError("Q023 trampoline-completion manifest digest differs")
    ...
    trusted_event = _q023_event(
        records,
        job_id=job_id,
        producer_control_plane_version=str(inventory["version"]),
    )
    if trusted_event != event:
        raise ValueError("Q023 reconciler result differs from canonical mirrored ledger")
    ...
    evidence = publish_q023_registered_execution_evidence(
        event,
        mirror_ack,
        inventory,
        authorized_pic_root=authorized_pic_root,
