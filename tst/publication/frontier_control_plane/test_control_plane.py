...
"""
    ...
    def test_completed_q043_registered_slice_retirement_is_evidence_bound(
        self,
    ) -> None:
        campaign = control_plane_common.Q043_REGISTERED_CAMPAIGN
        manifest = self._write_science_config(
            authorize=False,
            campaign=campaign,
            test_id="q043-case-001",
            registered_science_authorization_id="q043-re-001-v1",
        ...
        slices = [
            {
                **base_slice,
                ...
                "test_id": f"q043-case-{index:03d}",
            }
            for index in range(1, control_plane_common.Q043_REGISTERED_CASE_COUNT + 1)
        ]
        ...
        matrix_path = (
            self.pic_root / control_plane_common.Q043_REGISTERED_MATRIX_RELATIVE
        )
        matrix_path.parent.mkdir(parents=True)
        matrix = {
            "record_type": control_plane_common.Q043_REGISTERED_MATRIX_RECORD_TYPE,
            "status": control_plane_common.Q043_REGISTERED_MATRIX_STATUS,
            ...
            "case_count": control_plane_common.Q043_REGISTERED_CASE_COUNT,
            "authorization": {
                "launch_authorized": False,
            ...
            "case_admissions": [
                {
                    "case_id": f"q043-case-{index:03d}",
                    "campaign_id": campaign,
                    "status": control_plane_common.Q043_REGISTERED_CASE_STATUS,
                    "execution_binding": {
                        "registered_execution_identity": {
                            "registered_science_authorization_id": (
                                f"q043-re-{index:03d}-v1"
                            )
                        }
                    },
                }
                for index in range(
                    1, control_plane_common.Q043_REGISTERED_CASE_COUNT + 1
                )
            ],
        }
        ...
        self._rewrite_reviewed_storage_preflight_artifact(
            lambda artifact: artifact.update(
                started_utc="2026-06-04T00:00:00Z",
                completed_utc="2026-06-04T00:00:00Z",
            )
        )
        ...
        with self.assertRaisesRegex(ValueError, "matrix digest changed"):
            promote(
                self.policy,
                retire_completed_q043_registered_slices=True,
                q043_registered_matrix=matrix_path,
                q043_registered_matrix_sha256="0" * 64,
        ...
        with patch(
            ...
            promote(
                ...
                retire_completed_q043_registered_slices=True,
                q043_registered_matrix=matrix_path,
                q043_registered_matrix_sha256=sha256(matrix_path),
                expected_active_policy_sha256=expected_policy_sha256,
                expected_active_promotion_sha256=expected_promotion_sha256,
                control_plane_dir=successor,
                authorized_pic_root=self.pic_root,
    ...
    def test_nonempty_registered_allowlist_cannot_be_silently_removed(self) -> None:
        ...
        self._write_policy(
            registered_science_slices=[],
            science_submission_freeze=self._authorized_science_freeze(manifest),
            admission_smoke_overrides={"status": "closed_after_pass"},
        )
        with self.assertRaisesRegex(ValueError, "evidence-bound retirement"):
            self._promote_policy()
