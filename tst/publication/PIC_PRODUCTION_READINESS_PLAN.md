...
| Q-005 | Bell | Section 5.2 dispersion and phase | Verifying: bounded serial-host extractor validation passes and the reused Bell fixture is now quantitatively classified as an engineering proxy because its growth and phase miss the deck-bound no-Hall analytical branch; source-local `Q023-PAPER-BELL-LINEAR` preparations now freeze Section 5.2 paper-value 1D/2D/3D geometry and normalization, a dedicated right-polarized eigenmode generator and a fail-closed raw-trace analytical-dispersion contract. The physical 1D preparation uses an exact transverse-invariant thin 2D3V carrier because the particle module intentionally supports 2D/3D meshes. Bounded Debug cycle-zero initialization passes for all three dimensions and a 2D schema-7 restart continuation advances one cycle. Clean freeze, exact variants, convergence, MPI, GPU and external review remain open |
...
| Q-029 | Hall-extension qualification | Separately named derived mode with linear/nonlinear Bell and shock-front tests. The authorized target requires an `implemented_and_qualified` outcome | Verifying: opt-in experimental CT source and host odd-in-`alpha_H` manufactured-source smoke pass; source-local `q029_hall_extension_source_local_candidate_2026-05-30.json` freezes the exact implemented additive CT-current normalization and dimensionless `chi_H` form with an intentionally unavailable generator and synthetic-contract-only analyzer that always emits `qualifying_evidence=false`; reviewed physical Bai mapping, runtime generator, linear/nonlinear Bell, shock-front, GPU and decomposition qualification remain open |
...
8. Close remaining local prerequisites: Q-009 MPI and HIP stress slices after
   the bounded coupled Debug/UBSan and repaired x2-inflow host-ASan/UBSan passes,
