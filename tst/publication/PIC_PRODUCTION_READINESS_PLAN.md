...
| Q-002 | Physical-mode configuration | Parser rejection and metadata tests | Verifying: host parser contract accepts five bounded named-mode compositions, rejects 28 invalid combinations and checks runtime identity; registered F0 Frontier metadata capture and parser/startup smoke pass; bounded clean-candidate F2 eight-rank one-node Athena runtime-metadata adapter and launch contract are staged from commit `5a9f1ac3`; descriptor-pinned registered-science analyzer publication and no-write recomputation are hardened at `60af2d4b`; curated policy binding, live promotion, serialized execution and external review remain open |
