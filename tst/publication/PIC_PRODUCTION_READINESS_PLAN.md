...
| PIC-P0-005 | P0 | AMR retains the `MeshBlockPack` object while reconstructing child MeshBlock/coordinate objects. Cross-rank particle migration already resolves new geometric ownership; retained particles now receive an explicit post-AMR refresh hook that validates MeshBlock-sized capacities and refreshes the stable pack pointer | `src/mesh/mesh_refinement.cpp`; `src/mesh/load_balance.cpp`; `src/particles/particles.cpp`; particle and mesh boundary-helper construction paths | Serial shock-rich refine smoke, a six-transition serial refine/derefine stress with five restart continuations and a coupled PIC/MHD reflecting/outflow Debug-plus-UBSan lifetime stress pass; broader coupled boundaries, multi-rank MPI migration/load-balance/restart, ASan and GPU evidence remain open | Complete Q-009 matrix |
...
build argv: Release, MPI, HIP, `Kokkos_ARCH_ZEN3`,
...
| --- | --- | --- | --- |
| Q-001 | Clean provenance | Curated baseline, manifest, executable checksum | Verifying: historical `origin/PIC` baseline remains pinned at `3bcd3f21`; local `PIC` carries the successor implementation series pending final clean-commit curation; compact readiness records and external-artifact checksums exist; release manifest, final executable checksum and archived clean candidate remain open |
...
| Q-027 | Frontier QOS-selection and budget boundary | `debug`-preferred, `normal`-fallback selection is recorded and policy-compliant; serialized PIC submission ledger is complete; cumulative reserved-plus-consumed usage remains `<=10000` node-hours; expanded work remains blocked pending revised authorization and user permission | Verifying: mirrored ledger genesis event `849bf340` was initialized by predecessor snapshot `cde67c16`; registered F0 job `4744225` and F1 dirty-candidate GPU gyro job `4744232` remain historical bounded evidence; predecessor digest `3e933edd`, policy SHA `228440de` and promotion SHA `700a059a` are preserved as read-only chronology in `q027_active_control_plane_generation_2026-05-30.json`; security-hardening `d8820c33`, anchor-repair `121f9f8f`, recovery-hardening `7b722ab2`, recovery-provenance `7ea4aa8b`, accounting-provenance `1843ce72`, bootstrap-scheduler `7397038b`, cross-root serialization `88db7e94` and inner-lock `0d07f132` candidates are preserved as superseded chronology after independent audits found activation blockers; live outer-anchor digest `80c0797b` retains the closed bootstrap controls and serializes ledger mutation and policy promotion through a descriptor lock on site-owned `/lustre/orion/ast207`, outside replaceable PIC domains. Its 208-test control-plane suite, 36-test focused ledger suite, static checks, paired temporary install, 286-test publication discovery and independent combined-domain clone/hardlink retest pass in `q027_control_plane_activation_lock_hardening_candidate_2026-05-30.json`. Quiescent paired Orion and Project Home install, one-time anchor migration and active-policy promotion pass in `q027_outer_anchor_paired_activation_2026-05-30.json` with the original genesis and unchanged ledger preserved. The first clean build-profile preflight then rejected caller-dependent Frontier `MODULEPATH` provenance before artifact creation; staged successor digest `c8002a1d` resets a reviewed seed before loading, publishes the exact reviewed 19-component value after all module work and passes its 212-test control-plane suite, 288-test publication discovery, strict writer and runtime mutation regressions and independent stripped-wrapper audit in `q027_control_plane_modulepath_hardening_candidate_2026-05-30.json`. Paired successor immutable install and idempotent anchor migration pass; promotion, science freeze and applicable Frontier prerequisites remain open; cumulative consumption remains `0.07444444444444442` node-hours with no active reservation |
...
| Q-038 | Frontier environment profile selection | Minimum-supported baseline plus controlled XNACK/OFI A/B evidence, redacted allowlist capture and restricted-submit-export rank-0 propagation smoke test | Verifying: registered F0 baseline captures `HSA_XNACK=0`, `MPICH_GPU_SUPPORT_ENABLED=1` and `SLURM_EXPORT_ENV=ALL`, and maps eight ranks to eight distinct `ROCR_VISIBLE_DEVICES`; hardened F1 dirty-candidate gyro job captures the same allowlist, a visible rank-0 GPU, `libamdhip64`, `libmpi_amd` and `libmpi_gtl_hsa`; live outer-anchor control-plane `80c0797b` retains the closed `frontier_minimum_supported`, `frontier_xnack1_experimental` and `frontier_ofi_tuned_experimental` selector, strict Frontier value validation, Bash-startup sanitization, `sbatch --export=NIL`, a scheduler `env -i` boundary and descriptor-based redacted allowlist capture closed before workload execution while deliberately leaving `frontier_selected_production` undefined. Its first clean writer preflight exposed caller-dependent site-hook `MODULEPATH` regeneration before artifact creation; staged `c8002a1d` closes that gap with explicit reviewed pre-load seed and post-load canonical publication plus stripped, inherited and poisoned-wrapper equality regressions; paired promotion, matched XNACK/OFI A/B and Athena communication evidence remain open |
...
4. Preserve the local independent adversarial PASS for historical successor
   ...
   artifact creation. Install and promote staged reviewed successor
   `c8002a1d` from operational commit `37ccee56`, then rerun the exact clean
   build and freeze.
...
7. Install the paired immutable successor control plane. While the predecessor
   policy remains active, run the installed successor's audited
   `initialize_frontier_ledger.py --migrate-existing-anchor` transition to
   create or repair the paired read-only genesis anchors without appending a
   second genesis. Then promote the closed successor policy and archive its
   exact digest, anchors and promotion record.
