...
| Q-002 | Physical-mode configuration | Parser rejection and metadata tests | Verifying: host parser contract accepts five bounded named-mode compositions, rejects 28 invalid combinations and checks runtime identity; registered F0 Frontier metadata capture and parser/startup smoke pass; bounded clean-candidate F2 eight-rank one-node Athena runtime-metadata adapter, launch contract and one-attempt policy row are staged from commit `5a9f1ac3`; curated policy binding, live promotion, serialized execution and external review remain open |
