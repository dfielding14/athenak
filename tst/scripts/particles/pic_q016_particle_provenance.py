...
def _particle_vtk_files(basename):
...
def _spectrum_agreement(path, semantics):
    data = read_particle_vtk(path)
...
def run(**kwargs):
    ...
    full_files = _particle_vtk_files(full)
    _RESULTS["full_initial"] = _snapshot(full_files[0])
    _RESULTS["full_final"] = _snapshot(full_files[-1])
    _RESULTS["full_final_path"] = full_files[-1]
    ...
    _RESULTS["restart_final"] = _snapshot(_particle_vtk_files(restarted)[-1])
...
def analyze():
    ...
    full_f_error, full_f = _spectrum_agreement(_RESULTS["full_final_path"], "full_f")
    delta_f_error, delta_f = _spectrum_agreement(
        _RESULTS["full_final_path"], "delta_f_perturbation"
    )
    metrics = {
