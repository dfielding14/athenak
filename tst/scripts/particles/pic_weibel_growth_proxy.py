...
_INPUT_DECK = 'tests/pic_weibel_growth_proxy.athinput'
...
def _mode1_amplitude(dataset):
    jy = np.asarray(dataset['prtcl_jy'], dtype=float)
    jy_x = np.mean(jy, axis=(0, 1))
    jy_fluc = jy_x - np.mean(jy_x)
    ...
    mode = np.sum(jy_fluc * phase) / jy_x.size
    return float(np.abs(mode))


def _load_mode_series(basename):
    pattern = os.path.join(_athena_exe_dir(), 'bin', basename + '.prtcl_jy.*.bin')
    ...
    gamma, _, _ = fit_exponential_growth(t, a + floor, t[2], t[-3], floor=1.0e-30)
    return {
        'gamma': float(gamma),
    }
...
def run(**kwargs):
    ...
    _remove_outputs('pic_weibel_growth_proxy_np1')
    _run_case('serial', 1, 'pic_weibel_growth_proxy_np1')
    _RESULTS['np1'] = _load_mode_series('pic_weibel_growth_proxy_np1')

    if _athena_mpi_enabled():
        _remove_outputs('pic_weibel_growth_proxy_np2')
        _run_case('mpi2', 2, 'pic_weibel_growth_proxy_np2')
        _RESULTS['np2'] = _load_mode_series('pic_weibel_growth_proxy_np2')
