...
production campaigns governed under `tst/publication/`.
...
| Change MHD backreaction or stage ordering | `pic_paper_coupling_conservation_vl2_tsc.py`, `pic_paper_task_stage_trace_vl2_tsc.py`, `pic_mhd_current_coupling.py` | `src/mhd/mhd_tasks.cpp`, particle task wiring, and paired VL2 decks |
...
- `cd tst && python3 run_tests.py particles/pic_bell_growth_proxy --cmake=-DAthena_ENABLE_MPI=ON`
