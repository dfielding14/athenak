"""Bounded serial-host Q-009 repeated AMR particle lifetime stress."""
...
_INPUT_DECK = "tests/pic_q009_repeated_amr_lifetime.athinput"
_SOURCE_ROOT = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "..", "..", "..")
)
_STAGES = [
    ("refine_1", "refine", 0.5, 2),
    ("derefine_1", "derefine", 2.0, 4),
    ("refine_2", "refine", 0.5, 6),
    ("derefine_2", "derefine", 2.0, 8),
    ("refine_3", "refine", 0.5, 10),
    ("derefine_3", "derefine", 2.0, 12),
]
_RESULTS = {}
...
def _athena_exe_dir():
    return os.environ.get(
        "ATHENA_Q009_EXE_DIR", os.path.join(os.getcwd(), "build", "src")
    )
...
def _particle_integrity(initial, stages):
    ...
    for meshblock_limit, snapshot in zip(meshblock_limits, snapshots):
        ...
        checks["in_domain"] &= bool(
            np.all((snapshot["points"][:, 0] >= 0.0) & (snapshot["points"][:, 0] <= 1.0))
        )
...
def run(**kwargs):
    ...
    initial = _initial_particle_snapshot("pic_q009_refine_1")
...
def analyze():
    ...
    summary = _summary()
    logger.info("Q-009 repeated AMR lifetime metrics: %s", summary)
    ...
    return (
        summary["restart_continuations"] >= 1
        and len(stages) == 6
