...
from scripts.particles import pic_paper_coupling_conservation_vl2_tsc as conservation
...
def _check_static_task_graph():
    ...
    required_fragments = [
        ...
        ('paper communication task graph',
         particles_tasks,
...
def _run_staged_deltaf_smoke():
    command = [
        './athena', '-i', os.path.relpath(_INPUT_DECK, _EXE_DIR),
        'job/basename=pic_paper_task_stage_trace_vl2_tsc_deltaf',
        'particles/pic_deltaf_mode=physical',
        'particles/pic_deltaf_f0=kappa_iso',
        'particles/track_displacement=true',
    ]
    logger.info('Executing staged delta-f smoke: %s', ' '.join(command))
    proc = subprocess.run(command, cwd=_EXE_DIR, capture_output=True, text=True)
    output = (proc.stdout or '') + (proc.stderr or '')
    if proc.returncode != 0 or 'deltaf=physical' not in output:
        raise RuntimeError('Staged delta-f smoke failed\n' + output)
    _RESULTS['staged_deltaf_smoke'] = 'pass'
...
def analyze():
    ...
    return (_RESULTS.get('static_graph') == 'pass'
            ...
            and _RESULTS.get('staged_deltaf_smoke') == 'pass'
