...
_INPUT_DECK = 'tests/pic_entity_deposit_mink.athinput'
...
def run(**kwargs):
    ...
    if _athena_mpi_enabled():
        cases += [('np2', 2), ('np3', 3), ('np4', 4)]
    else:
        logger.info('MPI disabled: running serial-only entity-style deposit check')

    for case_name, nproc in cases:
        basename = 'pic_entity_dep_mink_' + case_name
...
def analyze():
    ...
    for case_name, result in _RESULTS.items():
        ...
        ok = _check_with_tolerance(case_name + ':npart', measured['npart'],
                                   expected['npart'], 1.0e-8, 1.0e-8) and ok
