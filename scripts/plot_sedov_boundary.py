...
def equilibrium_function(temp: float, pressure_over_k: float,
                         hrate: float, lhd: np.ndarray) -> float:
    return pressure_over_k * float(ism_cooling(temp, lhd)) / temp - hrate


def bisect_root(lo: float, hi: float, pressure_over_k: float,
                hrate: float, lhd: np.ndarray) -> float:
...
def stable_equilibria(pressure_over_k: float, hrate: float,
                      lhd: np.ndarray) -> tuple[float, float]:
    temp_min = 1.0
    temp_max = 1.0e7
    roots: list[float] = []
    prev_t = temp_min
...
def tvns_log_xi5_from_log_eta(log_eta: np.ndarray, gamma: float) -> np.ndarray:
    nu1 = -(13.0 * gamma**2 - 7.0 * gamma + 12.0) / \
...
def tvns_profile(
        xi: np.ndarray, gamma: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def build_series(args: argparse.Namespace) -> dict[str, Any]:
    ...
    age_start_cgs = math.sqrt((radius_start_cgs / beta) ** 5 *
                              ambient_mass_density_cgs / energy_cgs)
    ...
    radial_x = np.divide(
        dx1_sample,
        radius_sample,
        out=np.zeros_like(radius_sample),
        where=radius_sample > 0.0)
    radial_y = np.divide(
        dx2_sample,
        radius_sample,
        out=np.zeros_like(radius_sample),
    ...
    shocked_pressure = shocked_density * \
        (shock_speed_cgs * xi_profile) ** 2 * sound_speed_shape / gamma
    ...
    return {
        ...
        "metadata": {
            ...
            "origin_cgs": (
                origin_x1 * length_cgs,
                origin_x2 * length_cgs,
                origin_x3 * length_cgs,
            ),
...
def plot_positive(ax: Any, x: np.ndarray, y: np.ndarray,
                  label: str, **kwargs: Any) -> None:
...
def plot_series(path: Path, series: dict[str, Any]) -> None:
    ...
    plot_positive(
        ...
        time,
        series["pressure"],
        label="TVNS boundary pressure [dyn cm^-2]")
    plot_reference_line(
        ax,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="AthenaK input file")
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("sedov_inner_x1_boundary.png"))
    parser.add_argument("--csv-output", type=Path, default=None)

    parser.add_argument("--t-min", type=float, default=0.0)
    parser.add_argument("--t-max", type=float, default=10.0)
    parser.add_argument("--samples", type=int, default=512)
    parser.add_argument("--ghost-index", type=int, default=1,
                        help="1 is the innermost ghost cell")
