#!/usr/bin/env python3
"""Run R14 without turning its finite-limiter hard-bound diagnostic fatal."""
...
class RelaxedR14Error(RuntimeError):
...
def patch_segment(segment: Path) -> None:
    ...
    if text.count(needle) != 1:
        raise RelaxedR14Error(f"unexpected batch command shape: {script}")
    script.write_text(
    ...
    manifest.update({
        ...
        "claim_scope": (
            "R14 finite-rate limiter scan; every hard-bound event remains "
...
def launch(root: Path, *, submit: bool) -> Path:
    ...
    if segments:
        raise RelaxedR14Error(f"relaxed R14 lineage already exists: {segments[-1]}")
...
def continue_segment(segment: Path, *, submit: bool) -> Path | None:
    ...
    if str(manifest["case_id"]) != CASE_ID:
        raise RelaxedR14Error(f"expected {CASE_ID} segment: {segment}")
    ...
    if not math.isfinite(restart_time) or restart_time <= start_time:
        raise RelaxedR14Error(f"restart did not advance beyond segment start: {segment}")
...
def main() -> int:
    ...
    launch_command = commands.add_parser("launch", help="launch relaxed R14")
    launch_command.add_argument("--submit", action="store_true")
    advance = commands.add_parser("advance", help="continue relaxed R14")
