...
F117_FORBIDDEN_PROMOTION_RELATIVES = (
    Path("accounting/mks24_stage_i_E03_forcing_policy_F117_recost_evidence.json"),
    Path("accounting/mks24_stage_i_E03_forcing_policy_F117_recost_evidence.json.staged"),
    Path(
        "accounting/"
...
def authenticate_f118_committed_tools(
    value: object, repository: Path, expected_head: str
) -> list[dict[str, object]]:
    """Authenticate the exact seven committed tools selected by F118."""

    if not isinstance(value, list) or len(value) != len(F118_REQUIRED_COMMITTED_TOOLS):
        raise ValueError("F118 committed_tools must contain exactly seven tools")
    retained = []
    for index, (relative, expected_mode) in enumerate(
        sorted(F118_REQUIRED_COMMITTED_TOOLS.items())
    ):
        record = require_exact_keys(
            value[index],
            {"path", "revision", "sha256", "mode"},
            f"F118 committed tool {index}",
        )
        digest = require_sha256(record["sha256"], f"F118 committed tool {relative} SHA-256")
        if record != {
            "path": relative,
            ...
            raise ValueError("F118 committed_tools identity, order, revision, or mode differs")
        ...
        try:
            ...
            raise ValueError(f"F118 committed tool {relative} mode is not ASCII") from error
        if (
            tree.returncode
            or not tree_line.startswith(f"{git_mode} blob ")
            or not tree_line.endswith(f"\t{relative}")
        ):
            raise ValueError(f"F118 committed tool {relative} Git mode differs")
...
def parse_historical_f116_source_authority(
    root: Path,
    bindings: object,
    ceiling_path: Path,
    ceiling_sha256: str,
    ...
    """Authenticate immutable F116 and its nested historical F115 authority."""

    retained = require_exact_keys(
        bindings,
        {"evidence", "publication_audit", "provenance_review", "plasma_review"},
        "historical F116 source authority bindings",
    )
    paths = {
    ...
    loaded: dict[str, tuple[dict[str, object], str]] = {}
    for key, path in paths.items():
        relative, expected = input_binding(retained[key], f"historical F116 {key}")
        if root_path(root, relative.as_posix(), f"historical F116 {key}") != path:
            raise ValueError(f"historical F116 {key} is not the exact retained path")
        if root == DEFAULT_ROOT and expected != F116_CANONICAL_SHA256[key]:
            raise ValueError(f"historical F116 {key} differs from canonical authority")
        value = parse_json(
            tracker.read(path, expected, f"historical F116 {key}", expected_mode=0o444),
            f"historical F116 {key}",
        )
        if not isinstance(value, dict):
            raise ValueError(f"historical F116 {key} must be an object")
        loaded[key] = value, expected
    ...
    if (
        ...
        raise ValueError("historical F116 source authority identity differs")
    ...
    if (
        ...
        or not isinstance(current["verified_revisions"], list)
        or require_revision(current["head"], "historical F116 current source head")
        not in current["verified_revisions"]
    ):
    ...
    if (
        ...
        or audit.get("execution_epoch") != EXECUTION_EPOCH
        or audit.get("historical_f115_authority") != historical_f115
        or audit.get("authority_and_enforcement") != F116_AUTHORIZATION
    ):
        raise ValueError("historical F116 publication audit identity differs")
    require_declared_binding(
        audit.get("artifact"),
        paths["evidence"],
        evidence_sha256,
        "historical F116 publication artifact",
    )
    reviews = audit.get("independent_reviews")
    if not isinstance(reviews, dict) or reviews.get(
        "reviews_bind_exact_published_f116_sha256"
    ) != evidence_sha256:
        raise ValueError("historical F116 publication review binding differs")
    for key, audit_key in (
        ("provenance_review", "provenance_security"),
        ("plasma_review", "plasma_scientific_continuation"),
    ):
        require_declared_binding(
            reviews.get(audit_key),
            paths[key],
            loaded[key][1],
            f"historical F116 {key}",
        )
...
def parse_current_source_authority(
    ...
    """Authenticate F118 current authority and immutable historical F116/F115."""
    ...
    retained = require_exact_keys(
        ...
        "F118 current source authority bindings",
    )
    if retained["checkpoint"] != "F-118":
        raise ValueError("current source authority binding is not F-118")
    final_relative, final_sha256, final_revisions = source_bundle_binding(
        retained["final_source_bundle"], "F118 final source bundle binding"
    )
    if (
        root_path(root, final_relative.as_posix(), "F118 final source bundle")
        != source_bundle_path
        or final_sha256 != source_bundle_sha256
        or final_revisions != source_bundle_revisions
    ):
        raise ValueError("F118 six-part binding does not select the current source bundle")
    paths = {
        "evidence": root / F118_RELATIVE,
    ...
    loaded: dict[str, tuple[dict[str, object], str]] = {}
    for key, path in paths.items():
        relative, expected = input_binding(retained[key], f"F118 {key} binding")
        if root_path(root, relative.as_posix(), f"F118 {key}") != path:
            raise ValueError(f"F118 {key} is not the exact retained path")
        value = parse_json(
            tracker.read(path, expected, f"F118 {key}", expected_mode=0o444),
            f"F118 {key}",
        )
        if not isinstance(value, dict):
            raise ValueError(f"F118 {key} must be an object")
        loaded[key] = (value, expected)
    ...
    generated = parse_utc_timestamp(retained_evidence["generated_utc"], "F118 generation")
    if (
        retained_evidence["schema_version"] != 1
        or retained_evidence["record_type"]
        != "stage-i-current-source-authority-supersession-evidence"
    ...
    historical_f116 = parse_historical_f116_source_authority(
        root,
    ...
    current_head = require_revision(current_bundle["head"], "F118 current source bundle head")
    if (
    ...
    expected_predecessor = dict(historical_f116["current_source_bundle"])
    ...
    for key, review_kind, decision in (
        ...
        if (
            ...
            or retained_review["checkpoint"] != "F-118"
            ...
            or retained_review["published_f118"]
            != {"path": str(paths["evidence"]), "sha256": evidence_sha256}
            or retained_review["verified"] != verified
        ):
            raise ValueError(f"F118 {key} identity differs")
        agent_id = require_nonempty_string(reviewer["agent_id"], f"F118 {key} reviewer")
        require_nonempty_string(reviewer["identity"], f"F118 {key} reviewer identity")
        if agent_id in reviewers:
    ...
    published = parse_utc_timestamp(retained_audit["published_utc"], "F118 publication")
    if (
        retained_audit["schema_version"] != 1
        or retained_audit["record_type"]
        != "stage-i-current-source-authority-supersession-publication-audit"
        or retained_audit["checkpoint"] != "F-118"
        or retained_audit["execution_epoch"] != EXECUTION_EPOCH
        or retained_audit["authority_and_enforcement"] != F118_AUTHORIZATION
        or retained_audit["historical_f116_authority"]
        != {
            "evidence_sha256": historical_f116["evidence_sha256"],
            "publication_audit_sha256": historical_f116["publication_audit_sha256"],
    ...
    require_declared_binding(
        retained_audit["artifact"], paths["evidence"], evidence_sha256, "F118 publication artifact"
    )
    reviews = require_exact_keys(
        ...
        "F118 publication reviews",
    )
    if reviews["reviews_bind_exact_published_f118_sha256"] != evidence_sha256:
        raise ValueError("F118 publication audit independent-review binding differs")
    require_declared_binding(
    ...
    published_current = require_exact_keys(
        ...
        "F118 published current source bundle",
    )
    if published_current != {
        ...
        raise ValueError("F118 published current source bundle binding differs")
    ...
    published_predecessor = require_exact_keys(
        ...
        {"path", "sha256", "mode", "links", "head", "role", "selected_as_current"},
        "F118 published predecessor source bundle",
    )
    if published_predecessor != {
        "path": str(root / Path(str(predecessor_bundle["path"]))),
        ...
        raise ValueError("F118 published predecessor source bundle binding differs")

    historical_audit_path = root / F113_PUBLICATION_AUDIT_RELATIVE
    adoption = None
    if not os.path.lexists(historical_audit_path):
        adoption = {
            "status": "controlled-exact-transitive-F115-F116-F118-supersession",
            ...
            "f115_evidence_sha256": historical_f116["historical_f115"]["evidence_sha256"],
            "f116_publication_audit_sha256": historical_f116["publication_audit_sha256"],
            "f118_publication_audit_sha256": audit_sha256,
        }

    binding = {
        "checkpoint": "F-118",
        "evidence": {
            "path": F118_RELATIVE.as_posix(),
    ...
    if binding != retained:
        raise ValueError("F118 current source authority binding is not canonical")
...
def authenticate_f117_failed_recost_attempt(
    root: Path, tracker: InputTracker
) -> dict[str, object]:
    """Authenticate the exact unpromoted F117 attempt superseded only by F119."""

    allowed_names = {
        relative.name for relative in F117_FAILED_ATTEMPT_RELATIVES.values()
    }
    with absolute_descriptor(
        root / "accounting", "accounting directory", flags=os.O_RDONLY | os.O_DIRECTORY
    ) as descriptor:
        unexpected = sorted(
            entry.name
            for entry in os.scandir(descriptor)
            if entry.name.startswith(
                f"mks24_stage_i_{EXECUTION_EPOCH_SLUG}_F117_"
            )
            and entry.name not in allowed_names
        )
    if unexpected:
        raise ValueError(
            "F119 legacy F114 supersession requires no F117 artifact, review, audit, "
            f"or extra prerequisite: {unexpected}"
        )
    for relative in F117_FORBIDDEN_PROMOTION_RELATIVES:
        if os.path.lexists(root / relative):
            raise ValueError(
                "F119 legacy F114 supersession requires no F117 artifact, review, or audit"
            )
    ...
    for key, relative in F117_FAILED_ATTEMPT_RELATIVES.items():
        path = root / relative
        expected = F117_FAILED_ATTEMPT_SHA256[key]
        payloads[key] = tracker.read(
            path,
            expected,
            f"retained failed F117 {key}",
            expected_mode=0o644,
        )
        values[key] = parse_json(payloads[key], f"retained failed F117 {key}")

    packet = parse_draft_packet(payloads["packet"])
    request = parse_request(payloads["request"])
    packet_paths = draft_output_paths(root, packet["checkpoint"], packet["artifact_name"])
    if (
        packet["checkpoint"] != "F-117"
        or request["checkpoint"] != "F-117"
        or packet_paths["packet"] != root / F117_FAILED_ATTEMPT_RELATIVES["packet"]
        or packet_paths["request"] != root / F117_FAILED_ATTEMPT_RELATIVES["request"]
        or packet_paths["reconciliation"]
        != root / F117_FAILED_ATTEMPT_RELATIVES["reconciliation"]
        or packet_paths["storage"] != root / F117_FAILED_ATTEMPT_RELATIVES["storage"]
    ):
        raise ValueError("retained failed F117 attempt namespace differs")
    ...
    for key in (
        ...
        if packet[key] != request[key]:
            raise ValueError(f"retained failed F117 packet/request {key} differs")
    packet_inputs = packet["inputs"]
    request_inputs = request["inputs"]
    assert isinstance(packet_inputs, dict)
    assert isinstance(request_inputs, dict)
    if set(packet_inputs) != set(request_inputs):
        raise ValueError("retained failed F117 packet/request input schemas differ")
    live_keys = {
        "reconciliation",
        "ledger",
        "reservations",
    ...
    if any(packet_inputs[key] is not None for key in live_keys):
        raise ValueError("retained failed F117 packet contains live prerequisite bindings")
    if any(
        packet_inputs[key] != request_inputs[key]
        for key in packet_inputs
        if key not in live_keys
    ):
        raise ValueError("retained failed F117 packet/request authority bindings differ")
    ...
    if (
        ...
        raise ValueError("retained failed F117 predecessor or source authority differs")
    for key in ("reconciliation", "storage_evidence"):
        expected_name = "reconciliation" if key == "reconciliation" else "storage"
        expected_binding = {
            "path": F117_FAILED_ATTEMPT_RELATIVES[expected_name].as_posix(),
            "sha256": F117_FAILED_ATTEMPT_SHA256[expected_name],
        }
        if request_inputs[key] != expected_binding:
            raise ValueError(f"retained failed F117 {key} binding differs")

    reconciliation = parse_reconciliation(values["reconciliation"], root)
    if reconciliation["consistent"] is not True or reconciliation["issues"] != []:
        raise ValueError("retained failed F117 reconciliation is not clean")
    storage = require_exact_keys(
        ...
        "retained failed F117 storage evidence",
    )
    if (
        ...
        raise ValueError("retained failed F117 storage evidence identity differs")
    require_integer(storage["available_bytes"], "retained failed F117 available bytes", minimum=1)
    require_integer(
        storage["retained_stage_i_bytes"],
        "retained failed F117 retained bytes",
        minimum=0,
    )
    require_integer(
        storage["required_safety_bytes"],
        "retained failed F117 storage safety bytes",
        minimum=1,
    )
    require_integer(
        storage["projected_authorized_wave_growth_bytes"],
        "retained failed F117 projected storage growth",
        minimum=1,
    )
    require_sha256(
        storage["profile_projections_sha256"],
        "retained failed F117 storage projection SHA-256",
    )
    return {
        "checkpoint": "F-117",
        **{
...
def parse_predecessor_recost(
    ...
    if legacy_bootstrap:
        exact_f117 = (
            ...
            and current_artifact_name
            == "mks24_stage_i_E03_forcing_policy_F117_recost_evidence.json"
        )
        exact_f119 = (
            current_checkpoint == "F-119"
            and current_artifact_name
            == "mks24_stage_i_E03_forcing_policy_F119_recost_evidence.json"
        )
        if exact_f119:
            failed_f117 = authenticate_f117_failed_recost_attempt(root, tracker)
        elif not exact_f117:
            raise ValueError(
                "legacy F114 bootstrap is reserved exactly for first schema-2 F-117 "
                "or authenticated failed-F117 supersession by F-119"
            )
    ...
    if legacy_bootstrap:
        retained = {
            ...
            "bootstrap": (
                ...
                if failed_f117 is not None
                else "exact-retained-legacy-F114-once"
            ),
        }
        if failed_f117 is not None:
            retained["superseded_failed_attempt"] = failed_f117
        return retained
    if not publications:
...
def authenticate_predecessor_recost_input(
    root: Path,
    ...
    """Authenticate one request's exact predecessor publication chain."""
...
def build_payload(
    ...
    *,
    ...
    (
        ...
        predecessor_review_sha256,
        predecessor_audit_sha256,
    ) = authenticate_predecessor_recost_input(
        root,
        inputs,
        request_timestamp,
        artifact_name,
...
def directory_namespace_binding(profile: os.stat_result) -> tuple[int, ...]:
    """Return the mutation-relevant namespace profile of one open directory."""
...
def require_directory_namespace_unchanged(
    directory: int,
    expected: os.stat_result,
    label: str,
) -> os.stat_result:
    """Close one finite namespace read at its final descriptor observation.

    This detects mutations before or during the final descriptor read.  A
    same-UID raw syscall issued after that observation is outside the
    process-enforced boundary; cooperating Stage I writers are excluded by
    the authenticated mutation lock.
    """

    observed = os.fstat(directory)
    require_directory_profile(observed, label)
    if (
        not same_inode(observed, expected)
        or directory_namespace_binding(observed)
        != directory_namespace_binding(expected)
    ):
        raise ValueError(f"{label} changed during namespace closure")
    return observed
...
def closed_transaction_private_names(
    directory: int,
    transaction: PublicationTransaction,
    label: str,
) -> list[str]:
    """Return one transaction's private names under an exact namespace closure."""

    before = os.fstat(directory)
    require_directory_profile(before, f"{label} private namespace")
    retained = transaction_private_names(directory, transaction)
    require_directory_namespace_unchanged(
        directory, before, f"{label} private namespace"
    )
    return retained
...
def require_no_transaction_private_remnants(
    ...
    retained = closed_transaction_private_names(directory, transaction, label)
    if retained:
        raise ValueError(
...
def recover_linked_publication(
    ...
    *,
    ...
    private_names = closed_transaction_private_names(directory, transaction, label)
    ...
    *,
    ...
    with absolute_descriptor(
        ...
        if stat.S_IMODE(observed.st_mode) == mode and observed.st_nlink == 2:
            ...
            private_names = closed_transaction_private_names(
                directory, transaction, label
            )
            if len(private_names) != 1:
                raise ValueError(
...
def classify_draft_trio_target(
    ...
    if stat.S_IMODE(observed.st_mode) == entry.mode and observed.st_nlink == 1:
        ...
        require_no_transaction_private_remnants(
            directory, entry.transaction, entry.label
        )
...
def recover_draft_trio_rollback_quarantine(
    ...
    entry: DraftTrioPublicationEntry,
    mutation_lock: MutationLock | None,
    mutation_authority: Callable[[], None] | None = None,
) -> None:
    """Restore one exact linked canonical name from a reserved rollback slot.

    This is a recovery-only mutation while the canonical namespace is
    intentionally partial.  The immediate authority callback authenticates
    the source boundary; the caller must close the exact namespace before any
...
def install_draft_trio_entry(
    ...
    *,
    ...
    if publication_barrier is not None:
        publication_barrier()
    elif mutation_authority is not None:
        mutation_authority()
    try:
        moved = move_bound_name_noreplace(
            directory,
...
def rollback_draft_trio_entries(
    ...
    Rollback is a recovery-only authority boundary.  It cannot authenticate a
    normal request-last namespace while deliberately retracting that
    namespace, and every caller raises after the rollback attempt.
    """
...
def publish_draft_prerequisite_trio(
    ...
    *,
    mutation_lock: MutationLock | None = None,
    publication_barrier: Callable[[], None] | None = None,
    publication_state_barrier: Callable[
        [tuple[tuple[Path, bytes, int, str, os.stat_result], ...]], None
    ]
    | None = None,
    mutation_authority: Callable[[], None] | None = None,
) -> None:
    ...
    with absolute_descriptor(
        ...
        def private_publication_state(
        ) -> tuple[tuple[Path, bytes, int, str, os.stat_result], ...]:
            """Return every exact in-flight private inode owned by this trio."""
            ...
            for entry in entries:
                if entry.private_name is None or entry.private_identity is None:
                    continue
                try:
                    profile = read_bound_exact_file(
                        directory,
                        entry.private_name,
                        entry.payload,
                        entry.mode,
                        f"{entry.label} in-flight private publication",
                        expected_identity=entry.private_identity,
                    )
                except FileNotFoundError:
                    continue
                retained.append(
                    (
                        entry.path.with_name(entry.private_name),
                        entry.payload,
                        entry.mode,
                        f"{entry.label} in-flight private publication",
                        profile,
        ...
        def close_publication_state() -> None:
            if publication_state_barrier is not None:
                publication_state_barrier(private_publication_state())
            elif publication_barrier is not None:
                publication_barrier()
            elif mutation_authority is not None:
                mutation_authority()
        ...
        def forward_complete() -> None:
            for entry in entries:
                close_publication_state()
                install_draft_trio_entry(
                    directory,
                    parent_profile,
                    entry,
                    mutation_lock,
                    publication_barrier=close_publication_state,
                    mutation_authority=mutation_authority,
                )
                close_publication_state()
            for entry in entries:
...
def retain_exact_publication_for_retry(
    ...
    *,
    ...
    Retry retention is a recovery-only authority boundary invoked while
    unwinding a failed action closure; callers do not return action success.
    """
    ...
    *,
    ...
    with absolute_descriptor(
        ...
        try:
            ...
            if publication_state_barrier is not None:
                publication_state_barrier(
                    (
                        (
                            path.with_name(private_name),
                            payload,
                            mode,
                            f"{label} in-flight private publication",
                            finalized_profile,
                        ),
                    )
                )
...
def preflight_draft_authority_and_predecessor(
    root: Path,
    repository: Path,
    packet: dict[str, object],
    tracker: InputTracker,
) -> None:
    """Authenticate draft authority and predecessor before publishing prerequisites."""
    ...
    artifact_name = require_nonempty_string(
        packet["artifact_name"], "recost draft artifact name"
    )
    checkpoint = require_safe_id(packet["checkpoint"], "recost draft checkpoint")
    ...
    if root == DEFAULT_ROOT and ceiling_sha256 != F113_CANONICAL_SHA256:
        raise ValueError("canonical F113 ceiling digest differs from the reviewed digest")
    tracker.authenticate(
        ceiling_path, ceiling_sha256, "ceiling evidence", expected_mode=0o644
    )
    parse_current_source_authority(
        root,
        repository,
        inputs["source_authority"],
        bundle_path,
...
@dataclass(frozen=True)
class F119LiveSourceAuthority:
    """Retain the exact F118-selected live HEAD and seven-tool contract."""

    head: str
    tools: tuple[tuple[str, str, int], ...]

    def authenticate(self, repository: Path, label: str) -> None:
        """Rebind live HEAD, clean selected paths, modes, and bytes."""

        relatives = [relative for relative, _, _ in self.tools]

        def require_head() -> None:
            observed = git_run(
                repository, ["rev-parse", "HEAD"], capture_output=True
            )
            if observed.returncode or observed.stdout.decode().strip() != self.head:
                raise ValueError(
                    "live repository HEAD differs from the exact F118-selected authority"
                )

        def require_clean_selected_paths() -> None:
            for arguments in (
                ["diff", "--quiet", "--", *relatives],
                ["diff", "--cached", "--quiet", "--", *relatives],
            ):
                if git_run(repository, arguments).returncode:
                    raise ValueError(
                        f"{label} live F118-selected tools are not committed and clean"
                    )

        require_head()
        require_clean_selected_paths()
        for relative, digest, mode in self.tools:
            path = repository / relative
            with absolute_descriptor(
                path, f"{label} live F118-selected tool {relative}", flags=os.O_RDONLY
            ) as descriptor:
                require_regular_profile(
                    os.fstat(descriptor),
                    f"{label} live F118-selected tool {relative}",
                    expected_mode=mode,
                )
                if sha256_descriptor(descriptor) != digest:
                    raise ValueError(
                        f"{label} live F118-selected tool {relative} checksum differs"
                    )
        require_clean_selected_paths()
        require_head()


def f119_live_source_authority(
    root: Path,
    repository: Path,
    source_authority: object,
) -> F119LiveSourceAuthority:
    """Authenticate and retain the compact live F118 source closure."""
    ...
    authority = require_exact_keys(
        ...
        "F119 action F118 source authority",
    )
    if authority["checkpoint"] != "F-118":
        raise ValueError("F119 action source authority is not exact F118")
    evidence_relative, evidence_sha256 = input_binding(
        authority["evidence"], "F119 action F118 evidence binding"
    )
    evidence_path = root_path(
        root, evidence_relative.as_posix(), "F119 action F118 evidence"
    )
    if evidence_path != root / F118_RELATIVE:
        raise ValueError("F119 action F118 evidence is not the exact retained path")
    tracker = InputTracker()
    evidence = parse_json(
        tracker.read(
            evidence_path,
            evidence_sha256,
            "F119 action F118 evidence",
            expected_mode=0o444,
        ),
        "F119 action F118 evidence",
    )
    ...
    implementation = require_exact_keys(
        ...
        "F119 action F118 implementation",
    )
    current = require_exact_keys(
        implementation["current_source_bundle"],
        {
        ...
        "F119 action F118 current source bundle",
    )
    head = require_revision(current["head"], "F119 action F118 current source head")
    tools = authenticate_f118_committed_tools(
        implementation["committed_tools"], repository, head
    )
    tracker.reauthenticate_all()
    retained_tools = tuple(
        (
            str(tool["path"]),
            require_sha256(tool["sha256"], "F119 action selected-tool SHA-256"),
            int(str(tool["mode"]), 8),
        )
        for tool in tools
    )
    authority_closure = F119LiveSourceAuthority(head=head, tools=retained_tools)
    authority_closure.authenticate(repository, "F119 action initialization")
...
@dataclass
class F119NamespaceFile:
    """One exact required direct-child F119 namespace binding."""
...
def require_f119_namespace_variants(
    root: Path,
    mutation_lock: MutationLock | None,
    label: str,
    variants: tuple[tuple[F119NamespaceFile, ...], ...],
    *,
    authority_barrier: Callable[[], None] | None = None,
) -> tuple[F119NamespaceFile, ...]:
    """Close one complete exact F119 namespace variant and live authority."""

    if not variants:
        raise ValueError(f"{label}: F119 namespace closure requires an expected variant")
    accounting = root / "accounting"
    ...
    retirement_prefix = f".{paths['packet'].name}.retired.sha256-"
    normalized_variants: list[tuple[F119NamespaceFile, ...]] = []
    for variant in variants:
        names = [expected.path.name for expected in variant]
        if (
            len(set(names)) != len(names)
            or any(expected.path.parent != accounting for expected in variant)
        ):
            raise ValueError(f"{label}: F119 namespace expectation is not direct and unique")
        normalized_variants.append(tuple(variant))
    ...
    with absolute_descriptor(
        ...
        before = os.fstat(directory)
        require_directory_profile(before, f"{label} accounting namespace")
        names = tuple(sorted(entry.name for entry in os.scandir(directory)))
        present = tuple(
            name
            for name in names
            if (
                name.startswith(namespace_prefix)
                or name.startswith(retirement_prefix)
                or PUBLICATION_PRIVATE_NAME_PATTERN.fullmatch(name) is not None
            )
        )
        matching = [
            variant
            for variant in normalized_variants
            if tuple(sorted(expected.path.name for expected in variant)) == present
        ]
        if len(matching) != 1:
            raise ValueError(
                f"{label}: complete expected F119 namespace differs; observed {present}"
            )
        selected = matching[0]
        retained_profiles: list[tuple[F119NamespaceFile, os.stat_result]] = []
        ...
        require_parent_path_bound(
            accounting, directory, before, f"{label} accounting namespace"
        )
        authenticate_mutation_lock(mutation_lock)
        if authority_barrier is not None:
            authority_barrier()
        closing_names = tuple(sorted(entry.name for entry in os.scandir(directory)))
        if closing_names != names:
            raise ValueError(f"{label}: accounting directory listing changed")
        closing_profiles: list[tuple[F119NamespaceFile, os.stat_result]] = []
        for expected, profile in retained_profiles:
            closing = read_bound_exact_file(
                directory,
                expected.path.name,
                expected.payload,
                expected.mode,
            ...
            if file_security_content_binding(closing) != file_security_content_binding(
                profile
            ):
                raise ValueError(f"{expected.label} changed during F119 closure")
            closing_profiles.append((expected, closing))
        require_parent_path_bound(
            accounting, directory, before, f"{label} accounting namespace"
        )
        authenticate_mutation_lock(mutation_lock)
        require_directory_namespace_unchanged(
            directory, before, f"{label} accounting namespace"
        )
    for expected, profile in closing_profiles:
...
def require_f119_install_namespace_barrier(
    ...
    *,
    ...
    """Require one complete exact install/retirement F119 namespace state."""

    try:
        return require_f119_namespace_variants(
            root,
            mutation_lock,
            label,
            variants,
        ...
        paths = draft_output_paths(root, F119_CHECKPOINT, F119_ARTIFACT_NAME)
        if os.path.lexists(paths["request"]):
            raise ValueError(
                f"{label}: F119 request is the last commit marker; "
                "refusing draft-packet installation or supersession"
            ) from error
        if os.path.lexists(paths["reconciliation"]) or os.path.lexists(paths["storage"]):
            raise ValueError(f"{label}: F119 draft prerequisite namespace is partial") from error
        raise ValueError(
            f"{label}: exact F119 pre-request namespace contains an orphan marker, "
            "managed publication-private remnant, or differs from the complete "
            "required state"
        ) from error
...
def require_f119_draft_namespace_barrier(
    ...
    *,
    ...
    """Require one complete exact request-last F119 draft namespace state."""
    ...
    try:
        ...
        raise ValueError(
            f"{label}: exact F119 request-last namespace contains an orphan marker, "
            "managed publication-private remnant, or differs from the required prefix"
        ) from error


@dataclass(frozen=True)
class F119ActionClosure:
    """Bind every F119 canonical action to exact namespace and live F118 source."""

    root: Path
    repository: Path
    mutation_lock: MutationLock | None
    live_source: F119LiveSourceAuthority

    def authenticate_authority(self) -> None:
        """Rebind the live F118-selected HEAD/tools immediately before mutation."""

        authenticate_mutation_lock(self.mutation_lock)
        self.live_source.authenticate(self.repository, "F119 action closure")
        authenticate_mutation_lock(self.mutation_lock)

    def close_install(
        self,
        label: str,
        *variants: tuple[F119NamespaceFile, ...],
    ) -> tuple[F119NamespaceFile, ...]:
        """Close one exact install/retirement state plus live source authority."""

        return require_f119_install_namespace_barrier(
            self.root,
            self.mutation_lock,
            label,
            variants=tuple(variants),
            authority_barrier=self.authenticate_authority,
        )
    ...
    def close_draft(
        ...
        *variants: tuple[F119NamespaceFile, ...],
        ...
        """Close one exact request-last state plus live source authority."""

        return require_f119_draft_namespace_barrier(
            self.root,
            self.mutation_lock,
            label,
            variants=tuple(variants),
    ...
    def close_install_publication_state(
        self,
        label: str,
        private_state: tuple[tuple[Path, bytes, int, str, os.stat_result], ...],
        *variants: tuple[F119NamespaceFile, ...],
    ) -> tuple[F119NamespaceFile, ...]:
        """Close one install state plus exact authenticated in-flight privates."""

        private_expectations = tuple(
            F119NamespaceFile(path, payload, mode, item_label, identity)
            for path, payload, mode, item_label, identity in private_state
        )
        return self.close_install(
            label, *(variant + private_expectations for variant in variants)
        )

    def close_draft_publication_state(
        self,
        label: str,
        private_state: tuple[tuple[Path, bytes, int, str, os.stat_result], ...],
        *variants: tuple[F119NamespaceFile, ...],
    ) -> tuple[F119NamespaceFile, ...]:
        """Close one draft state plus exact authenticated in-flight privates."""

        private_expectations = tuple(
            F119NamespaceFile(path, payload, mode, item_label, identity)
            for path, payload, mode, item_label, identity in private_state
        )
        return self.close_draft(
            label, *(variant + private_expectations for variant in variants)
        )


def f119_action_closure(
    root: Path,
    repository: Path,
    packet: dict[str, object],
    mutation_lock: MutationLock | None,
) -> F119ActionClosure:
    """Construct one exact action-immediate F119 namespace/source closure."""

    inputs = packet["inputs"]
    assert isinstance(inputs, dict)
    live_source = f119_live_source_authority(
        root, repository, inputs["source_authority"]
    )
    helper = require_exact_keys(
        inputs["stage_i_helper"], {"revision", "sha256"}, "F119 action Stage I helper"
    )
    selected_helper = next(
        item
        for item in live_source.tools
        if item[0] == STAGE_I_RELATIVE.as_posix()
    )
    if helper != {"revision": live_source.head, "sha256": selected_helper[1]}:
        raise ValueError("F119 action live source binding differs from the packet")
    return F119ActionClosure(root, repository, mutation_lock, live_source)
...
def restore_retired_f119_packet(
    ...
    Restoration is a recovery-only authority boundary.  It runs while the
    failed action's namespace is noncanonical, and every caller re-raises
    instead of returning action success.
    """
...
def retire_stale_f119_packet(
    ...
    mutation_lock: MutationLock | None,
    action_closure: F119ActionClosure | None = None,
) -> F119NamespaceFile | None:
    """Retire one authenticated stale F119 packet before no-clobber replacement."""

    def close_install(
        label: str, *variants: tuple[F119NamespaceFile, ...]
    ) -> tuple[F119NamespaceFile, ...]:
        if action_closure is not None:
            return action_closure.close_install(label, *variants)
        return require_f119_install_namespace_barrier(
            root, mutation_lock, label, variants=tuple(variants)
        )

    mutation_authority = (
        action_closure.authenticate_authority if action_closure is not None else None
    )
    try:
        with absolute_descriptor(
        ...
        close_install("F119 absent-packet post-retirement barrier", ())
        return None
    payload = b"".join(chunks)
    packet_expectation = F119NamespaceFile(
        target,
        payload,
        0o644,
        "canonical F119 draft packet",
        profile,
    )
    if payload == candidate_payload:
        close_install(
            "F119 exact-packet post-retirement barrier", (packet_expectation,)
        )
        return None
    tracker = InputTracker()
    ...
    retired_expectation = F119NamespaceFile(
        retired,
        payload,
        0o644,
    ...
    with absolute_descriptor(
        ...
        authenticate_mutation_lock(mutation_lock)
        close_install(
            "F119 action-immediate pre-retirement barrier",
            (packet_expectation,),
        )
        moved = move_bound_name_noreplace(
            directory,
        ...
        retired_expectation.identity = moved
        require_parent_path_bound(
            target.parent, directory, parent_profile, "F119 packet retirement"
        )
        authenticate_mutation_lock(mutation_lock)
        try:
            close_install(
                "F119 action-immediate post-retirement barrier",
                (retired_expectation,),
            )
        except BaseException:
            restore_retired_f119_packet(
...
def locked_install_draft_packet(
    ...
    if expected_checkpoint == F119_CHECKPOINT:
        ...
        validate_f119_draft_packet(root, repository, packet, tracker)
        tracker.reauthenticate_all()
        action_closure = f119_action_closure(
            root, repository, packet, mutation_lock
        )
    else:
        action_closure = None
    target = paths["packet"]
    packet_expectation = F119NamespaceFile(
        ...
        packet_payload,
        0o644,
        "managed F119 recost request draft packet",
    )
    retired = None
    if expected_checkpoint == F119_CHECKPOINT:
        ...
        try:
            assert action_closure is not None
            action_closure.close_install(
                "F119 pre-publication barrier",
                (() if retired is None else (retired,)),
                (
                    (packet_expectation,)
                    if retired is None
                    else (packet_expectation, retired)
                ),
            )
        except BaseException:
            if retired is not None:
                restore_retired_f119_path(
                    target,
                    retired.path,
                    mutation_lock,
                    action_closure.authenticate_authority,
                )
            raise
    publication_label = f"managed {expected_checkpoint} recost request draft packet"
    f119_pre_or_post_publication_barrier: Callable[[], None] | None = None
    f119_publication_state_barrier: (
        Callable[
            [tuple[tuple[Path, bytes, int, str, os.stat_result], ...]], None
        ]
        | None
    ...
    if action_closure is not None:
        f119_pre_or_post_publication_barrier = lambda: action_closure.close_install(
            "F119 action-immediate publication barrier",
            (() if retired is None else (retired,)),
            (
                (packet_expectation,)
                if retired is None
        ...
        f119_publication_state_barrier = (
            lambda private_state: action_closure.close_install_publication_state(
                "F119 action-immediate in-flight publication barrier",
                private_state,
                (() if retired is None else (retired,)),
                (
                    (packet_expectation,)
    ...
    try:
        created = write_exact_or_verify(
            ...
            mutation_lock=mutation_lock,
            publication_barrier=f119_pre_or_post_publication_barrier,
            publication_state_barrier=f119_publication_state_barrier,
            mutation_authority=(
                action_closure.authenticate_authority
                if action_closure is not None
                else None
    ...
    try:
        ...
        if expected_checkpoint == F119_CHECKPOINT:
            assert action_closure is not None
            action_closure.close_install(
                "F119 final install return barrier",
                (
                    (packet_expectation,)
                    if retired is None
                    else (packet_expectation, retired)
        ...
        if expected_checkpoint == F119_CHECKPOINT and created:
            ...
            retain_exact_publication_for_retry(
                ...
                label=publication_label,
                mutation_lock=mutation_lock,
                mutation_authority=action_closure.authenticate_authority,
            )
        if retired is not None:
            assert action_closure is not None
            restore_retired_f119_path(
        ...
        json.dumps(
            {
                ...
                "authenticated_retired_predecessor": (
                    str(retired.path) if retired is not None else None
                ),
...
def locked_draft_request(
    ...
    action_closure: F119ActionClosure | None = None
    packet_expectation: F119NamespaceFile | None = None
    if packet["checkpoint"] == F119_CHECKPOINT:
        ...
        packet_expectation = F119NamespaceFile(
            ...
            packet_payload,
            0o644,
            "canonical F119 recost request draft packet",
        )
        action_closure.close_draft(
            "F119 draft-request initial closure", (packet_expectation,)
        )
    else:
        preflight_draft_authority_and_predecessor(root, repository, packet, tracker)
    ...
    reconciliation_expectation = F119NamespaceFile(
        paths["reconciliation"],
        reconcile_payload,
        0o644,
        "draft reconciliation evidence",
    )
    storage_expectation = F119NamespaceFile(
        paths["storage"],
        storage_payload,
        0o644,
        "draft storage evidence",
    )
    request_expectation = F119NamespaceFile(
        paths["request"],
        request_payload,
        0o644,
        "schema-2 recost request draft",
    )
    f119_draft_variants: tuple[tuple[F119NamespaceFile, ...], ...] = ()
    if action_closure is not None:
        assert packet_expectation is not None
        f119_draft_variants = (
            (packet_expectation,),
            (packet_expectation, reconciliation_expectation),
            (packet_expectation, reconciliation_expectation, storage_expectation),
            (
                packet_expectation,
        ...
        f119_publication_barrier = lambda: action_closure.close_draft(
            "F119 draft-request publication barrier", *f119_draft_variants
        )
        f119_publication_state_barrier = (
            lambda private_state: action_closure.close_draft_publication_state(
                "F119 draft-request in-flight publication barrier",
                private_state,
                *f119_draft_variants,
            )
        )
    ...
    if action_closure is not None:
        action_closure.close_draft(
            "F119 final draft-request return closure",
            f119_draft_variants[-1],
        )
    generation_command = [
        sys.executable,
        str(source_path),
