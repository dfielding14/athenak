#!/usr/bin/env python3
"""Post-process scalar_mixing production runs with shell counts and density rebins."""
...
SPECTRUM_NORMALIZATION = "raw_shell_sum"
...
def _spectrum_payload(
    ...
    *,
    ...
    return {
        ...
        "spec_raw": np.asarray(shell_sum, dtype=np.float64),
...
def _analyze_case(
    ...
    meta["spectrum_slice_count"] = 1 if dim == 2 else len(base.THREED_SPECTRUM_SLICE_FRACTIONS) * dim
    ...
    if not latest_only:
        ...
        if dim == 2:
            ...
            velocity_entry, velocity_slice_count, velocity_method = _slice_averaged_velocity_entry(
                velocity_snapshot["velx"],
                velocity_snapshot["vely"],
                velocity_snapshot["velz"],
                kmax=spectrum_kmax,
