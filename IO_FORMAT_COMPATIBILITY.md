...
| `.cbin` | Existing shared/per-rank full-volume | Header/schema-compatible | Baseline fixture; ATHDF/XDMF conversion as supported | Frozen readback/reassembly and conversion smoke tests passed; producer and reader reject invalid coarsening factors before unsafe arithmetic |
| `.cbin` | New per-node full-volume | New-reader compatible | Shared/per-node numerical comparison; additive shard-inventory metadata and explicit empty-shard tests | One-node MPI aggregation/equality passed; canonical reader validates complete dense node IDs and valid empty shards |
...
| `.pdf` | New N-dimensional dense | New-reader compatible | Mixed-scale and weighting readback tests | Three-dimensional mass-weighted and four-dimensional scalar-/volume-weighted writer-readback passed; declared V2 preamble, cycle, finite-metadata, retained-allocation, and invalid-scale checks passed |
