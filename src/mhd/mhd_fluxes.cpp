...
TaskStatus MHD::SaveFlux(Driver *pdrive, int stage) {
  ...
  // Save x1-face fluxes
  par_for("mhd_save_flx1", DevExeSpace(), 0, nmb1, ks, ke, js, je, is, ie+1,
  ...
  if (multi_d) {
    par_for("mhd_save_flx2", DevExeSpace(), 0, nmb1, ks, ke, js, je+1, is, ie,
    KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
  ...
  if (three_d) {
    par_for("mhd_save_flx3", DevExeSpace(), 0, nmb1, ks, ke+1, js, je, is, ie,
    KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
