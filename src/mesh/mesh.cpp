...
void Mesh::NewTimeStep(const Real tlim) {
  ...
  if (pmb_pack->phydro != nullptr && pmb_pack->phydro->pvisc != nullptr) {
    pmb_pack->phydro->pvisc->NewTimeStep();
  }
  if (pmb_pack->pmhd != nullptr) {
    if (pmb_pack->pmhd->pvisc != nullptr) pmb_pack->pmhd->pvisc->NewTimeStep();
    if (pmb_pack->pmhd->presist != nullptr) pmb_pack->pmhd->presist->NewTimeStep();
  }
  ...
  if (pmb_pack->ppart != nullptr) {
    dt_cycle = std::min(dt_cycle, (cfl_no)*(pmb_pack->ppart->dtnew) );
  }
