...
void WENOZ(const Real &q_im2, const Real &q_im1, const Real &q_i, const Real &q_ip1,
  ...
  // The commented out code is the original WENO-Z
  ...
  // New WENO-Z from Vassili Mewes, which should be faster
  const Real qs[5] = {q_im2, q_im1, q_i, q_ip1, q_ip2};
  const Real beta_coeff[2]{13. / 12., 0.25};

  Real beta[3];
  beta[0] = beta_coeff[0] * SQR(qs[0] +     qs[2] - 2.0*qs[1]) +
            beta_coeff[1] * SQR(qs[0] + 3.0*qs[2] - 4.0*qs[1]);

  beta[1] = beta_coeff[0] * SQR(qs[1] + qs[3] - 2.0*qs[2]) +
            beta_coeff[1] * SQR(qs[1] - qs[3]);

  beta[2] = beta_coeff[0] * SQR(qs[4] +      qs[2] - 2.0*qs[3]) +
            beta_coeff[1] * SQR(qs[4] + 3.0* qs[2] - 4.0*qs[3]);

  // Rescale epsilon
  const Real epsL = 1.0e-42;

  // WENO-Z+: Acker et al. 2016
  const Real tau_5 = beta[0] - beta[2]; //no need for the abs, as tau_5 is squared below

  Real One_plus_sqr_indicator[3];
  One_plus_sqr_indicator[0] = 1.0 + SQR(tau_5 / (beta[0] + epsL));
  One_plus_sqr_indicator[1] = 1.0 + SQR(tau_5 / (beta[1] + epsL));
  One_plus_sqr_indicator[2] = 1.0 + SQR(tau_5 / (beta[2] + epsL));

  // compute qL_ip1
  // Factor of 1/6 in coefficients of f[] array applied to alpha_sum to reduce divisions
  Real f[3];
  f[0] = ( 2.0*qs[0] - 7.0*qs[1] + 11.0*qs[2]);
  f[1] = (-1.0*qs[1] + 5.0*qs[2] + 2.0 *qs[3]);
  f[2] = ( 2.0*qs[2] + 5.0*qs[3] -      qs[4]);

  Real alpha[3];
  alpha[0] = 0.1*One_plus_sqr_indicator[0];
  alpha[1] = 0.6*One_plus_sqr_indicator[1];
  alpha[2] = 0.3*One_plus_sqr_indicator[2];
  Real alpha_sum = 6.0*(alpha[0] + alpha[1] + alpha[2]);
  ...
  // compute qR_i
  // Factor of 1/6 in coefficients of f[] array applied to alpha_sum to reduce divisions
  f[0] = ( 2.0*qs[4] - 7.0*qs[3] + 11.0*qs[2]);
  f[1] = (-1.0*qs[3] + 5.0*qs[2] + 2.0 *qs[1]);
  f[2] = ( 2.0*qs[2] + 5.0*qs[1] -      qs[0]);

  alpha[0] = 0.1*One_plus_sqr_indicator[2];
  alpha[1] = 0.6*One_plus_sqr_indicator[1];
  alpha[2] = 0.3*One_plus_sqr_indicator[0];
  alpha_sum = 6.0*(alpha[0] + alpha[1] + alpha[2]);
