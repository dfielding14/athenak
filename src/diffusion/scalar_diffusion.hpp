...
//! \brief Contains data and functions that implement scalar diffusion for passive
//! scalars. The diffusive flux is F = -kappa * rho * grad(s), where s is the
...
class ScalarDiffusion {
 public:
  ScalarDiffusion(std::string block, int nscalars, MeshBlockPack *pp,
                  ParameterInput *pin);
  ~ScalarDiffusion();

  // data
  Real dtnew;          // diffusion timestep constraint
  Real kappa_max;      // largest scalar diffusivity, used for timestep control
  DvceArray1D<Real> kappas;  // device-resident diffusivity for each passive scalar
