...
ScalarDiffusion::ScalarDiffusion(std::string block, int nscalars, MeshBlockPack *pp,
                                 ParameterInput *pin) :
  kappas("scalar_diffusivities", nscalars),
  pmy_pack(pp) {
  const Real default_kappa = pin->GetReal(block, "scalar_diffusivity");
  kappa_max = 0.0;
  auto host_kappas = Kokkos::create_mirror_view(kappas);
  for (int n = 0; n < nscalars; ++n) {
    const std::string key = "scalar_diffusivity_" + std::to_string(n);
    const Real kappa_n = pin->GetOrAddReal(block, key, default_kappa);
    if (kappa_n < 0.0) {
      std::cout << "### FATAL ERROR in " << __FILE__
                << " at line " << __LINE__ << std::endl
                << "<" << block << ">/" << key << " must be non-negative" << std::endl;
      std::exit(EXIT_FAILURE);
    }
    host_kappas(n) = kappa_n;
    kappa_max = std::max(kappa_max, kappa_n);
  }
  Kokkos::deep_copy(kappas, host_kappas);
  ...
  for (int m = 0; m < (pp->nmb_thispack); ++m) {
    dtnew = std::min(dtnew, fac*SQR(size.h_view(m).dx1)/kappa_max);
    if (pp->pmesh->multi_d) {
      dtnew = std::min(dtnew, fac*SQR(size.h_view(m).dx2)/kappa_max);
    }
    if (pp->pmesh->three_d) {
      dtnew = std::min(dtnew, fac*SQR(size.h_view(m).dx3)/kappa_max);
    }
  }
}
...
//! The diffusive flux for each scalar n is: F_n = -kappa_n * rho_face * grad(s_n)
...
void ScalarDiffusion::AddScalarDiffusionFlux(const DvceArray5D<Real> &w0,
  ...
  if (nscalars == 0 || kappa_max <= 0.0) return;
  ...
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    ...
    for (int n = 0; n < nscalars_; ++n) {
      ...
      flx1(m,ns,k,j,i) -= kappas_(n) * rho_face * ds_dx;
    }
  });
  ...
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    ...
    for (int n = 0; n < nscalars_; ++n) {
      ...
      // Diffusive flux: F = -kappa * rho * grad(s)
      flx2(m,ns,k,j,i) -= kappas_(n) * rho_face * ds_dy;
    }
  });
  ...
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    ...
    for (int n = 0; n < nscalars_; ++n) {
      ...
      // Diffusive flux: F = -kappa * rho * grad(s)
      flx3(m,ns,k,j,i) -= kappas_(n) * rho_face * ds_dz;
    }
  });
