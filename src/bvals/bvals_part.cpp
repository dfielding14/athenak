...
//----------------------------------------------------------------------------------------
//! \fn void ParticlesBoundaryValues::RegridPrtcl()
//! \brief

TaskStatus ParticlesBoundaryValues::RegridPrtcl() {
  ...
  if (pmy_part->pmy_pack->pmesh->adaptive) {
    par_for("regrid_part",DevExeSpace(),0,(npart-1),KOKKOS_LAMBDA(const int p) {
      ...
      if (not found) {
        // Find nearest mesh block
        Real min_distance = 1e30; // Large initial value
        int nearest_mb = 0;
  
        for (int m = 0; m < nmb; ++m) {
          // Calculate distance from particle to mesh block center
          Real x1_center = 0.5 * (mbsize.d_view(m).x1min + mbsize.d_view(m).x1max);
          Real x2_center = 0.5 * (mbsize.d_view(m).x2min + mbsize.d_view(m).x2max);
          Real x3_center = 0.5 * (mbsize.d_view(m).x3min + mbsize.d_view(m).x3max);
    
          Real dx1 = x1 - x1_center;
          Real dx2 = x2 - x2_center;
          Real dx3 = x3 - x3_center;
          Real distance = dx1*dx1 + dx2*dx2 + dx3*dx3; // squared distance (no sqrt needed)
        ...
        // Assign particle to nearest mesh block
