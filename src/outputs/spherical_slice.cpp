...
void SphericalSliceOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin) {
  ...
  if (i_write) {
    ...
    int npoints = sharded ? CountAsInt(shard_owned_angles.size(), "sphslice point count")
                          : psph->nangles;
    std::ostringstream parameter_dump_stream;
    pin->ParameterDump(parameter_dump_stream);
    std::string parameter_dump = parameter_dump_stream.str();
    std::size_t parameter_dump_size = parameter_dump.size();
    ...
    write_metadata(metadata_stream);
    std::string metadata = metadata_stream.str();
    std::size_t serialization_bytes =
        CheckedAdd(metadata.size(), parameter_dump_size,
    ...
    CheckedFileWrite(output, parameter_dump.data(), sizeof(char), parameter_dump.size(),
                     temporary_filename, "sphslice parameter dump");
    if (sharded) {
