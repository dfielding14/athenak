...
ParticlePositionsOutput::ParticlePositionsOutput(ParameterInput *pin, Mesh *pm, OutputParameters op) :
...
void ParticlePositionsOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin) {
  ...
  float *data = new float[4*npout_thisrank];
  ...
  for (int p=0; p<npout_thisrank; ++p) {
    ...
    if (pm->multi_d) {
      data[(4*p)+2] = static_cast<float>(outpart_rdata(IPY,p));
    } else {
      data[(4*p)+2] = static_cast<float>(pm->mesh_size.x2min);
    }
    if (pm->three_d) {
      data[(4*p)+3] = static_cast<float>(outpart_rdata(IPZ,p));
    } else {
      data[(4*p)+3] = static_cast<float>(pm->mesh_size.x3min);
    }
  }
  // swap data for this variable into big endian order
  //if (!big_end) {
  //  for (int i=0; i<(4*npout_thisrank); ++i) { Swap4Bytes(&data[i]); }
  //}
  ...
  // Write particle positions
  {
    ...
    std::size_t myoffset=header_offset + 4*rank_offset[global_variable::my_rank]*datasize;
    // collective writes for minimum number of particles across ranks
    if (partfile.Write_any_type_at_all(&(data[0]),4*npout_min,myoffset,"float")
          != 4*npout_min) {
      std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
          << std::endl << "particle data not written correctly to particle position dump file, "
          << "file is broken." << std::endl;
      exit(EXIT_FAILURE);
    }
    // individual writes for remaining particles on each rank
    myoffset += datasize*4*npout_min;
    int nremain = pm->nprtcl_thisrank - npout_min;
    if (nremain > 0) {
      if (partfile.Write_any_type_at_all(&(data[4*npout_min]),4*nremain,myoffset,"float")
            != 4*nremain) {
        std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
            << std::endl << "particle data not written correctly to particle position dump file, "
            << "file is broken." << std::endl;
