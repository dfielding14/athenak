...
TrackedParticleOutput::TrackedParticleOutput(ParameterInput *pin, Mesh *pm,
  ...
  if (pm->pmb_pack->ppart != nullptr && pm->pmb_pack->ppart->IsLagrangianMC()) {
    FatalTrackedParticleError(
        "TrackedParticleOutput assumes drift-particle velocity slots. Use "
        "file_type=prtcl_thermo_history for lagrangian_mc particles.");
  }
  // allocate arrays
...
void TrackedParticleOutput::LoadOutputData(Mesh *pm) {
  ...
  par_for("part_update",DevExeSpace(),0,(npart-1), KOKKOS_LAMBDA(const int p) {
    int tag = pi(PTAG,p);
    if (tag >= 0 && tag < ntrack_local) {
      int index = Kokkos::atomic_fetch_add(&counter(0),1);
      if (index < ntrack_local) {
  ...
  auto host_counter = Kokkos::create_mirror_view_and_copy(HostMemSpace(), counter);
