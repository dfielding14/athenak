...
TrackedParticleOutput::TrackedParticleOutput(ParameterInput *pin, Mesh *pm,
  ...
  if (pm->pmb_pack->ppart != nullptr && pm->pmb_pack->ppart->IsFluxTracer()) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              ...
              << "slots. Use file_type=prtcl_thermo_history for flux tracer particles."
              << std::endl;
...
void TrackedParticleOutput::LoadOutputData(Mesh *pm) {
  ...
  HostArray1D<int> host_counter("tracked_particle_count_host", 1);
...
//  std::size_t header_offset = 0;
  ...
  for (int p=0; p<npout_min; ++p) {
    ...
    std::size_t myoffset =
        header_offset + 6*sizeof(float)*static_cast<std::size_t>(outpart(p).tag);
