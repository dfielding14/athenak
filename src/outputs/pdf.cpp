...
void ValidatePDFResult(const HostArray1D<Real> &result, int total_bins,
                       const char *context) {
  for (int n = 0; n < total_bins; ++n) {
    if (!output_diagnostics::IsFinite(result(n))) {
...
void PDFOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin) {
  const char *validation_context = "Local PDF result";
#if MPI_PARALLEL_ENABLED
  if (out_params.shard_mode == FileShardMode::shared) {
    validation_context = "Reduced shared PDF result";
  } else if (IsNodeSharded(out_params.shard_mode)) {
    validation_context = "Reduced node PDF result";
  }
#endif
  if (out_params.pdf_legacy_layout) {
    if (global_variable::my_rank == 0) {
      ...
      ValidatePDFResult(values, pdf_data.total_bins, validation_context);
