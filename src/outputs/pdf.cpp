...
//! uint32 layout (0=dense), uint32 ndim, uint32 writer rank,
...
void PDFOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin) {
  ...
  if (global_variable::my_rank == 0) {
    ...
    if (!pdf_data.bins_written) {
      ...
      CheckedPDFPrint(header, temporary_header_name, "layout = dense\n");
      CheckedPDFPrint(header, temporary_header_name, "distribution = shared\n");
    ...
    std::uint64_t count = static_cast<std::uint64_t>(pdf_data.total_bins);
