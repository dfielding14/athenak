...
void ParticleVTKOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin) {
  ...
  for (int n=0; n<(pm->pmb_pack->ppart->nidata); ++n) {
    ...
    if (n == static_cast<int>(PGID)) {
      ...
      msg << std::endl << "SCALARS last_level float" << std::endl
          << "LOOKUP_TABLE default" << std::endl;
    } else if (n == static_cast<int>(PSEEDID)) {
      msg << std::endl << "SCALARS seed_id float" << std::endl
          << "LOOKUP_TABLE default" << std::endl;
    } else {
      msg << std::endl << "SCALARS idata_" << n << " float" << std::endl
          << "LOOKUP_TABLE default" << std::endl;
    }
  ...
  if (pm->pmb_pack->ppart->IsLagrangianMC()) {
    ...
    if (global_variable::my_rank == 0) {
      std::string header = msg.str();
      if (partfile.Write_any_type_at(header.data(), header.size(), header_offset,
                                     "byte") != header.size()) {
        FatalParticleVTKError(
            "Particle creation-time header could not be written.");
      }
    }
    header_offset += msg.str().size();
    ...
    if (partfile.Write_any_type_at_all(data, npout_min, myoffset, "float")
        != npout_min) {
      FatalParticleVTKError(
          "Particle creation-time data could not be written.");
    }
    myoffset += datasize*npout_min;
    int nremain = pm->nprtcl_thisrank - npout_min;
    if (nremain > 0) {
