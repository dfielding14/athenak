...
void ProblemGenerator::SelfGravity(ParameterInput *pin, const bool restart) {
  ...
  std::string fluid_block = (pmbp->pmhd != nullptr) ? "mhd" : "hydro";
  Real four_pi_G = pin->GetOrAddReal("gravity", "four_pi_G", 1.0);
  Real rho0 = pin->GetOrAddReal("problem", "rho0", 1.0);
  Real n_jeans_input = pin->GetOrAddReal("problem", "n_jeans", -1.0);
  std::string eos_type = pin->GetString(fluid_block, "eos");
  Real cs;
  ...
  Real k_jeans = std::sqrt(four_pi_G*rho0)/cs;
  // If n_jeans is specified, set effective four_pi_G to enforce k_J = n_jeans * k
  if (n_jeans_input > 0.0){
    Real lambda_jeans = lambda/n_jeans_input;
    ...
    jw_four_pi_G = four_pi_G;
    k_jeans = n_jeans_input*k_wave;
  }
  Real jeans_ratio = k_jeans/k_wave;
  Real omega2 = SQR(k_wave)*cs*cs - four_pi_G*rho0;
  ...
  std::cout << "  n_Jeans = " << jeans_ratio << " (lambda/lambda_Jeans)" << std::endl;
  ...
  if (pmbp->phydro != nullptr) {
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      Real M = (omega2 < 0.0) ? rho0*(omega/k_wave)*amp*coskx:0.0;
  ...
  // Initialize MHD variables with the same density/momentum perturbation and zero field.
  if (pmbp->pmhd != nullptr) {
    auto &u0 = pmbp->pmhd->u0;
    auto &b0 = pmbp->pmhd->b0;
    auto &bcc0 = pmbp->pmhd->bcc0;

    par_for("jeans_wave_mhd_init", DevExeSpace(), 0, (pmbp->nmb_thispack-1), ks, ke, js, je, is, ie,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      Real &x1min = size.d_view(m).x1min;
      Real &x1max = size.d_view(m).x1max;
      Real x1v = CellCenterX(i-is, indcs.nx1, x1min, x1max);
