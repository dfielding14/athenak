...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart)
{
    ...
    Real max_init_p = pin->GetReal("problem", "max_init_perturb_log2freq");
    Real min_init_p = pin->GetReal("problem", "min_init_perturb_log2freq");
    ...
    KOKKOS_LAMBDA(int m, int j, int i)
    {
        if(mb_bcs.d_view(m,BoundaryFace::inner_x3) == BoundaryFlag::user)
        {
            for (int k=0; k<ng; k++)
            {
                ...
                u0(m,IM2,ks-k-1,j,i) = rho_cold*u0(m,IM2,ks,j,i)/u0(m,IDN,ks,j,i);
                u0(m,IM3,ks-k-1,j,i) = rho_cold*u0(m,IM3,ks,j,i)/u0(m,IDN,ks,j,i);
                u0(m,IEN,ks-k-1,j,i) = pres/gm1 +
                0.5*(SQR(u0(m,IM1,ks-k-1,j,i)) + SQR(u0(m,IM2,ks-k-1,j,i)) +
                SQR(u0(m,IM3,ks-k-1,j,i)))/u0(m,IDN,ks-k-1,j,i);
            }
        }
        ...
        if(mb_bcs.d_view(m,BoundaryFace::outer_x3) == BoundaryFlag::user)
        {
            for (int k=0; k<ng; k++)
            {
                ...
                u0(m,IM2,ke+k,j,i) = rho_hot*u0(m,IM2,ke,j,i)/u0(m,IDN,ke,j,i);
                u0(m,IM3,ke+k,j,i) = rho_hot*u0(m,IM3,ke,j,i)/u0(m,IDN,ke,j,i);
                u0(m,IEN,ke+k,j,i) = pres/gm1 +
                0.5*(SQR(u0(m,IM1,ke+k,j,i)) + SQR(u0(m,IM2,ke+k,j,i)) +
                SQR(u0(m,IM3,ke+k,j,i)))/u0(m,IDN,ke+k,j,i);
            }
        }
    });
}
...
void HistoryOutput(HistoryData *pdata, Mesh *pm)
{
    ...
    for(int i=0;i>TRMLHISTVARS;i++)
    {
        history_sum.the_array[i]=0.0;
    }
    ...
    int &cnt_z_vely_)
    {
        ...
        if(temp_bin == 1) mb_sum.the_array[17] = x3v*vol;
