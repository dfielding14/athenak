...
void GravPotGradient(Real x1, Real x2, Real x3,
    Real G, Real r_s, Real rho_s,
    Real M_gal, Real a_gal,
    Real R200, Real rho_mean,
    Real &dphi_dx1, Real &dphi_dx2, Real &dphi_dx3);
...
namespace {
  ...
  // Constants for initial magnetic field
  Real B0;
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      Real bz = sqrt(2*(profile.GetDensity(250)*profile.GetTemperature(250))/beta);
      ...
      u0(m,IEN,k,j,i) += 0.5*(bz*bz + 0.25*bz*bz);
    });
  }
...
void MassLossSource(Mesh* pm, const Real bdt) {
  ...
  auto &eos = (pmbp->phydro != nullptr)? pmbp->phydro->peos->eos_data : pmbp->pmhd->peos->eos_data;
  Real gm1 = eos.gamma - 1.0;
  auto &profile = profile_reader;
...
void GravitySource(Mesh* pm, const Real bdt) {
  ...
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    ...
    // Use analytic form of gradient for now
    Real f_x1_, f_x2_, f_x3_;
    GravPotGradient(x1v,x2v,x3v,G,r_s,rho_s,m_g,a_g,r_m,rho_m,f_x1_,f_x2_,f_x3_);
...
void GravPotGradient(Real x1, Real x2, Real x3,
  ...
  // Calculate dphi/dr first

  // NFW component derivative
  Real x = r / r_s;
  Real dphi_NFW_dr = -4 * M_PI * G * rho_s * r_s * (1/(x*(1+x)) - log(1+x)/(x*x));

  // Plummer component derivative
  Real dphi_Plummer_dr = G * M_gal * r / pow(r2 + SQR(a_gal), 1.5);

  // Outer component derivative
  Real dphi_Outer_dr = 4 * M_PI * G * rho_mean * ((4.0/3.0) * pow(5 * R200, 1.5) * 0.5 / sqrt(r) + (1.0/3.0) * r);

  // Total derivative with respect to r
  Real dphi_dr = dphi_NFW_dr + dphi_Plummer_dr + dphi_Outer_dr;

  // Apply chain rule: dphi/dxi = (dphi/dr) * (dr/dxi) = (dphi/dr) * (xi/r)
  // If r is tiny, use zero gradient to avoid numerical issues
  if (r < 2*tiny) {
    dphi_dx1 = 0.0;
    dphi_dx2 = 0.0;
    dphi_dx3 = 0.0;
  } else {
    dphi_dx1 = -dphi_dr * (x1/r);
    dphi_dx2 = -dphi_dr * (x2/r);
    dphi_dx3 = -dphi_dr * (x3/r);
  }
}
...
//===========================================================================//
KOKKOS_INLINE_FUNCTION
void SetCoolingFlowStateW(const DvceArray5D<Real> &w0, 
                         int m, int k, int j, int i, 
                         Real x1v, Real x2v, Real x3v, 
                         Real gm1, const ProfileReader &profile) {
    // Calculate radius
  ...
  if (pmbp->phydro != nullptr) {
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      if (mb_bcs.d_view(m, BoundaryFace::inner_x1) == BoundaryFlag::user) {
        ...
        SetCoolingFlowState(u0, m, k, j, i, x1v, x2v, x3v, gm1, profile);
        SetCoolingFlowStateW(w0, m, k, j, i, x1v, x2v, x3v, gm1, profile);
        SetRotation(u0, m, k, j, i, x1v, x2v, x3v, r_c, v_c);
      }
      ...
      if (mb_bcs.d_view(m, BoundaryFace::outer_x1) == BoundaryFlag::user) {
        ...
        SetCoolingFlowStateW(w0, m, k, j, i_out, x1v, x2v, x3v, gm1, profile);
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      if (mb_bcs.d_view(m, BoundaryFace::outer_x2) == BoundaryFlag::user) {
        ...
        SetCoolingFlowStateW(w0, m, k, j_out, i, x1v, x2v, x3v, gm1, profile);
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      if (mb_bcs.d_view(m, BoundaryFace::outer_x3) == BoundaryFlag::user) {
        ...
        SetCoolingFlowStateW(w0, m, k_out, j, i, x1v, x2v, x3v, gm1, profile);
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      if (mb_bcs.d_view(m, BoundaryFace::outer_x1) == BoundaryFlag::user) {
        ...
        b0.x1f(m,k,j,i_out) = bz/2.;
        b0.x2f(m,k,j,i_out) = 0.0;
        b0.x3f(m,k,j,i_out) = bz;

        if (i==ng-1) {b0.x1f(m,k,j,i_out+1) = 0.0;}

        u0(m,IEN,k,j,i_out) += 0.5*(bz*bz + 0.25*bz*bz);
      }
    });
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      if (mb_bcs.d_view(m, BoundaryFace::inner_x2) == BoundaryFlag::user) {
        ...
        u0(m,IEN,k,j,i) += 0.5*bz*bz + 0.125*bz*bz;
      }
      ...
      if (mb_bcs.d_view(m, BoundaryFace::outer_x2) == BoundaryFlag::user) {
        ...
        b0.x1f(m,k,j_out,i) = bz/2.;
        b0.x2f(m,k,j_out,i) = 0.0;
        b0.x3f(m,k,j_out,i) = bz;

        if (j==ng-1) {b0.x1f(m,k,j_out+1,i) = 0.0;}

        u0(m,IEN,k,j_out,i) += 0.5*bz*bz + 0.125*bz*bz;
      }
    });
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      if (mb_bcs.d_view(m, BoundaryFace::outer_x3) == BoundaryFlag::user) {
        ...
        b0.x1f(m,k_out,j,i) = bz/2.;
        b0.x2f(m,k_out,j,i) = 0.0;
        b0.x3f(m,k_out,j,i) = bz;

        if (k==ng-1) {b0.x1f(m,k_out,j,i) = 0.0;}

        u0(m,IEN,k_out,j,i) += 0.5*bz*bz + 0.125*bz*bz;
      }
    });
  } 
