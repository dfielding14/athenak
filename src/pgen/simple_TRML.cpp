...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) 
{
    ...
    glob_shear_vel_thresh = pin->GetOrAddReal("problem", "hist_shear_vel_frac", 0.45)*glob_velocity;
    ...
    if (pmbp->phydro != nullptr) {
        ...
        par_for("TRML_problem", DevExeSpace(),0,(pmbp->nmb_thispack-1),ks,ke,js,je,is,ie,
        KOKKOS_LAMBDA(int m, int k, int j, int i) 
        {
            ...
            u0(m,IDN,k,j,i) = dens;
            u0(m,IM1,k,j,i) = dens*shiftedtanh(coordz*sharpness,-0.5*velocity,0.5*velocity);
            u0(m,IM2,k,j,i) = 0.0;
            u0(m,IM3,k,j,i) = 0.0;
            ...
            for(int i0=min_init_p;i0<=max_init_p;i0++)
            {
                ...
                {
                    u0(m,IM3,k,j,i)+=velocity*init_p_vel_frac*sin(2*3.141592653589*coordy*pow(2,j0))*sin(2*3.141592653589*coordx*pow(2,i0))*u0(m,IDN,k,j,i)*exp(-coordz*coordz*init_p_sharp/2);
                }
            }
...
void CoolingSrc(Mesh* pm, Real bdt, Driver* pdrive, int stage)
{
    ...
    Kokkos::parallel_reduce("TRML_cooling_src", Kokkos::RangePolicy<>(DevExeSpace(), 0, nmkji),
    KOKKOS_LAMBDA(const int &idx, Real &stage_cooling)
    {
        ...
        if(beta!=0.0)
        {
            Real partofcooling = 1-pow(temp/temp_at_t_cool_min,-beta)*bdt/t_cool_min*beta;
            if (partofcooling > 0)
...
void CoolingTimestep(Mesh* pm)
{
    if (glob_custom_min_timestep>0.0) pm->pgen->dtnew = glob_custom_min_timestep;
    else pm->pgen->dtnew = glob_t_cool_min;
}
...
void TRMLZBoundary(Mesh *pm)
{
    ...
    par_for("TRML_boundary", DevExeSpace(),0,(pmbp->nmb_thispack-1),js,je,is,ie,
    KOKKOS_LAMBDA(int m, int j, int i) 
    {
        if(mb_bcs.d_view(m,BoundaryFace::inner_x3) == BoundaryFlag::user)
        {
            for (int k=0; k<ng; k++) 
            {
                ...
                u0(m,IM2,ghost_inner_k,j,i) = rho_cold*u0(m,IM2,ks,j,i)/u0(m,IDN,ks,j,i);
                u0(m,IM3,ghost_inner_k,j,i) = rho_cold*u0(m,IM3,ks,j,i)/u0(m,IDN,ks,j,i);
                u0(m,IEN,ghost_inner_k,j,i) = pres/gm1 +
                0.5*(SQR(u0(m,IM1,ghost_inner_k,j,i)) + SQR(u0(m,IM2,ghost_inner_k,j,i)) +
                SQR(u0(m,IM3,ghost_inner_k,j,i)))/u0(m,IDN,ghost_inner_k,j,i);
            }
        }
        ...
        if(mb_bcs.d_view(m,BoundaryFace::outer_x3) == BoundaryFlag::user)
        {
            for (int k=0; k<ng; k++) 
            {
                ...
                u0(m,IM2,ghost_outer_k,j,i) = rho_hot*u0(m,IM2,ke,j,i)/u0(m,IDN,ke,j,i);
                u0(m,IM3,ghost_outer_k,j,i) = rho_hot*u0(m,IM3,ke,j,i)/u0(m,IDN,ke,j,i);
                u0(m,IEN,ghost_outer_k,j,i) = pres/gm1 +
                0.5*(SQR(u0(m,IM1,ghost_outer_k,j,i)) + SQR(u0(m,IM2,ghost_outer_k,j,i)) +
                SQR(u0(m,IM3,ghost_outer_k,j,i)))/u0(m,IDN,ghost_outer_k,j,i);
            }
        }
    });
}
...
struct reduction_identity< array_sum::array_type<Real, TRMLHISTVARS> > {
  KOKKOS_FORCEINLINE_FUNCTION static array_sum::array_type<Real, TRMLHISTVARS> sum() {
...
void HistoryOutput(HistoryData *pdata, Mesh *pm)
{
    ...
    Kokkos::parallel_reduce("TRML_Hist",Kokkos::RangePolicy<>(DevExeSpace(), 0, nmkji),
    KOKKOS_LAMBDA(const int &idx, array_sum::array_type<Real, TRMLHISTVARS> &mb_sum,
    Real &min_z_intermediate_, Real &max_z_intermediate_, Real &min_z_shear_, Real &max_z_shear_,
    Real &min_z_vely_, Real &max_z_vely_, int &cnt_z_intermediate_, int &cnt_z_shear_,
    int &cnt_z_vely_) 
    {
        int m = (idx)/nkji;
        int k = (idx - m*nkji)/nji;
        ...
        int temp_bin;
        if(temp/T_cold<T_ci_over_T_cold) temp_bin = 0;
        else if(temp/T_cold>=T_ci_over_T_cold && temp/T_cold<T_ih_over_T_cold) temp_bin = 1;
        else temp_bin = 2;
