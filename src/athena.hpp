...
enum PhysicsModule {HydroDynamics, MagnetoHydroDynamics, FrameTracking,
...
inline void par_for_outer(const std::string &name, DevExeSpace exec_space,
  ...
  Kokkos::TeamPolicy<> policy(exec_space, nk, 512);
#else
...
inline void par_for_outer(const std::string &name, DevExeSpace exec_space,
  ...
  const int nkj = nk*nj;
#if defined(KOKKOS_ARCH_VEGA90A)
  Kokkos::TeamPolicy<> policy(exec_space, nkj, 512);
#else
...
#if defined(KOKKOS_ARCH_VEGA90A)
  Kokkos::TeamPolicy<> policy(exec_space, nnkj, 512);
#else
...
#if defined(KOKKOS_ARCH_VEGA90A)
  Kokkos::TeamPolicy<> policy(exec_space, nmnkj, 512);
#else
